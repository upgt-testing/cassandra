...
public class CASMultiDCTest_RestartInjected_Randomized_123 {
    ...
    public static void beforeClass() throws Throwable {
        ...
        Consumer<IInstanceConfig> conf = config -> config.with(Feature.NETWORK).set("paxos_variant", "v2").set("write_request_timeout_in_ms", 1000L).set("cas_contention_timeout_in_ms", 1000L).set("request_timeout_in_ms", 1000L);
        ...
        CLUSTER.forEach(i -> i.runOnInstance(() -> {
            ...
            Assert.assertTrue(PaxosCommit.getEnableDcLocalCommit());
        }));
    }
    ...
    public void testLocalSerialLocalCommit() {
        ...
        RestartFramework.at("before_cas_operation").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_cas_operation").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }
    ...
    public void testDcLocalCommitDisabled() {
        ...
        RestartFramework.at("after_dc_local_commit_disabled").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
