...
public class PrepareBatchStatementsTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void testPreparedBatch() throws Exception {
        try (ICluster<IInvokableInstance> c = init(builder().withNodes(1).withConfig(config -> config.with(GOSSIP, NETWORK, NATIVE_PROTOCOL)).start())) {
            try (com.datastax.driver.core.Cluster cluster = com.datastax.driver.core.Cluster.builder().addContactPoint("127.0.0.1").build();
                ...
                RestartFramework.at("after_schema_creation").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                ...
                RestartFramework.at("after_first_batch_exec").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                ...
                prepared = s.prepare(batch1);
                s.execute(prepared.bind(1, 1, 1, 1, 1, 1));
                RestartFramework.at("after_second_batch_exec").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                c.get(1).runOnInstance(() -> {
