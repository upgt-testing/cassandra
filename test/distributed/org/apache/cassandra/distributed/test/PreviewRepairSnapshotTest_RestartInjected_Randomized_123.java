...
public class PreviewRepairSnapshotTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public void testSnapshotOfSStablesContainingMismatchingTokens() throws IOException {
        try (Cluster cluster = init(Cluster.build(2).withConfig(config -> config.set("snapshot_on_repaired_data_mismatch", true).with(GOSSIP).with(NETWORK)).start())) {
            ...
            for (int i = 0; i < 100; i++) {
                ...
                for (int j = 0; j < 10; ++j) cluster.coordinator(1).execute(withKeyspace("insert into %s.tbl (id) values (?)"), ConsistencyLevel.ALL, matchingHashBlob(i, j));
            ...
            for (int i = 1; i <= 2; i++) markRepaired(cluster, i);
            RestartFramework.at("after_mark_repaired_initial").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.get(1).nodetoolResult("repair", "-vd", "-pr", KEYSPACE, "tbl").asserts().success().stdoutContains("Repaired data is in sync");
            RestartFramework.at("after_first_preview_repair").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_mismatch_insert").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            markRepaired(cluster, 2);
            RestartFramework.at("after_mark_repaired_node2").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            cluster.get(1).nodetoolResult("repair", "-vd", KEYSPACE, "tbl").asserts().success().stdoutContains("Repaired data is inconsistent");
            RestartFramework.at("after_final_repair").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private IIsolatedExecutor.SerializableRunnable checkSnapshot(Set<Token> mismatchingTokens, int expectedSnapshotSize) {
        return () -> {
            ...
            String snapshotTag = await().atMost(1, MINUTES).pollInterval(100, MILLISECONDS).until(() -> {
