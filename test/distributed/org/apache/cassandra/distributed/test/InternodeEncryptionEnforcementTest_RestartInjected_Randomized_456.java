...
public final class InternodeEncryptionEnforcementTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void testInboundConnectionsAreRejectedWhenAuthFails() throws IOException, TimeoutException {
        ...
        try (Cluster cluster = builder.start();
            ...
            RestartFramework.at("after_open_connections").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            /*
            ...
            RestartFramework.at("after_auth_verification").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public void testOutboundConnectionsAreInterruptedWhenAuthFails() throws IOException, TimeoutException {
        ...
        try (Cluster cluster = builder.start();
            ...
            RestartFramework.at("after_logs_watch").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            /*
    ...
    public void testConnectionsAreRejectedWithInvalidConfig() throws Throwable {
        ...
        try (Cluster cluster = builder.start()) {
            ...
            RestartFramework.at("after_connection_attempt").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            /*
            ...
            RestartFramework.at("after_instance1_verification").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.get(2).runOnInstance(() -> {
                assertTrue(MessagingService.instance().messageHandlers.isEmpty());
                OutboundConnections outbound = getOnlyElement(MessagingService.instance().channelManagers.values());
                assertFalse(outbound.small.isConnected() || outbound.large.isConnected() || outbound.urgent.isConnected());
            });
            RestartFramework.at("after_instance2_verification").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    private void openConnections(Cluster cluster) {
        cluster.schemaChange("CREATE KEYSPACE test_connections_from_1 " + "WITH replication = {'class': 'SimpleStrategy', 'replication_factor': 2};", false, cluster.get(1));
        cluster.schemaChange("CREATE KEYSPACE test_connections_from_2 " + "WITH replication = {'class': 'SimpleStrategy', 'replication_factor': 2};", false, cluster.get(2));
    }
    ...
    private Cluster.Builder createCluster(final Class authenticatorClass) {
        return builder().withNodes(2).withConfig(c -> {
            c.with(Feature.NETWORK);
            c.with(Feature.NATIVE_PROTOCOL);
            HashMap<String, Object> encryption = new HashMap<>();
            encryption.put("keystore", "test/conf/cassandra_ssl_test.keystore");
