...
public class SystemKeyspacesDataLocationTest_RestartInjected extends TestBaseImpl
{
    @Test
    public void preparedStatementsShouldNotBeMovedOnStartupTest() throws IOException, ExecutionException, InterruptedException
    {
        try (Cluster cluster = Cluster.build(1).withDataDirCount(3).start())
        {
            ...
            RestartFramework.at("after_sstables_create")
                .on(cluster)
                .restart("node")
                .withIndex(0)
            ...
            RestartFramework.at("after_initial_location_verification")
                .on(cluster)
                .restart("node")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
            ...
            RestartFramework.at("after_manual_startup")
                .on(cluster)
                .restart("node")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
