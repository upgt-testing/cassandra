...
public class LargeColumnTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    private void testLargeColumns(int nodes, int columnSize, int rowCount) throws Throwable {
        ...
        try (ICluster cluster = init(builder().withNodes(nodes).withConfig(config -> config.set("commitlog_segment_size", String.format("%dMiB", (columnSize * 3) >> 20)).set("internode_application_send_queue_reserve_endpoint_capacity", String.format("%dB", (columnSize * 2))).set("internode_application_send_queue_reserve_global_capacity", String.format("%dB", (columnSize * 3))).set("native_transport_max_request_data_in_flight_per_ip", String.format("%dB", (columnSize * 3))).set("native_transport_max_request_data_in_flight", String.format("%dB", (columnSize * 6))).set("write_request_timeout", "30s").set("read_request_timeout", "30s").set("memtable_heap_space", "1024MiB")).start())) {
            ...
            RestartFramework.at("after_inserts").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
