...
public class ReadDigestConsistencyTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public static final String CREATE_TABLE = String.format("CREATE TABLE %s.%s (" + "   k int, " + "   c int, " + "   s1 int static, " + "   s2 set<int> static, " + "   v1 int, " + "   v2 set<int>, " + "   PRIMARY KEY (k, c))", KEYSPACE, TABLE_NAME);
    ...
    public void testDigestConsistency() throws Exception {
        try (Cluster cluster = init(builder().withNodes(2).start())) {
            ...
            RestartFramework.at("after_first_digest_test").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public static void testDigestConsistency(ICoordinator coordinator) {
        ...
        String[] columnss1 = { "k, c", "*", "v1", "v2", "v1, s1", "v1, s2", "v2[3]", "v2[2..4]", "v1, s2[7]", "v1, s2[6..8]" };
        String[] columnss2 = { "s1", "s2", "s2[7]", "s2[6..8]" };
