...
public class FailedBootstrapTest_RestartInjected_Randomized_123 extends TestBaseImpl {

    private static final Logger logger = LoggerFactory.getLogger(FailedBootstrapTest_RestartInjected.class);

    private static final int NODE_TO_REMOVE = 2;

    @Test
    public void roleSetupDoesNotProduceUnavailables() throws IOException {
        Cluster.Builder builder = Cluster.build(3).withConfig(c -> c.with(Feature.values())).withInstanceInitializer(BB::install);
        ...
        try (Cluster cluster = builder.start()) {
            ...
            RestartFramework.at("after_replace_host_start").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_gossip_info").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            Awaitility.await().atMost(1, TimeUnit.MINUTES).until(// why 42?  just need something large enough to make sure multiple attempts happened
            () -> added.callOnInstance(() -> BB.SETUP_SCHEDULE_COUNTER.get()) >= 42);
            RestartFramework.at("after_await_setup_counter").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public static class BB {
        ...
        public static void install(ClassLoader classLoader, Integer num) {
            if (num != 4)
                return;
            new ByteBuddy().rebase(StreamResultFuture.class).method(named("maybeComplete")).intercept(MethodDelegation.to(BB.class)).make().load(classLoader, ClassLoadingStrategy.Default.INJECTION);
            new ByteBuddy().rebase(CassandraRoleManager.class).method(named("scheduleSetupTask")).intercept(MethodDelegation.to(BB.class)).make().load(classLoader, ClassLoadingStrategy.Default.INJECTION);
        }
