...
public class HostReplacementTest_RestartInjected_Randomized_456 extends TestBaseImpl {

    private static final Logger logger = LoggerFactory.getLogger(HostReplacementTest_RestartInjected.class);
    ...
    public void replaceDownedHost() throws IOException {
        ...
        try (Cluster cluster = Cluster.build(2).withConfig(c -> c.with(Feature.GOSSIP, Feature.NETWORK)).withTokenSupplier(node -> even.token(node == 3 ? 2 : node)).start()) {
            ...
            RestartFramework.at("after_collect_state").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_replace_start").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void replaceAliveHost() throws IOException {
        ...
        try (Cluster cluster = Cluster.build(2).withConfig(c -> c.with(Feature.GOSSIP, Feature.NETWORK).set(Constants.KEY_DTEST_API_STARTUP_FAILURE_AS_SHUTDOWN, false)).withTokenSupplier(node -> even.token(node == 3 ? 2 : node)).start()) {
            ...
            // collect rows to detect issues later on if the state doesn't match
            SimpleQueryResult expectedState = nodeToRemove.coordinator().executeWithResult("SELECT * FROM " + KEYSPACE + ".tbl", ConsistencyLevel.ALL);
            RestartFramework.at("before_replace_attempt").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            // now create a new node to replace the other node
            ...
            RestartFramework.at("after_replace_failure").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // make sure all nodes are healthy
            awaitRingHealthy(seed);
            RestartFramework.at("after_ring_healthy").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void seedGoesDownBeforeDownHost() throws IOException {
        ...
        try (Cluster cluster = Cluster.build(3).withConfig(c -> c.with(Feature.GOSSIP, Feature.NETWORK)).withTokenSupplier(node -> even.token(node == 4 ? 2 : node)).start()) {
            ...
            RestartFramework.at("after_collect_state_tokens").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            // shutdown the seed, then the node to remove
            stopUnchecked(seed);
            RestartFramework.at("after_seed_stop").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            stopUnchecked(nodeToRemove);
            RestartFramework.at("after_node_stop").on(cluster).restart("node").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
            // restart the seed
            seed.startup();
            ...
            RestartFramework.at("after_assert_in_ring").on(cluster).restart("node").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_token_verification").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // now create a new node to replace the other node
            ...
            RestartFramework.at("after_ring_join").on(cluster).restart("node").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
            // make sure all nodes are healthy
