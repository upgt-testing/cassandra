...
public class HostReplacementOfDownedClusterTest_RestartInjected_Randomized_123 extends TestBaseImpl {

    private static final Logger logger = LoggerFactory.getLogger(HostReplacementOfDownedClusterTest_RestartInjected.class);
    ...
    public void hostReplacementOfDeadNode() throws IOException {
        ...
        try (Cluster cluster = Cluster.build(2).withConfig(c -> c.with(Feature.GOSSIP, Feature.NETWORK)).withTokenSupplier(node -> even.token(node == 3 ? 2 : node)).start()) {
            ...
            RestartFramework.at("after_collect_state").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // now stop all nodes
            stopAll(cluster);
            RestartFramework.at("after_stop_all").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // with all nodes down, now start the seed (should be first node)
            seed.startup();
            RestartFramework.at("after_seed_startup").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // at this point node2 should be known in gossip, but with generation/version of 0
            assertGossipInfo(seed, addressToReplace, 0, -1);
            RestartFramework.at("after_gossip_check").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_token_check").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // now create a new node to replace the other node
            IInvokableInstance replacingNode = replaceHostAndStart(cluster, nodeToRemove);
            RestartFramework.at("after_replace_host").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            awaitRingJoin(seed, replacingNode);
            awaitRingJoin(replacingNode, seed);
            RestartFramework.at("after_await_ring_join").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertNotInRing(seed, nodeToRemove);
            logger.info("Current ring is {}", assertNotInRing(replacingNode, nodeToRemove));
            RestartFramework.at("after_assert_not_in_ring").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void hostReplacementOfDeadNodeAndOtherNodeStartsAfter() throws IOException {
        ...
        try (Cluster cluster = Cluster.build(numStartNodes).withConfig(c -> c.with(Feature.GOSSIP, Feature.NETWORK)).withTokenSupplier(node -> even.token(node == (numStartNodes + 1) ? 2 : node)).start()) {
            ...
            InetSocketAddress addressToReplace = nodeToRemove.broadcastAddress();
            setupCluster(cluster);
            RestartFramework.at("after_cluster_setup_2").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // collect rows/tokens to detect issues later on if the state doesn't match
            SimpleQueryResult expectedState = nodeToRemove.coordinator().executeWithResult("SELECT * FROM " + KEYSPACE + ".tbl", ConsistencyLevel.ALL);
            List<String> beforeCrashTokens = getTokenMetadataTokens(seed);
            RestartFramework.at("after_collect_state_2").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // now stop all nodes
            stopAll(cluster);
            RestartFramework.at("after_stop_all_2").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // with all nodes down, now start the seed (should be first node)
            seed.startup();
            RestartFramework.at("after_seed_startup_2").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // at this point node2 should be known in gossip, but with generation/version of 0
            assertGossipInfo(seed, addressToReplace, 0, -1);
            RestartFramework.at("after_gossip_check_2").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // make sure node1 still has node2's tokens
            List<String> currentTokens = getTokenMetadataTokens(seed);
            Assertions.assertThat(currentTokens).as("Tokens no longer match after restarting").isEqualTo(beforeCrashTokens);
            RestartFramework.at("after_token_check_2").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // now create a new node to replace the other node
            IInvokableInstance replacingNode = replaceHostAndStart(cluster, nodeToRemove);
            RestartFramework.at("after_replace_host_2").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // wait till the replacing node is in the ring
            awaitRingJoin(seed, replacingNode);
            awaitRingJoin(replacingNode, seed);
            RestartFramework.at("after_await_ring_join_2").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // we see that the replaced node is properly in the ring, now lets add the other node back
            nodeToStartAfterReplace.startup();
            RestartFramework.at("after_third_node_startup").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            awaitRingJoin(seed, nodeToStartAfterReplace);
            awaitRingJoin(replacingNode, nodeToStartAfterReplace);
            RestartFramework.at("after_third_node_ring_join").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // make sure all nodes are healthy
            awaitRingHealthy(seed);
            RestartFramework.at("after_ring_healthy").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_assert_ring_2").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            validateRows(seed.coordinator(), expectedState);
            validateRows(replacingNode.coordinator(), expectedState);
        }
    }
