...
public class HostReplacementAbruptDownedInstanceTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public void hostReplaceAbruptShutdown() throws IOException {
        ...
        try (Cluster cluster = Cluster.build(numStartNodes).withConfig(c -> c.with(Feature.GOSSIP, Feature.NETWORK)).withTokenSupplier(node -> even.token(node == (numStartNodes + 1) ? 2 : node)).start()) {
            ...
            RestartFramework.at("after_collect_expected_state").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_await_ring_healthy").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_assert_ring").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
