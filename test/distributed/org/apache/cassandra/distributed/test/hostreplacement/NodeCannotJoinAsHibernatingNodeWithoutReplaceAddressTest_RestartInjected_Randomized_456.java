...
public class NodeCannotJoinAsHibernatingNodeWithoutReplaceAddressTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void test() throws IOException, InterruptedException {
        ...
        try (Cluster cluster = init(Cluster.build(2).withConfig(c -> c.with(Feature.values()).set(Constants.KEY_DTEST_API_STARTUP_FAILURE_AS_SHUTDOWN, false)).withInstanceInitializer(BBHelper::install).withTokenSupplier(node -> even.token((node == 3 || node == 4) ? 2 : node)).start())) {
            ...
            RestartFramework.at("before_startup_attempt").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public static class BBHelper {
        ...
        private static void shutdownBeforeNormal(ClassLoader cl) {
            new ByteBuddy().rebase(PendingRangeCalculatorService.class).method(named("blockUntilFinished")).intercept(MethodDelegation.to(ShutdownBeforeNormal.class)).make().load(cl, ClassLoadingStrategy.Default.INJECTION);
        }
    }
