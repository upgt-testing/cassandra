...
public class LeveledCompactionTaskTest_RestartInjected_Randomized_123 extends TestBaseImpl {

    @Test
    public void testBuildCompactionCandidatesForAvailableDiskSpace() throws IOException {
        try (Cluster cluster = init(builder().withNodes(1).withConfig(config -> config.set("autocompaction_on_startup_enabled", false)).withInstanceInitializer(BB::install).start())) {
            ...
            RestartFramework.at("after_exception_filter_setup").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
