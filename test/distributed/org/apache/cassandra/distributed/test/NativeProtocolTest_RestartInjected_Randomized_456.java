...
public class NativeProtocolTest_RestartInjected_Randomized_456 extends TestBaseImpl {

    @Test
    public void withClientRequests() throws Throwable {
        try (ICluster cassandraCluster = init(builder().withNodes(3).withConfig(config -> config.with(GOSSIP, NETWORK, NATIVE_PROTOCOL)).start())) {
            try (com.datastax.driver.core.Cluster cluster = com.datastax.driver.core.Cluster.builder().addContactPoint("127.0.0.1").build();
                ...
                RestartFramework.at("after_table_create").on((Cluster) cassandraCluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                session.execute("INSERT INTO " + KEYSPACE + ".tbl (pk, ck, v) values (1,1,1);");
                RestartFramework.at("after_insert").on((Cluster) cassandraCluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void withCounters() throws Throwable {
        try (ICluster cassandraCluster = init(builder().withNodes(3).withConfig(config -> config.with(GOSSIP, NETWORK, NATIVE_PROTOCOL)).start())) {
            ...
            RestartFramework.at("after_counter_table_create").on((Cluster) cassandraCluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            session.execute("UPDATE " + KEYSPACE + ".tbl set ck = ck + 10 where pk = 1;");
            RestartFramework.at("after_counter_update").on((Cluster) cassandraCluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Statement select = new SimpleStatement("select * from " + KEYSPACE + ".tbl;").setConsistencyLevel(ConsistencyLevel.ALL);
            final ResultSet resultSet = session.execute(select);
    ...
    public void restartTransportOnGossippingOnlyMember() throws Throwable {
        ...
        try (Cluster cluster = builder().withNodes(originalNodeCount).withTokenSupplier(evenlyDistributedTokens(expandedNodeCount, 1)).withNodeIdTopology(singleDcNetworkTopology(expandedNodeCount, "dc0", "rack0")).withConfig(config -> config.with(NETWORK, GOSSIP, NATIVE_PROTOCOL)).start()) {
            ...
            RestartFramework.at("after_gossipping_member_startup").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertTrue(gossippingOnlyMember.callOnInstance((IIsolatedExecutor.SerializableCallable<Boolean>) () -> StorageService.instance.isNativeTransportRunning()));
            gossippingOnlyMember.runOnInstance((IIsolatedExecutor.SerializableRunnable) () -> StorageService.instance.stopNativeTransport());
            RestartFramework.at("after_stop_native_transport").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertFalse(gossippingOnlyMember.callOnInstance((IIsolatedExecutor.SerializableCallable<Boolean>) () -> StorageService.instance.isNativeTransportRunning()));
            gossippingOnlyMember.runOnInstance((IIsolatedExecutor.SerializableRunnable) () -> StorageService.instance.startNativeTransport());
            RestartFramework.at("after_start_native_transport").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertTrue(gossippingOnlyMember.callOnInstance((IIsolatedExecutor.SerializableCallable<Boolean>) () -> StorageService.instance.isNativeTransportRunning()));
        }
    }
    ...
    public void testBinaryReflectsRpcReadiness() throws Throwable {
        try (Cluster cluster = builder().withNodes(1).withConfig(config -> config.with(NETWORK, GOSSIP, NATIVE_PROTOCOL).set("start_native_transport", "false")).start()) {
            IInvokableInstance i = cluster.get(1);
            RestartFramework.at("after_cluster_start_transport_disabled").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_enable_native_transport").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_disable_native_transport").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertTrue(i.callOnInstance((IIsolatedExecutor.SerializableCallable<Boolean>) () -> StorageService.instance.isRpcReady(FBUtilities.getBroadcastAddressAndPort())));
        }
    }
}
