...
public class ReadFailureTest_RestartInjected extends TestBaseImpl
{
    ...
    public void testSpecExecRace() throws Throwable
    {
        try (Cluster cluster = init(Cluster.build().withNodes(2).withConfig(config -> config.set("tombstone_failure_threshold", TOMBSTONE_FAIL_THRESHOLD)).start()))
        {
            ...
            RestartFramework.at("after_tombstone_creation")
                .on(cluster)
                .restart("node")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
            ...
            RestartFramework.at("after_first_loop")
                .on(cluster)
                .restart("node")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
            ...
            RestartFramework.at("after_speculation_update")
                .on(cluster)
                .restart("node")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
