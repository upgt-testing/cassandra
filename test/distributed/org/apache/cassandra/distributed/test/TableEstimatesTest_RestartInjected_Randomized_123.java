...
public class TableEstimatesTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public void refreshTableEstimatesClearsInvalidEntries() {
        RestartFramework.at("after_invalid_table_estimates_inserts").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_refresh_table_estimates").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_first_table_estimates_verification").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Assertions.assertThat(qr).isExhausted();
    }
    ...
    public void refreshSizeEstimatesClearsInvalidEntries() {
        ...
        RestartFramework.at("after_refresh_size_estimates").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        node.executeInternal(size_estimatesInsert, "bad_keyspace", "bad_table", "-5", "5", 0L, 0L);
        RestartFramework.at("after_invalid_size_estimates_inserts").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_first_size_estimates_verification").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        node.nodetoolResult("refreshsizeestimates").asserts().success();
