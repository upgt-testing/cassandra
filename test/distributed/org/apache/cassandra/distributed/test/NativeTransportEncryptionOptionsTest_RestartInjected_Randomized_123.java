...
public class NativeTransportEncryptionOptionsTest_RestartInjected_Randomized_123 extends AbstractEncryptionOptionsImpl {
    ...
    public void nodeWillNotStartWithBadKeystore() throws Throwable {
        try (Cluster cluster = builder().withNodes(1).withConfig(c -> {
            ...
            c.set("client_encryption_options", ImmutableMap.of("optional", true, "keystore", "/path/to/bad/keystore/that/should/not/exist", "truststore", "/path/to/bad/truststore/that/should/not/exist"));
    ...
    public void optionalTlsConnectionDisabledWithoutKeystoreTest() throws Throwable {
        try (Cluster cluster = builder().withNodes(1).withConfig(c -> c.with(Feature.NATIVE_PROTOCOL)).createWithoutStarting()) {
            ...
            cluster.startup();
            RestartFramework.at("after_cluster_startup").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_tls_connection_test").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public void optionalTlsConnectionAllowedWithKeystoreTest() throws Throwable {
        try (Cluster cluster = builder().withNodes(1).withConfig(c -> {
            ...
            tlsConnection.assertCannotConnect();
            cluster.startup();
            RestartFramework.at("after_cluster_startup_with_keystore").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_tls_negotiation").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public void optionalTlsConnectionAllowedToRegularPortTest() throws Throwable {
        try (Cluster cluster = builder().withNodes(1).withConfig(c -> {
            ...
            cluster.startup();
            RestartFramework.at("after_cluster_startup_with_ssl_port").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_encrypted_port_connection").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Assert.assertEquals("TLS native connection should not be possible on the regular port if an SSL port is specified", ConnectResult.FAILED_TO_NEGOTIATE, connectionToUnencryptedPort.connect());
            RestartFramework.at("after_unencrypted_port_test").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public void unencryptedNativeConnectionNotlisteningOnTlsPortTest() throws Throwable {
        try (Cluster cluster = builder().withNodes(1).withConfig(c -> {
            c.with(Feature.NATIVE_PROTOCOL);
            c.set("native_transport_port_ssl", 9043);
            c.set("client_encryption_options", ImmutableMap.builder().putAll(validKeystore).put("enabled", false).put("optional", false).build());
        }).createWithoutStarting()) {
            assertCannotStartDueToConfigurationException(cluster);
        }
    }
    ...
    public void negotiatedProtocolMustBeAcceptedProtocolTest() throws Throwable {
        try (Cluster cluster = builder().withNodes(1).withConfig(c -> {
            c.with(Feature.NATIVE_PROTOCOL);
            c.set("client_encryption_options", ImmutableMap.builder().putAll(validKeystore).put("enabled", true).put("accepted_protocols", ImmutableList.of("TLSv1.1", "TLSv1.2", "TLSv1.3")).build());
        }).start()) {
            InetAddress address = cluster.get(1).config().broadcastAddress().getAddress();
            int port = (int) cluster.get(1).config().get("native_transport_port");
            ...
            RestartFramework.at("after_tls10_connection_test").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_tls11_connection_test").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            TlsConnection tls12Connection = new TlsConnection(address.getHostAddress(), port, Collections.singletonList("TLSv1.2"));
            Assert.assertEquals("Should be possible to establish a TLSv1.2 connection", ConnectResult.NEGOTIATED, tls12Connection.connect());
            Assert.assertEquals("TLSv1.2", tls12Connection.lastProtocol());
            RestartFramework.at("after_tls12_connection_test").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            TlsConnection tls13Connection = new TlsConnection(address.getHostAddress(), port, Collections.singletonList("TLSv1.3"));
            Assert.assertEquals("Should be possible to establish a TLSv1.3 connection", ConnectResult.NEGOTIATED, tls13Connection.connect());
            Assert.assertEquals("TLSv1.3", tls13Connection.lastProtocol());
            RestartFramework.at("after_tls13_connection_test").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public void connectionCannotAgreeOnClientAndServerTest() throws Throwable {
        try (Cluster cluster = builder().withNodes(1).withConfig(c -> {
            ...
            RestartFramework.at("after_cipher_mismatch_test").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public void nodeMustNotStartWithNonExistantProtocolTest() throws Throwable {
        try (Cluster cluster = builder().withNodes(1).withConfig(c -> {
            c.with(Feature.NATIVE_PROTOCOL);
            c.set("client_encryption_options", ImmutableMap.<String, Object>builder().putAll(nonExistantProtocol).put("enabled", true).build());
        }).createWithoutStarting()) {
            assertCannotStartDueToConfigurationException(cluster);
        }
    }
    ...
    public void nodeMustNotStartWithNonExistantCiphersTest() throws Throwable {
        try (Cluster cluster = builder().withNodes(1).withConfig(c -> {
            c.with(Feature.NATIVE_PROTOCOL);
            c.set("client_encryption_options", ImmutableMap.<String, Object>builder().putAll(nonExistantCipher).put("enabled", true).build());
        }).createWithoutStarting()) {
    ...
    private void testEndpointVerification(boolean requireEndpointVerification, boolean ipInSAN) throws Throwable {
        try (Cluster cluster = builder().withNodes(1).withConfig(c -> {
            c.with(Feature.NATIVE_PROTOCOL);
            c.set("client_encryption_options", ImmutableMap.builder().putAll(validKeystore).put("enabled", true).put("require_client_auth", true).put("require_endpoint_verification", requireEndpointVerification).build());
        }).start()) {
            RestartFramework.at("after_cluster_start_endpoint_verification").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            InetAddress address = cluster.get(1).config().broadcastAddress().getAddress();
            ...
            SslContext sslContext = sslContextBuilder.trustManager(createTrustManagerFactory("test/conf/cassandra_ssl_test.truststore", "cassandra")).build();
            RestartFramework.at("after_ssl_context_creation").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            final SSLOptions sslOptions = socketChannel -> sslContext.newHandler(socketChannel.alloc());
            com.datastax.driver.core.Cluster driverCluster = com.datastax.driver.core.Cluster.builder().addContactPoint(address.getHostAddress()).withSSL(sslOptions).build();
            if (!ipInSAN) {
                expectedException.expect(NoHostAvailableException.class);
            }
            driverCluster.connect();
            RestartFramework.at("after_driver_connection").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }

    private KeyManagerFactory createKeyManagerFactory(final String keyStorePath, final String keyStorePassword) throws Exception {
    ...
    private TrustManagerFactory createTrustManagerFactory(final String trustStorePath, final String trustStorePassword) throws Exception {
