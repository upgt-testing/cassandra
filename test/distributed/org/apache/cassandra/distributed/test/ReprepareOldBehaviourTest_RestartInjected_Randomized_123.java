...
public class ReprepareOldBehaviourTest_RestartInjected_Randomized_123 extends ReprepareTestBase {

    @Test
    public void testReprepareMixedVersion() throws Throwable {
        testReprepare(PrepareBehaviour::oldBehaviour, cfg(true, true), cfg(false, false));
    }
    ...
    public void testReprepareUsingOldBehavior() throws Throwable {
        ...
        try (ICluster<IInvokableInstance> c = init(builder().withNodes(2).withConfig(config -> config.with(GOSSIP, NETWORK, NATIVE_PROTOCOL)).withInstanceInitializer(PrepareBehaviour::oldBehaviour).start())) {
            ...
            try (com.datastax.driver.core.Cluster cluster = com.datastax.driver.core.Cluster.builder().addContactPoint("127.0.0.1").addContactPoint("127.0.0.2").withLoadBalancingPolicy(lbp).build();
                ...
                RestartFramework.at("after_prepare").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                session.execute(select.bind());
                RestartFramework.at("after_first_execute").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                lbp.setPrimary(1);
                RestartFramework.at("after_switch_node").on(c).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
                session.execute(select.bind());
            }
        }
    }
    ...
    public void testReprepareMixedVersionWithoutReset() throws Throwable {
        try (ICluster<IInvokableInstance> c = init(builder().withNodes(2).withConfig(config -> config.with(GOSSIP, NETWORK, NATIVE_PROTOCOL)).withInstanceInitializer(PrepareBehaviour::oldBehaviour).start())) {
            ForceHostLoadBalancingPolicy lbp = new ForceHostLoadBalancingPolicy();
            c.schemaChange(withKeyspace("CREATE TABLE %s.tbl (pk int, ck int, v int, PRIMARY KEY (pk, ck));"));
            RestartFramework.at("after_table_create_mixed").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // 1 has old behaviour
                ...
                for (boolean withUse : new boolean[] { true, false }) {
                    for (boolean clearBetweenExecutions : new boolean[] { true, false }) {
                        try (com.datastax.driver.core.Cluster cluster = com.datastax.driver.core.Cluster.builder().addContactPoint("127.0.0.1").addContactPoint("127.0.0.2").withLoadBalancingPolicy(lbp).build();
                            ...
                            final PreparedStatement select = session.prepare(withKeyspace("SELECT * FROM %s.tbl"));
                            RestartFramework.at("after_prepare_mixed").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                            session.execute(select.bind());
                            RestartFramework.at("after_first_execute_mixed").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                            ...
                            session.execute(select.bind());
                            RestartFramework.at("after_second_execute_mixed").on(c).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
                            if (clearBetweenExecutions)
                                c.get(2).runOnInstance(QueryProcessor::clearPreparedStatementsCache);
                            lbp.setPrimary(firstContact);
                            session.execute(select.bind());
                            RestartFramework.at("after_third_execute_mixed").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                            c.get(2).runOnInstance(QueryProcessor::clearPreparedStatementsCache);
                        }
                    }
                }
