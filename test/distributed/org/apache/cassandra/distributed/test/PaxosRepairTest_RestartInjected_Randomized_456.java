...
public class PaxosRepairTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    private static void assertAllAlive(Cluster cluster) {
        ...
        cluster.stream().forEach(instance -> {
            instance.runOnInstance(() -> {
                ...
                for (InetAddressAndPort endpoint : endpoints) Assert.assertTrue(FailureDetector.instance.isAlive(endpoint));
            });
        });
    }
    ...
    public void paxosRepairTest() throws Throwable {
        ...
        try (Cluster cluster = init(Cluster.build(3).withConfig(WITH_NETWORK).withoutVNodes().start())) {
            ...
            RestartFramework.at("after_first_cas").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_final_cas").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.forEach(i -> i.runOnInstance(() -> {
                compactPaxos();
                Map<Integer, PaxosRow> rows = getPaxosRows();
                assertLowBoundPurged(rows.values());
    ...
    public void topologyChangePaxosTest() throws Throwable {
        ...
        try (Cluster cluster = Cluster.build(4).withConfig(WITH_NETWORK).withoutVNodes().createWithoutStarting()) {
            for (int i = 1; i <= 3; i++) cluster.get(i).startup();
            init(cluster);
            cluster.schemaChange("CREATE TABLE " + KEYSPACE + '.' + TABLE + " (pk int, ck int, v int, PRIMARY KEY (pk, ck))");
            RestartFramework.at("after_table_create").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.coordinator(1).execute("INSERT INTO " + KEYSPACE + '.' + TABLE + " (pk, ck, v) VALUES (1, 1, 1) IF NOT EXISTS", ConsistencyLevel.QUORUM);
    ...
    public void paxosCleanupWithReproposal() throws Throwable {
        // TODO: fails with vnode enabled
        try (Cluster cluster = init(Cluster.build(3).withConfig(cfg -> cfg.set("paxos_variant", "v2").set("paxos_purge_grace_period", "0s").set("paxos_state_purging", Config.PaxosStatePurging.repaired.toString()).set("truncate_request_timeout", "1000ms")).withoutVNodes().start())) {
            cluster.schemaChange("CREATE TABLE " + KEYSPACE + '.' + TABLE + " (pk int, ck int, v int, PRIMARY KEY (pk, ck))");
            RestartFramework.at("after_table_create").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.verbs(PAXOS_COMMIT_REQ).drop();
            try {
            ...
            RestartFramework.at("after_paxos_flush").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_cleanup").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Assert.assertFalse(hasUncommitted(cluster, KEYSPACE, TABLE));
            cluster.forEach(i -> i.runOnInstance(PaxosRepairTest_RestartInjected::compactPaxos));
            for (int i = 1; i <= 3; ++i) assertRows(cluster.get(i).executeInternal("SELECT * FROM " + KEYSPACE + '.' + TABLE + " WHERE pk = 1"), row(1, 1, 1));
            Assert.assertFalse(hasUncommittedQuorum(cluster, KEYSPACE, TABLE));
    ...
    public void paxosCleanupWithReproposalClashingTimestamp() throws Throwable {
        // TODO: fails with vnode enabled
        try (Cluster cluster = init(Cluster.build(5).withConfig(cfg -> cfg.set("paxos_variant", "v2").set("paxos_purge_grace_period", "0s").set("paxos_cache_size", "0MiB").set("truncate_request_timeout", "1000ms")).withoutVNodes().start())) {
            cluster.schemaChange("CREATE TABLE " + KEYSPACE + '.' + TABLE + " (pk int, ck int, v int, PRIMARY KEY (pk, ck))");
            RestartFramework.at("after_table_create").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // we ensure:
            ...
            cluster.verbs(PAXOS2_PREPARE_RSP, PAXOS2_PROPOSE_RSP, PAXOS_COMMIT_RSP).outbound().to(1).messagesMatching((from, to, msg) -> {
                if (fetchResponseIds.get(from) == msg.id()) {
                    if (from == 1)
                        ...
                        Uninterruptibles.awaitUninterruptibly(haveFetchedClashingRepair);
                }
                return false;
            }).drop();
    ...
    private static void assertRepairFailsWithVersion(Cluster cluster, String version) {
        for (int i = 1; i <= cluster.size(); ++i) setVersion(cluster.get(i), cluster.get(2).broadcastAddress(), version);
        try {
            repair(cluster, KEYSPACE, TABLE);
