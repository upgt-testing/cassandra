...
public class GossipShutdownTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public void shutdownStayDownTest() throws IOException, InterruptedException, ExecutionException {
        ...
        try (Cluster cluster = init(builder().withNodes(2).withConfig(config -> config.with(GOSSIP).with(NETWORK)).start())) {
            ...
            sleep(10000);
            f.get();
            RestartFramework.at("after_gossip_exchange").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } finally {
            es.shutdown();
        }
    }
