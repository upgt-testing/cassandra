 ...
 */
public class FailingResponseDoesNotLogTest_RestartInjected_Randomized_456 extends TestBaseImpl {

    @BeforeClass
    public static // need to make sure not to conflict with TestBaseImpl.beforeClass
    void // need to make sure not to conflict with TestBaseImpl.beforeClass
    beforeClassTopLevel() {
    ...
    public void dispatcherErrorDoesNotLock() throws IOException {
        try (WithProperties properties = new WithProperties().set(CUSTOM_QUERY_HANDLER_CLASS, AlwaysRejectErrorQueryHandler.class.getName());
            Cluster cluster = Cluster.build(1).withConfig(c -> c.with(Feature.NATIVE_PROTOCOL, Feature.GOSSIP).set("client_error_reporting_exclusions", ImmutableMap.of("subnets", Collections.singletonList("127.0.0.1")))).start()) {
            ...
            RestartFramework.at("after_failed_query").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
