 ...
 */
public class UpdateSystemAuthAfterDCExpansionTest_RestartInjected_Randomized_456 extends TestBaseImpl {

    static final Logger logger = LoggerFactory.getLogger(UpdateSystemAuthAfterDCExpansionTest_RestartInjected.class);

    static final String username = "shinynewuser";

    static void assertRolePresent(IInstance instance) {
        assertRows(instance.executeInternal(String.format("SELECT role FROM %s.%s WHERE role = ?", SchemaConstants.AUTH_KEYSPACE_NAME, ROLES), username), row(username));
    }

    static void assertRoleAbsent(IInstance instance) {
        assertRows(instance.executeInternal(String.format("SELECT role FROM %s.%s WHERE role = ?", SchemaConstants.AUTH_KEYSPACE_NAME, ROLES), username));
    }
    ...
    String alterKeyspaceStatement(String ntsOptions) {
        return String.format("ALTER KEYSPACE " + SchemaConstants.AUTH_KEYSPACE_NAME + " WITH replication = {'class': 'NetworkTopologyStrategy', %s};", ntsOptions);
    }
    ...
    public void validateExpandAndContract(String initialDatacenters, String expandedDatacenters, String beforeDecommissionedDatacenters, String afterDecommissionedDatacenters) throws Throwable {
        try (Cluster cluster = Cluster.build(1).withConfig(config -> config.set("auto_bootstrap", true).with(GOSSIP).with(NETWORK)).withTokenSupplier(TokenSupplier.evenlyDistributedTokens(2)).withNodeIdTopology(networkTopology(2, (nodeid) -> nodeid % 2 == 1 ? dcAndRack("dc1", "rack1") : dcAndRack("dc2", "rack2"))).withNodes(1).createWithoutStarting()) {
            ...
            cluster.get(1).runOnInstance(() -> DatabaseDescriptor.getRoleManager().createRole(AuthenticatedUser.SYSTEM_USER, RoleResource.role(username), new RoleOptions()));
            RestartFramework.at("after_role_creation").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_initial_keyspace_alter").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_expanded_keyspace_alter").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_role_replication_check").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_before_decommission_keyspace_alter").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_remove_node").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_final_keyspace_alter").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
