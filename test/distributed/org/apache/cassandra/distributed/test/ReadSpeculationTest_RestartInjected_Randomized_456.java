...
public class ReadSpeculationTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void speculateTest() throws Throwable {
        try (Cluster cluster = builder().withNodes(3).start()) {
            ...
            cluster.get(1).runOnInstance(() -> {
                ...
                new TestScenario(12_000, 12_000, TimeUnit.SECONDS.toMicros(3), Config.CQLStartTime.REQUEST, TimeUnit.SECONDS.toNanos(2), TimeUnit.SECONDS.toNanos(1)).assertWillSpeculate();
                ...
                new TestScenario(12_000, 1_500, TimeUnit.SECONDS.toMicros(2), Config.CQLStartTime.REQUEST, TimeUnit.SECONDS.toNanos(3), TimeUnit.SECONDS.toNanos(1)).assertWillNotSpeculate();
                new TestScenario(12_000, 1_500, TimeUnit.SECONDS.toMicros(2), Config.CQLStartTime.QUEUE, TimeUnit.SECONDS.toNanos(3), TimeUnit.SECONDS.toNanos(1)).assertWillNotSpeculate();
                ...
                // tracked latency is 2 seconds. Speculation timeout is always computed against the start time, so we will not speculate
                // since there is only
                new TestScenario(12_000, 10_000, TimeUnit.SECONDS.toMicros(2), Config.CQLStartTime.REQUEST, TimeUnit.SECONDS.toNanos(11), TimeUnit.SECONDS.toNanos(1)).assertWillNotSpeculate();
                new TestScenario(12_000, 1_500, TimeUnit.SECONDS.toMicros(2), Config.CQLStartTime.QUEUE, TimeUnit.SECONDS.toNanos(11), TimeUnit.SECONDS.toNanos(1)).assertWillNotSpeculate();
                new TestScenario(12_000, 2000, TimeUnit.SECONDS.toMicros(3), Config.CQLStartTime.QUEUE, TimeUnit.SECONDS.toNanos(9), TimeUnit.SECONDS.toNanos(0)).assertWillNotSpeculate();
            });
            RestartFramework.at("after_speculation_tests").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    private static class TestScenario {
        ...
        private void assertWillSpeculate() {
            ...
            CoordinatorHelper.unsafeExecuteInternal("SELECT * FROM " + KEYSPACE + ".tbl WHERE pk = 1", ConsistencyLevel.QUORUM, ConsistencyLevel.QUORUM, new Dispatcher.RequestTime(before - enqueuedNsAgo, before - startedNsAgo));
