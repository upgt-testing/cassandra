...
public class PaxosUncommittedIndexTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void indexCqlIsExportableAndParsableTest() throws Throwable {
        ...
        try (Cluster dtestCluster = init(Cluster.build(1).withConfig(c -> c.with(Feature.NATIVE_PROTOCOL)).start())) {
            RestartFramework.at("after_cluster_start").on(dtestCluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            try (com.datastax.driver.core.Cluster clientCluster = com.datastax.driver.core.Cluster.builder().addContactPoint("127.0.0.1").build()) {
                Assert.assertTrue(clientCluster.getMetadata().exportSchemaAsString().contains(expectedCreateCustomIndex));
