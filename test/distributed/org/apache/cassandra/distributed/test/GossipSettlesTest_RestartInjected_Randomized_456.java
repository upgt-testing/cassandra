...
public class GossipSettlesTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void testGossipSettles() throws Throwable {
        ...
        try (Cluster cluster = builder().withNodes(3).withConfig(config -> config.with(GOSSIP).with(NETWORK)).withSubnet(1).start()) {
            ...
            RestartFramework.at("after_gossip_stop").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_verification").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    static String addStoragePortToInstanceName(String s) {
        ...
        }).collect(// to match final blank line
        Collectors.joining("\n", "", "\n"));
    }

    static <V> Map<String, V> addPortToKeys(Map<String, V> source) {
        return source.entrySet().stream().collect(Collectors.toMap(entry -> addStoragePortToIP(entry.getKey()), Map.Entry::getValue));
    }

    static <K> Map<K, String> addPortToValues(Map<K, String> source) {
        return source.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, entry -> addStoragePortToIP(entry.getValue())));
    }

    static List<String> addPortToList(List<String> list) {
        return list.stream().map(GossipSettlesTest_RestartInjected::addStoragePortToIP).collect(Collectors.toList());
    }

    static Map<String, List<String>> addPortToSchemaVersions(Map<String, List<String>> source) {
        return source.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, hostAndPortEntry -> addPortToList(hostAndPortEntry.getValue())));
    }
}
