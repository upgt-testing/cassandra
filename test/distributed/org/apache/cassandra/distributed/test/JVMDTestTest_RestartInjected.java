...
public class JVMDTestTest_RestartInjected extends TestBaseImpl
{
    @Test
    ...
    public void modifySchemaWithStoppedNode() throws Throwable
    {
        try (Cluster cluster = init(Cluster.build().withNodes(2).withConfig(c -> c.with(Feature.GOSSIP).with(Feature.NETWORK)).start()))
        {
            ...
            RestartFramework.at("after_first_schema_change")
                .on(cluster)
                .restart("node")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
            ...
            RestartFramework.at("after_second_schema_change")
                .on(cluster)
                .restart("node")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
            ...
            assertFalse(cluster.get(1).isShutdown());
            assertFalse(cluster.get(2).isShutdown());
            RestartFramework.at("after_both_nodes_restart")
                .on(cluster)
                .restart("node")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
            ...
            RestartFramework.at("after_third_schema_change")
                .on(cluster)
                .restart("node")
                .withIndex(1)
                .withMode(RestartMode.GRACEFUL)
