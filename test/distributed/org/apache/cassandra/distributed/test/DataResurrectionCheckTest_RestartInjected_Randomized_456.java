...
public class DataResurrectionCheckTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void testDataResurrectionCheck() throws Exception {
        ...
        try (WithProperties properties = new WithProperties().set(CHECK_DATA_RESURRECTION_HEARTBEAT_PERIOD, 3600000)) {
            ...
            try (Cluster cluster = build().withNodes(1).withDataDirCount(// we will expect heartbeat to be in the first data dir
            3).withConfig(config -> config.with(NATIVE_PROTOCOL).set("startup_checks", getStartupChecksConfig(ENABLED_PROPERTY, "true"))).start()) {
                IInvokableInstance instance = cluster.get(1);
                checkHeartbeat(instance);
                RestartFramework.at("after_heartbeat_check").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                ...
                RestartFramework.at("after_schema_creation").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                ...
                await().timeout(1, MINUTES).pollInterval(5, SECONDS).until(() -> {
                ...
                RestartFramework.at("after_violation_detection").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                ...
                assertNull(executeChecksOnInstance(instance, EXCLUDED_TABLES_CONFIG_PROPERTY, "ks1.tb1,ks2.tb1", EXCLUDED_KEYSPACES_CONFIG_PROPERTY, "ks3"));
                RestartFramework.at("after_exclusion_tests").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
        }
    }
    ...
    private Throwable executeChecksOnInstance(IInvokableInstance instance, final String... config) {
        ...
        return instance.callsOnInstance((IIsolatedExecutor.SerializableCallable<Throwable>) () -> {
            try {
                ...
                for (int i = 0; i < config.length - 1; i = i + 2) startupChecksOptions.set(check_data_resurrection, config[i], config[i + 1]);
    ...
    private Map<StartupCheckType, Map<String, Object>> getStartupChecksConfig(String... configs) {
        return new EnumMap<StartupCheckType, Map<String, Object>>(StartupCheckType.class) {
            ...
            {
                put(check_data_resurrection, new HashMap<String, Object>() {

                    {
                        for (int i = 0; i < configs.length - 1; i = i + 2) put(configs[i], configs[i + 1]);
                    }
                });
            }
        };
    ...
    private void checkHeartbeat(IInvokableInstance instance) throws Exception {
        File heartbeatFile = new File(((String[]) instance.config().get("data_file_directories"))[0], DEFAULT_HEARTBEAT_FILE);
