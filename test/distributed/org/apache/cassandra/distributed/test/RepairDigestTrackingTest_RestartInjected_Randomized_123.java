...
public class RepairDigestTrackingTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public void testInconsistenciesFound() throws Throwable {
        try (Cluster cluster = init(builder().withNodes(2).start())) {
            ...
            RestartFramework.at("after_enable_tracking").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            for (int i = 0; i < 10; i++) {
                cluster.coordinator(1).execute("INSERT INTO " + KS_TABLE + " (k, c, v) VALUES (?, ?, ?)", ConsistencyLevel.ALL, i, i, i);
            }
            ...
            RestartFramework.at("after_mark_repaired").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_create_inconsistency").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_range_read").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testPurgeableTombstonesAreIgnored() throws Throwable {
        try (Cluster cluster = init(builder().withNodes(2).start())) {
            ...
            RestartFramework.at("after_tombstone_flush").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            cluster.forEach(i -> i.flush(KEYSPACE));
            RestartFramework.at("after_data_flush").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_mark_all_repaired").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_overwrite").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            long ccBefore = getConfirmedInconsistencies(cluster.get(1));
    ...
    public void testSnapshottingOnInconsistency() throws Throwable {
        try (Cluster cluster = init(Cluster.create(2))) {
            ...
            cluster.get(1).runOnInstance(assertNotRepaired());
            RestartFramework.at("after_create_mismatch").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_partition_read").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            long ccAfter = getConfirmedInconsistencies(cluster.get(1));
            ...
            RestartFramework.at("after_enable_snapshot").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.coordinator(1).execute("SELECT * FROM " + KS_TABLE + " WHERE k=0", ConsistencyLevel.ALL);
    ...
    public void testRepairedReadCountNormalizationWithInitialUnderread() throws Throwable {
        ...
        try (Cluster cluster = init(Cluster.create(2))) {
            ...
            cluster.forEach(c -> c.flush(KEYSPACE));
            RestartFramework.at("after_initial_flush").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // nothing is repaired yet
            cluster.forEach(i -> i.runOnInstance(assertNotRepaired()));
            // mark everything repaired
            cluster.forEach(i -> i.runOnInstance(markAllRepaired()));
            cluster.forEach(i -> i.runOnInstance(assertRepaired()));
            RestartFramework.at("after_mark_repaired").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Add some unrepaired data to both nodes
            for (int i = 20; i < 30; i++) {
                cluster.coordinator(1).execute("INSERT INTO " + KS_TABLE + " (k, c, v1) VALUES (1, ?, ?) USING TIMESTAMP 1", ConsistencyLevel.ALL, i, i);
            }
            RestartFramework.at("after_unrepaired_inserts").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_recreate_mismatch").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRepairedReadCountNormalizationWithInitialOverread() throws Throwable {
        ...
        try (Cluster cluster = init(Cluster.create(2))) {
            ...
            // Add some unrepaired data to both nodes
            for (int i = 10; i < 13; i++) {
                cluster.coordinator(1).execute("INSERT INTO " + KS_TABLE + " (k, c, v1) VALUES (0, ?, ?) USING TIMESTAMP 1", ConsistencyLevel.ALL, i, i);
                cluster.coordinator(1).execute("INSERT INTO " + KS_TABLE + " (k, c, v1) VALUES (1, ?, ?) USING TIMESTAMP 1", ConsistencyLevel.ALL, i, i);
            }
            cluster.forEach(c -> c.flush(KEYSPACE));
            RestartFramework.at("after_unrepaired_flush").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_deletions").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            // Verify single partition read
            long ccBefore = getConfirmedInconsistencies(cluster.get(1));
            assertRows(cluster.coordinator(1).execute("SELECT * FROM " + KS_TABLE + " WHERE k=0 LIMIT 5", ConsistencyLevel.ALL), rows(rows(0, 12, 10), rows(0, 6, 5)));
            RestartFramework.at("after_partition_read").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            long ccAfterPartitionRead = getConfirmedInconsistencies(cluster.get(1));
            // Verify partition range read
            assertRows(cluster.coordinator(1).execute("SELECT * FROM " + KS_TABLE + " LIMIT 11", ConsistencyLevel.ALL), rows(rows(1, 12, 10), rows(1, 6, 0), rows(0, 12, 12)));
            RestartFramework.at("after_range_read").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            long ccAfterRangeRead = getConfirmedInconsistencies(cluster.get(1));
            if (ccAfterPartitionRead != ccAfterRangeRead)
                if (ccAfterPartitionRead != ccBefore)
    ...
    public void testLocalDataAndRemoteRequestConcurrency() throws Exception {
        try (Cluster cluster = init(Cluster.build(3).withInstanceInitializer(BBHelper::install).withConfig(config -> config.set("repaired_data_tracking_for_partition_reads_enabled", true).with(GOSSIP).with(NETWORK)).start())) {
            ...
            RestartFramework.at("after_schema_setup").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_select").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public static class BBHelper {
        ...
        public static void install(ClassLoader classLoader, Integer num) {
            ...
            if (num == 1) {
                new ByteBuddy().rebase(SEPExecutor.class).method(named("maybeExecuteImmediately")).intercept(MethodDelegation.to(BBHelper.class)).make().load(classLoader, ClassLoadingStrategy.Default.INJECTION);
                new ByteBuddy().rebase(SinglePartitionReadCommand.class).method(named("executeLocally")).intercept(MethodDelegation.to(BBHelper.class)).make().load(classLoader, ClassLoadingStrategy.Default.INJECTION);
                new ByteBuddy().rebase(ReplicaLayout.class).method(named("forTokenReadLiveSorted").and(takesArguments(AbstractReplicationStrategy.class, Token.class))).intercept(MethodDelegation.to(BBHelper.class)).make().load(classLoader, ClassLoadingStrategy.Default.INJECTION);
            }
        }
        ...
        @SuppressWarnings({ "unused" })
        public static UnfilteredPartitionIterator executeLocally(ReadExecutionController executionController, @SuperCall Callable<UnfilteredPartitionIterator> zuperCall) {
            try {
    ...
    private Object[][] rows(Object[][] head, Object[][]... tail) {
        return Stream.concat(Stream.of(head), Stream.of(tail).flatMap(Stream::of)).toArray(Object[][]::new);
    }

    private Object[][] rows(int partitionKey, int start, int end) {
        if (start == end)
            return new Object[][] { new Object[] { partitionKey, start, end } };
        IntStream clusterings = start > end ? IntStream.range(end - 1, start).map(i -> start - i + end - 1) : IntStream.range(start, end);
    ...
    private IIsolatedExecutor.SerializableRunnable assertNotRepaired() {
        return () -> {
            try {
                Iterator<SSTableReader> sstables = Keyspace.open(KEYSPACE).getColumnFamilyStore(TABLE).getLiveSSTables().iterator();
                while (sstables.hasNext()) {
                    ...
                    Assert.assertEquals("repaired at is set for sstable: " + descriptor, stats.repairedAt, ActiveRepairService.UNREPAIRED_SSTABLE);
                }
            } catch (IOException e) {
        ...
        return () -> {
            try {
                ...
                while (sstables.hasNext()) {
                    SSTableReader sstable = sstables.next();
                    Descriptor descriptor = sstable.descriptor;
                    descriptor.getMetadataSerializer().mutateRepairMetadata(descriptor, currentTimeMillis(), null, false);
    ...
    private long getConfirmedInconsistencies(IInvokableInstance instance) {
        return instance.callOnInstance(() -> Keyspace.open(KEYSPACE).getColumnFamilyStore(TABLE).metric.confirmedRepairedInconsistencies.table.getCount());
    }
    ...
    private void setupSchema(Cluster cluster, String cql) {
        ...
        cluster.forEach(i -> i.runOnInstance(() -> Keyspace.open(KEYSPACE).getColumnFamilyStore(TABLE).disableAutoCompaction()));
    }
}
