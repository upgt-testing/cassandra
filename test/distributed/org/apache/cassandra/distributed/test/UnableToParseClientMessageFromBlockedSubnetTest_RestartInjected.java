...
public class UnableToParseClientMessageFromBlockedSubnetTest_RestartInjected extends TestBaseImpl
{
    ...
    public void badMessageCausesProtocolExceptionFromExcludeList() throws IOException, TimeoutException
    {
        ...
        try (SimpleClient client = SimpleClient.builder("127.0.0.1", 9042).protocolVersion(version).useBeta().build())
        {
            ...
            RestartFramework.at("after_bad_message")
                .on(cluster)
                .restart("node")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
            ...
            RestartFramework.at("after_log_verification")
                .on(cluster)
                .restart("node")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
