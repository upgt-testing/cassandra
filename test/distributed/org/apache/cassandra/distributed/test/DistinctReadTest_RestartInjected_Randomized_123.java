...
public class DistinctReadTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public void test() throws IOException {
        try (Cluster cluster = init(Cluster.build().withNodes(1).start())) {
            ...
            RestartFramework.at("after_distinct_queries").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
}
