...
public class IPMembershipTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public void sameIPFailWithoutReplace() throws IOException {
        try (Cluster cluster = Cluster.build(3).withConfig(c -> c.with(Feature.GOSSIP, Feature.NATIVE_PROTOCOL).set(Constants.KEY_DTEST_API_STARTUP_FAILURE_AS_SHUTDOWN, false)).start()) {
            ...
            RestartFramework.at("after_stress_write").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            for (boolean auto_bootstrap : Arrays.asList(true, false)) {
                ...
                Assertions.assertThatThrownBy(() -> nodeToReplace.startup()).hasMessage("A node with address /127.0.0.3:7012 already exists, cancelling join. Use " + REPLACE_ADDRESS.getKey() + " if you want to replace this node.");
            }
        }
    }
    ...
    public void startupNewIP() throws IOException, InterruptedException {
        try (Cluster cluster = Cluster.build(3).withConfig(c -> c.with(Feature.GOSSIP, Feature.NATIVE_PROTOCOL).// disable DistributedTestSnitch as it tries to query before we setup
        set("endpoint_snitch", "org.apache.cassandra.locator.SimpleSnitch")).start()) {
            RestartFramework.at("after_cluster_start").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            IInvokableInstance nodeToReplace = cluster.get(3);
            ToolRunner.invokeCassandraStress("write", "n=10000", "-schema", "replication(factor=3)", "-port", "native=9042").assertOnExitCode();
            RestartFramework.at("after_stress_write").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_ip_update").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nodeToReplace.startup();
            RestartFramework.at("after_node_startup").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
