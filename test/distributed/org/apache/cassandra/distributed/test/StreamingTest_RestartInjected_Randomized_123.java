...
public class StreamingTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    private void testStreaming(int nodes, int replicationFactor, int rowCount, String compactionStrategy) throws Throwable {
        ...
        1).withConfig(config -> config.with(NETWORK)).start()) {
            ...
            RestartFramework.at("after_truncate_ranges").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            {
            ...
            RestartFramework.at("before_rebuild").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
