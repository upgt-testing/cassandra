...
public class TombstoneCountWarningTest_RestartInjected_Randomized_123 extends TestBaseImpl {

    private static final Logger logger = LoggerFactory.getLogger(TombstoneCountWarningTest_RestartInjected.class);
    ...
    public static void setupClass() throws IOException {
        ...
        builder.withConfig(c -> c.set("tombstone_warn_threshold", TOMBSTONE_WARN).set("tombstone_failure_threshold", TOMBSTONE_FAIL).with(Feature.NATIVE_PROTOCOL, Feature.GOSSIP));
    ...
    private void failThreshold(String cql, boolean isScan) throws UnknownHostException {
        for (int i = 0; i < TOMBSTONE_FAIL + 1; i++) CLUSTER.coordinator(1).execute("INSERT INTO " + KEYSPACE + ".tbl (pk, ck, v) VALUES (1, ?, null)", ConsistencyLevel.ALL, i);
        enable(true);
        ...
        Assertions.assertThat(Iterables.getOnlyElement(warnings)).contains("nodes scanned over " + (TOMBSTONE_FAIL + 1) + " tombstones and aborted the query " + cql);
        ...
        try {
            ...
            // well in this case
            // can't include ', 3 failed)' as some times its 2
            Assertions.assertThat(e.getMessage()).contains("responses were required but only 0 replica responded");
            ...
            assertThat(e.getFailuresMap()).hasSizeBetween(1, 3).// coordinator changes from run to run, so can't assert map as the key is dynamic... so assert the domain of keys and the single value expect
            containsValue(RequestFailureReason.READ_TOO_MANY_TOMBSTONES.code).hasKeySatisfying(new Condition<InetAddress>() {
        ...
        warnings = CLUSTER.get(1).callsOnInstance(() -> {
            ...
            try {
                ...
                Assertions.assertThat(e.failureReasonByEndpoint.values()).as("Non READ_TOO_MANY_TOMBSTONES exists").allMatch(RequestFailureReason.READ_TOO_MANY_TOMBSTONES::equals);
            }
        ...
        if (isScan) {
            ...
            Assertions.assertThat(Iterables.getOnlyElement(warnings)).startsWith("Read " + TOMBSTONE_FAIL + " live rows and " + (TOMBSTONE_FAIL + 1) + " tombstone cells for query " + cql);
        }
        ...
        try {
            ...
            Assertions.assertThat(e.getFailuresMap()).isNotEmpty();
            Assertions.assertThat(e.getFailuresMap().values()).as("Non READ_TOO_MANY_TOMBSTONES exists").allMatch(i -> i.equals(RequestFailureReason.READ_TOO_MANY_TOMBSTONES.code));
        }
        assertWarnAborts(0, 2, 0);
    }
    ...
    private static long totalReadAborts() {
        return CLUSTER.stream().mapToLong(i -> i.metrics().getCounter("org.apache.cassandra.metrics.ClientRequest.Aborts.Read-ALL") + i.metrics().getCounter("org.apache.cassandra.metrics.ClientRequest.Aborts.RangeSlice")).sum();
    }
    ...
    public static class BB {
        ...
        private static void install(ClassLoader cl, int instanceId) {
            if (instanceId != 1)
                return;
            new ByteBuddy().rebase(ReadCallback.class).method(named("awaitResults")).intercept(MethodDelegation.to(BB.class)).method(named("onFailure")).intercept(MethodDelegation.to(BB.class)).make().load(cl, ClassLoadingStrategy.Default.INJECTION);
            new ByteBuddy().rebase(SEPExecutor.class).method(named("maybeExecuteImmediately")).intercept(MethodDelegation.to(BB.class)).make().load(cl, ClassLoadingStrategy.Default.INJECTION);
        }
