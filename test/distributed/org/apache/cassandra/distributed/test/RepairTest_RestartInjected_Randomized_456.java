...
public class RepairTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    private static void insert(ICluster<IInvokableInstance> cluster, String keyspace, int start, int end, int... nodes) {
        ...
        for (int i = start; i < end; ++i) for (int node : nodes) cluster.get(node).executeInternal(insert, Integer.toString(i));
    }
    ...
    private static void flush(ICluster<IInvokableInstance> cluster, String keyspace, int... nodes) {
        for (int node : nodes) cluster.get(node).runOnInstance(rethrow(() -> StorageService.instance.forceKeyspaceFlush(keyspace, ColumnFamilyStore.FlushReason.UNIT_TESTS)));
    }

    private static ICluster create(Consumer<IInstanceConfig> configModifier) throws IOException {
        configModifier = configModifier.andThen(config -> config.set("hinted_handoff_enabled", false).with(NETWORK).with(GOSSIP));
    ...
    void repair(ICluster<IInvokableInstance> cluster, boolean sequential, String compression) throws Exception {
        ...
        RestartFramework.at("after_populate").on((Cluster) cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        repair(cluster, KEYSPACE, ImmutableMap.of("parallelism", sequential ? "sequential" : "parallel"));
        RestartFramework.at("after_repair").on((Cluster) cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        verify(cluster, KEYSPACE, 0, 2001, 1, 2, 3);
        RestartFramework.at("after_verify").on((Cluster) cluster).restart("node").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
    }
    ...
    void shutDownNodesAndForceRepair(ICluster<IInvokableInstance> cluster, String keyspace, int downNode) throws Exception {
        ...
        RestartFramework.at("after_forced_repair").on((Cluster) cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ClusterUtils.stopUnchecked(cluster.get(downNode));
        RestartFramework.at("after_node_shutdown").on((Cluster) cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_populate_forced_repair").on((Cluster) cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testForcedNormalRepairWithOneNodeDown() throws Exception {
        ...
        RestartFramework.at("after_keyspace_create").on((Cluster) cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try {
            shutDownNodesAndForceRepair(cluster, forceRepairKeyspace, 3);
            RestartFramework.at("after_shutdown_and_repair").on((Cluster) cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            DistributedRepairUtils.assertParentRepairSuccess(cluster, 1, forceRepairKeyspace, "test", row -> {
                ...
                Assert.assertTrue("Given clusterSize = 3, RF = 2 and 1 node down in the replica set, it should yield only " + tokenCount + " successful repaired range.", successfulRanges.size() == tokenCount && !successfulRanges.contains(""));
            });
            RestartFramework.at("after_assertion").on((Cluster) cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } finally {
            if (cluster.get(3).isShutdown())
                cluster.get(3).startup(cluster);
            RestartFramework.at("after_node_startup").on((Cluster) cluster).restart("node").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        }
    }
}
