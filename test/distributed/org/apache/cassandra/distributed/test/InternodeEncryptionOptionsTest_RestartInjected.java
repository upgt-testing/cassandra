...
public class InternodeEncryptionOptionsTest_RestartInjected extends AbstractEncryptionOptionsImpl
{
    @Test
    ...
    public void optionalTlsConnectionDisabledWithoutKeystoreTest() throws Throwable
    {
        try (Cluster cluster = builder().withNodes(1).withConfig(c -> c.with(Feature.NETWORK)).createWithoutStarting())
        {
            ...
            RestartFramework.at("after_startup")
                .on(cluster)
                .restart("node")
                .withIndex(0)
            ...
            RestartFramework.at("after_connection_test")
                .on(cluster)
                .restart("node")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
