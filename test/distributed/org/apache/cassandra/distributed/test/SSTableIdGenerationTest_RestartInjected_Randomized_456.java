...
public class SSTableIdGenerationTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void testRestartWithUUIDEnabled() throws IOException {
        try (Cluster cluster = init(Cluster.build(1).withDataDirCount(1).withConfig(config -> config.set(ENABLE_UUID_FIELD_NAME, false)).start())) {
            ...
            RestartFramework.at("after_seq_sstables_create").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertSSTablesCount(cluster.get(1), 2, 0, KEYSPACE, "tbl");
            verfiySSTableActivity(cluster, true);
            RestartFramework.at("before_uuid_restart").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            restartNode(cluster, 1, true);
            RestartFramework.at("after_uuid_restart").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            createSSTables(cluster.get(1), KEYSPACE, "tbl", 3, 4);
            RestartFramework.at("after_uuid_sstables_create").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRestartWithUUIDDisabled() throws IOException {
        try (Cluster cluster = init(Cluster.build(1).withDataDirCount(1).withConfig(config -> config.set(ENABLE_UUID_FIELD_NAME, true)).start())) {
            ...
            verfiySSTableActivity(cluster, false);
            RestartFramework.at("before_fail_restart").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Assertions.assertThatExceptionOfType(RuntimeException.class).isThrownBy(() -> restartNode(cluster, 1, false)).withCauseInstanceOf(SystemExitException.class);
        }
    }

    @Test
    public final void testCompactionStrategiesWithMixedSSTables() throws Exception {
        testCompactionStrategiesWithMixedSSTables(SizeTieredCompactionStrategy.class, TimeWindowCompactionStrategy.class, LeveledCompactionStrategy.class);
    }
    ...
    private void testStreaming(boolean uuidEnabledOnTargetNode) throws Exception {
        ...
        try (Cluster cluster = init(Cluster.build(2).withDataDirCount(1).withConfig(config -> config.set(ENABLE_UUID_FIELD_NAME, false).with(Feature.NETWORK)).start())) {
    ...
    public void testSnapshot() throws Exception {
        ...
        try (Cluster cluster = init(Cluster.build(1).withDataDirCount(1).withConfig(config -> config.with(Feature.NETWORK).set("incremental_backups", true).set("snapshot_before_compaction", false).set("auto_snapshot", false).set(ENABLE_UUID_FIELD_NAME, false)).start())) {
            // create the tables
        ...
        try (Cluster cluster = init(Cluster.build(1).withDataDirCount(1).withConfig(config -> config.with(Feature.NETWORK, Feature.NATIVE_PROTOCOL).set("incremental_backups", true).set("snapshot_before_compaction", false).set("auto_snapshot", false).set(ENABLE_UUID_FIELD_NAME, false)).start())) {
            cluster.schemaChange(createTableStmt(KEYSPACE, "tbl_seq_only", null));
            cluster.schemaChange(createTableStmt(KEYSPACE, "tbl_seq_and_uuid", null));
            cluster.schemaChange(createTableStmt(KEYSPACE, "tbl_uuid_only", null));
    ...
    private static void restore(IInvokableInstance instance, Set<String> dirs, String targetTableName, int expectedRowsNum) {
        List<String> failedImports = instance.callOnInstance(() -> ColumnFamilyStore.getIfExists(KEYSPACE, targetTableName).importNewSSTables(dirs, false, false, true, true, true, true, true));
    ...
    private static Set<String> snapshot(IInvokableInstance instance, String ks, String tableName) {
        Set<String> snapshotDirs = instance.callOnInstance(() -> ColumnFamilyStore.getIfExists(ks, tableName).snapshot(SNAPSHOT_TAG).getDirectories().stream().map(File::toString).collect(Collectors.toSet()));
    ...
    private static String createTableStmt(String ks, String name, Class<? extends AbstractCompactionStrategy> compactionStrategy) {
        ...
        return format("CREATE TABLE %s.%s (pk int, ck int, v int, PRIMARY KEY (pk, ck)) " + "WITH compaction = {'class':'%s', 'enabled':'false'}", ks, name, compactionStrategy.getCanonicalName());
    }
    ...
    private static void assertSSTablesCount(Set<Descriptor> descs, String tableName, int expectedSeqGenIds, int expectedUUIDGenIds) {
        List<String> seqSSTables = descs.stream().filter(desc -> desc.id instanceof SequenceBasedSSTableId).map(descriptor -> descriptor.baseFile().toString()).sorted().collect(Collectors.toList());
        List<String> uuidSSTables = descs.stream().filter(desc -> desc.id instanceof UUIDBasedSSTableId).map(descriptor -> descriptor.baseFile().toString()).sorted().collect(Collectors.toList());
    ...
    private static Set<String> getBackupDirs(IInvokableInstance instance, String ks, String tableName) {
        return instance.callOnInstance(() -> getBackups(ks, tableName).stream().map(d -> d.directory).map(File::toString).collect(Collectors.toSet()));
    }
    ...
    private static void verfiySSTableActivity(Cluster cluster, boolean expectLegacyTableIsPopulated) {
        cluster.get(1).runOnInstance(() -> {
            ...
            assertThat(SystemKeyspace.getSSTableReadMeter("ks", "tab", seqGenId)).matches(m -> m.fifteenMinuteRate() == meter.fifteenMinuteRate() && m.twoHourRate() == meter.twoHourRate());
            ...
            assertThat(SystemKeyspace.getSSTableReadMeter("ks", "tab", uuidGenId)).matches(m -> m.fifteenMinuteRate() == meter.fifteenMinuteRate() && m.twoHourRate() == meter.twoHourRate());
    ...
    private static void checkRowsNumber(IInstance instance, String ks, String tableName, int expectedNumber) {
        ...
        assertThat(rows).withFailMessage("Invalid results for %s.%s - should have %d rows but has %d: \n%s", ks, tableName, expectedNumber, rows.length, result.toString()).hasNumberOfRows(expectedNumber);
    }
}
