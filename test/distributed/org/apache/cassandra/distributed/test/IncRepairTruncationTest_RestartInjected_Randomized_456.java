...
public class IncRepairTruncationTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void testTruncateDuringIncRepair() throws IOException, InterruptedException, ExecutionException {
        ...
        try (Cluster cluster = init(Cluster.build(2).withConfig(config -> config.with(GOSSIP).with(NETWORK)).start())) {
            ...
            RestartFramework.at("after_initial_repair").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            /*
            ...
            RestartFramework.at("after_final_repair_preview").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } finally {
            es.shutdown();
        }
    }
