...
public class InternodeErrorExclusionTest_RestartInjected extends TestBaseImpl
{
    @BeforeClass
    ...
    public void testNoSpammingInvalidLegacyProtocolMagicException() throws Throwable
    {
        try (Cluster cluster = Cluster.build(1)
                                      ...
                                      .start())
        {
            RestartFramework.at("after_cluster_start_nospam")
                .on(cluster)
                .restart("node")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
                .execute();
            causeException();
            RestartFramework.at("after_first_exception")
                .on(cluster)
                .restart("node")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
                .execute();
            causeException();
            RestartFramework.at("after_second_exception")
                .on(cluster)
                .restart("node")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
