...
public class CountersTest_RestartInjected extends TestBaseImpl
{
    @Test
    ...
    private static void testUpdateCounter(boolean droppedCompactStorage) throws Throwable
    {
        try (Cluster cluster = Cluster.build(2).withConfig(c -> c.with(GOSSIP, NATIVE_PROTOCOL).set("drop_compact_storage_enabled", true)).start())
        {
            ...
            if (droppedCompactStorage)
            {
                ...
                RestartFramework.at("after_drop_compact_storage")
                    .on(cluster)
                    .restart("node")
                    .withIndex(0)
                    .withMode(RestartMode.GRACEFUL)
            ...
            for (int i = 1; i <= cluster.size(); i++)
            {
                ...
                RestartFramework.at("after_counter_increment")
                    .on(cluster)
                    .restart("node")
                    .withIndex(0)
                    .withMode(RestartMode.GRACEFUL)
                ...
                RestartFramework.at("after_counter_decrement")
                    .on(cluster)
                    .restart("node")
                    .withIndex(0)
                    .withMode(RestartMode.GRACEFUL)
