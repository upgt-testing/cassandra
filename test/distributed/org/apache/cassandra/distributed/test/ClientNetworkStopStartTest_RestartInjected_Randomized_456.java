...
public class ClientNetworkStopStartTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void stopStartNative() throws IOException {
        ...
        try (Cluster cluster = init(Cluster.build(1).withConfig(c -> c.with(Feature.GOSSIP, Feature.NATIVE_PROTOCOL)).start())) {
            ...
            RestartFramework.at("after_native_insert").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
