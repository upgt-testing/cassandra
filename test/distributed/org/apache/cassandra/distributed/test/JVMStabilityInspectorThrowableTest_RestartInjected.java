...
public class JVMStabilityInspectorThrowableTest_RestartInjected extends TestBaseImpl
{
    ...
    public JVMStabilityInspectorThrowableTest_RestartInjected(DiskFailurePolicy policy, boolean testCorrupted,
    ...
    private static void test(DiskFailurePolicy policy, boolean shouldTestCorrupted, boolean expectNativeTransportRunning, boolean expectGossiperEnabled) throws Exception
    {
        ...
        try (final Cluster cluster = init(getCluster(policy).start()))
        {
            ...
            RestartFramework.at("after_daemon_setup")
                .on(cluster)
                .restart("node")
                .withIndex(0)
            ...
            RestartFramework.at("after_corruption")
                .on(cluster)
                .restart("node")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
            ...
            RestartFramework.at("after_failed_query")
                .on(cluster)
                .restart("node")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
