...
public class RepairBoundaryTest_RestartInjected_Randomized_123 extends TestBaseImpl {

    private static Cluster cluster;

    private static final String INSERT = withKeyspace("INSERT INTO %s.test (k, c1, c2) VALUES" + "(?, 'C1', ?);");
    ...
    void populate() {
        try {
            ...
            cluster.schemaChange(withKeyspace("CREATE TABLE %s.test (k blob, c1 text, c2 text," + "PRIMARY KEY (k))"));
    ...
    public void primaryRangeRepair() {
        populate();
        verify();
        RestartFramework.at("pr_after_deletes").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("pr_after_repair").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        delete(cluster.get(1), 999, 1000, 3001);
        delete(cluster.get(2), 1999, 2000, 1001);
        delete(cluster.get(3), 2999, 3000, 2001);
        RestartFramework.at("pr_after_populate").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster.forEach(i -> {
    ...
    public void singleTokenRangeRepair() {
        populate();
        verify();
        RestartFramework.at("str_after_deletes").on(cluster).restart("node").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        delete(cluster.get(1), 999, 1000);
        RestartFramework.at("str_after_populate").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("str_after_repair").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        assertRows(c2Row(999, 1001, 1999, 2000, 3001), cluster.get(2).executeInternal(ALL));
    }

    @BeforeClass
    public static void init() throws IOException {
        cluster = Cluster.build(3).withConfig(config -> config.set("hinted_handoff_enabled", false).set("num_tokens", 1).set("initial_token", Long.toString(config.num() * 1000)).with(NETWORK).with(GOSSIP)).start();
