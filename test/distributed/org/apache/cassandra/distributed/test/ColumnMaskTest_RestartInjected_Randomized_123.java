 ...
 */
public class ColumnMaskTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public void testMaskPropagation() throws Throwable {
        try (Cluster cluster = createClusterWithAuhentication(3)) {
            ...
            RestartFramework.at("after_insert_data").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertRowsInAllCoordinators(cluster, row(1, "****"), row(2, "****"), row(3, "****"));
            RestartFramework.at("after_verify_default_mask").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            cluster.schemaChange(withKeyspace("ALTER TABLE %s.t ALTER v DROP MASKED"));
            RestartFramework.at("after_drop_mask").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertRowsInAllCoordinators(cluster, row(1, "secret1"), row(2, "secret2"), row(3, "secret3"));
            RestartFramework.at("after_verify_unmasked").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            cluster.schemaChange(withKeyspace("ALTER TABLE %s.t ALTER v MASKED WITH mask_inner(null, 1)"));
            RestartFramework.at("after_apply_mask_inner_1").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertRowsInAllCoordinators(cluster, row(1, "******1"), row(2, "******2"), row(3, "******3"));
            RestartFramework.at("after_verify_mask_inner_1").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            cluster.schemaChange(withKeyspace("ALTER TABLE %s.t ALTER v MASKED WITH mask_inner(3, null)"));
            RestartFramework.at("after_apply_mask_inner_2").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertRowsInAllCoordinators(cluster, row(1, "sec****"), row(2, "sec****"), row(3, "sec****"));
            RestartFramework.at("after_verify_mask_inner_2").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public void testMaskLoading() throws Throwable {
        try (Cluster cluster = createClusterWithAuhentication(1)) {
            ...
            cluster.schemaChange(withKeyspace("CREATE FUNCTION %s.custom_mask(column text, replacement text) " + "RETURNS NULL ON NULL INPUT " + "RETURNS text " + "LANGUAGE java " + "AS 'return replacement;'"));
            RestartFramework.at("after_function_create").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.schemaChange(withKeyspace("CREATE TABLE %s.t (" + "a text MASKED WITH DEFAULT, " + "b text MASKED WITH mask_replace('redacted'), " + "c text MASKED WITH mask_inner(null, 1), " + "d text MASKED WITH mask_inner(3, null), " + "e text MASKED WITH %<s.custom_mask('obscured'), " + "PRIMARY KEY (a, b))"));
            RestartFramework.at("after_table_create").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            assertRowsWithRestart(node, row("****", "redacted", "******1", "sec****", "obscured"), row("****", "redacted", "******2", "sec****", "obscured"));
            RestartFramework.at("after_verify_initial_masks").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_alter_masks").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertRowsWithRestart(node, row("secret1", null, "#######", "sec###1", "censored"), row("secret2", null, "#######", "sec###2", "censored"));
            RestartFramework.at("after_verify_altered_masks").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }

    private static Cluster createClusterWithAuhentication(int nodeCount) throws IOException {
        Cluster cluster = init(Cluster.build().withNodes(nodeCount).withConfig(conf -> conf.with(GOSSIP, NATIVE_PROTOCOL).set("dynamic_data_masking_enabled", "true").set("user_defined_functions_enabled", "true").set("authenticator", "PasswordAuthenticator").set("authorizer", "CassandraAuthorizer")).start());
    ...
    private static void withAuthenticatedSession(IInvokableInstance instance, String username, String password, Consumer<Session> consumer) {
        ...
        Builder builder = com.datastax.driver.core.Cluster.builder().addContactPoints(address).withLoadBalancingPolicy(lbc).withCredentials(username, password);
