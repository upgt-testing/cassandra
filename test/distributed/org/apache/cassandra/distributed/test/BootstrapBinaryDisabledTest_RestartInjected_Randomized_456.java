 ...
 */
public class BootstrapBinaryDisabledTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void test() throws IOException, TimeoutException {
        ...
        try (Cluster cluster = init(Cluster.build(originalNodeCount).withTokenSupplier(TokenSupplier.evenlyDistributedTokens(expandedNodeCount)).withNodeIdTopology(NetworkTopology.singleDcNetworkTopology(expandedNodeCount, "dc0", "rack0")).withConfig(c -> {
            ...
            RestartFramework.at("after_first_bootstrap").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Test write survey behaviour
            bootstrap(cluster, config, true);
            RestartFramework.at("after_second_bootstrap").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    private void populate(IInvokableInstance inst) {
        for (int i = 0; i < 10; i++) inst.executeInternal("INSERT INTO " + KEYSPACE + ".tbl (pk) VALUES (?)", Integer.toString(i));
    }
