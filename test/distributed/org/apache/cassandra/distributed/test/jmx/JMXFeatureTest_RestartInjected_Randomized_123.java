...
public class JMXFeatureTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    private void testJmxFeatures(INodeProvisionStrategy.Strategy provisionStrategy) throws Exception {
        ...
        // Make sure the JMX infrastructure all cleans up properly by running this multiple times.
        int iterations = 2;
        for (int i = 0; i < iterations; i++) {
            try (Cluster cluster = Cluster.build(2).withDynamicPortAllocation(true).withNodeProvisionStrategy(provisionStrategy).withConfig(c -> c.with(Feature.values())).start()) {
                ...
                RestartFramework.at("after_instance_tests").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
                ...
                RestartFramework.at("after_all_getters_test").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
        }
    ...
    public void testShutDownAndRestartInstances() throws Exception {
        ...
        try (Cluster cluster = Cluster.build(2).withConfig(c -> c.with(Feature.values())).start()) {
            ...
            RestartFramework.at("after_first_instance_test").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ClusterUtils.stopUnchecked(instanceToStop);
            RestartFramework.at("after_instance_stop").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_ring_status_down").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_nodetool_status_down").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            testInstance(instances, cluster.get(2));
            ClusterUtils.start(instanceToStop, props -> {
            });
            RestartFramework.at("after_instance_restart").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_ring_status_up").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ...
            Assert.assertThat(statusResult.getStderr(), is(blankOrNullString()));
            Assert.assertThat(statusResult.getStdout(), containsString("UN  127.0.0.1"));
            RestartFramework.at("after_nodetool_status_up").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            testInstance(instances, cluster.get(1));
            testAllValidGetters(cluster);
            RestartFramework.at("after_all_getters_test").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        }
    }
