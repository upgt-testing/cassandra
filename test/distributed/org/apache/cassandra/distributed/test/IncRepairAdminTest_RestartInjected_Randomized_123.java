...
public class IncRepairAdminTest_RestartInjected_Randomized_123 extends TestBaseImpl {

    @Test
    public void testRepairAdminSummarizePending() throws IOException {
        try (Cluster cluster = init(Cluster.build(1).withConfig(config -> config.with(GOSSIP).with(NETWORK)).start())) {
            ...
            RestartFramework.at("after_repair_admin_summarize").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // then the table info should be present in the output
            res.asserts().success();
            String outputLine = stream(res.getStdout().split("\n")).filter(l -> l.contains(KEYSPACE) && l.contains("tbl")).findFirst().orElseThrow(() -> new AssertionError("should find tbl table in output of repair_admin summarize-pending"));
    ...
    private void repairAdminCancelHelper(boolean coordinator, boolean force) throws IOException {
        try (Cluster cluster = init(Cluster.build(3).withConfig(config -> config.with(GOSSIP).with(NETWORK)).start())) {
            ...
            RestartFramework.at("after_check_no_sessions").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            TimeUUID uuid = makeFakeSession(cluster);
            RestartFramework.at("after_make_fake_session").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            awaitNodetoolRepairAdminContains(cluster, uuid, "REPAIRING", false);
            RestartFramework.at("after_await_repairing_state").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_cancel_command").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            if (shouldFail) {
                res.asserts().failure();
                ...
                RestartFramework.at("after_verify_still_repairing").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            } else {
                res.asserts().success();
                awaitNodetoolRepairAdminContains(cluster, uuid, "FAILED", true);
                RestartFramework.at("after_verify_failed_state").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
        }
    }
    ...
    private static TimeUUID makeFakeSession(Cluster cluster) {
        ...
        Set<InetSocketAddress> participants = cluster.stream().map(i -> i.config().broadcastAddress()).collect(Collectors.toSet());
        cluster.forEach(i -> {
            i.runOnInstance(() -> {
                ColumnFamilyStore cfs = Keyspace.open(KEYSPACE).getColumnFamilyStore("tbl");
                Range<Token> range = new Range<>(cfs.metadata().partitioner.getMinimumToken(), cfs.metadata().partitioner.getRandomToken());
                ActiveRepairService.instance().registerParentRepairSession(sessionId, InetAddressAndPort.getByAddress(coordinator.getAddress()), Lists.newArrayList(cfs), Sets.newHashSet(range), true, currentTimeMillis(), true, PreviewKind.NONE);
                LocalSessionAccessor.prepareUnsafe(sessionId, InetAddressAndPort.getByAddress(coordinator.getAddress()), participants.stream().map(participant -> InetAddressAndPort.getByAddress(participant.getAddress())).collect(Collectors.toSet()));
