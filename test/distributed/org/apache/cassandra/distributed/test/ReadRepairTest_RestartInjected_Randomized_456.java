...
public class ReadRepairTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    private void testReadRepair(ReadRepairStrategy strategy) throws Throwable {
        try (Cluster cluster = init(Cluster.create(3))) {
            cluster.schemaChange(withKeyspace("CREATE TABLE %s.t (k int, c int, v int, PRIMARY KEY (k, c)) " + String.format("WITH read_repair='%s'", strategy)));
            ...
            RestartFramework.at("after_read_repair").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void readRepairTimeoutTest() throws Throwable {
        ...
        try (Cluster cluster = init(builder().withNodes(3).start())) {
            ...
            RestartFramework.at("timeout_after_table_create").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.get(1).executeInternal("INSERT INTO " + KEYSPACE + ".tbl (pk, ck, v) VALUES (1, 1, 1)");
            cluster.get(2).executeInternal("INSERT INTO " + KEYSPACE + ".tbl (pk, ck, v) VALUES (1, 1, 1)");
            RestartFramework.at("timeout_after_inserts").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertRows(cluster.get(3).executeInternal("SELECT * FROM " + KEYSPACE + ".tbl WHERE pk = 1"));
            cluster.verbs(READ_REPAIR_RSP).to(1).drop();
            RestartFramework.at("timeout_after_filter_setup").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            try {
                ...
                // it might not be the best way to check if the time taken is around the timeout value.
                long magicDelayAmount = 100L;
                ...
                assertRows(cluster.get(3).executeInternal("SELECT * FROM " + KEYSPACE + ".tbl WHERE pk = 1"), // the partition happened when the repaired node sending back ack. The mutation should be in fact applied.
                row(1, 1, 1));
            }
        }
    }
    ...
    public void failingReadRepairTest() throws Throwable {
        try (Cluster cluster = init(builder().withNodes(3).start())) {
            cluster.schemaChange("CREATE TABLE " + KEYSPACE + ".tbl (pk int, ck int, v int, PRIMARY KEY (pk, ck)) WITH read_repair='blocking'");
            RestartFramework.at("failing_after_table_create").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            for (int i = 1; i <= 2; ++i) cluster.get(i).executeInternal("INSERT INTO " + KEYSPACE + ".tbl (pk, ck, v) VALUES (1, 1, 1)");
            RestartFramework.at("failing_after_inserts").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertRows(cluster.get(3).executeInternal("SELECT * FROM " + KEYSPACE + ".tbl WHERE pk = 1"));
            cluster.filters().verbs(READ_REPAIR_REQ.id).to(3).drop();
            RestartFramework.at("failing_after_filter_setup").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void movingTokenReadRepairTest() throws Throwable {
        ...
        try (Cluster cluster = init(Cluster.build(4).withoutVNodes().withConfig(c -> c.set("reject_out_of_token_range_requests", false)).start(), 3)) {
            ...
            cluster.schemaChange("CREATE TABLE " + KEYSPACE + ".tbl (pk int, ck int, v int, PRIMARY KEY (pk, ck)) WITH read_repair='blocking'");
            RestartFramework.at("moving_after_table_create").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            int i = 0;
            while (true) {
            ...
            RestartFramework.at("moving_after_insert").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("moving_after_node_leaving").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void readRepairRTRangeMovementTest() throws Throwable {
        ...
        String key = "test1";
        try (Cluster cluster = init(Cluster.build().withConfig(config -> config.with(Feature.GOSSIP, Feature.NETWORK).set("read_request_timeout", String.format("%dms", Integer.MAX_VALUE)).set("native_transport_timeout", String.format("%dms", Integer.MAX_VALUE)).set("reject_out_of_token_range_requests", false)).withTokenSupplier(TokenSupplier.evenlyDistributedTokens(4)).withNodeIdTopology(NetworkTopology.singleDcNetworkTopology(4, "dc0", "rack0")).withNodes(3).start())) {
    ...
    public static class RRHelper {
        ...
        static void install(ClassLoader cl, int nodeNumber) {
            ...
            if (nodeNumber == 1) {
                new ByteBuddy().rebase(BlockingReadRepair.class).method(named("repairPartition")).intercept(MethodDelegation.to(RRHelper.class)).make().load(cl, ClassLoadingStrategy.Default.INJECTION);
            }
        }
