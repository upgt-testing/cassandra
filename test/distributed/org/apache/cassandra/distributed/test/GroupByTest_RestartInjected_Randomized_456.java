...
public class GroupByTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void groupByWithDeletesAndSrpOnPartitions() throws Throwable {
        try (Cluster cluster = init(builder().withNodes(2).withConfig((cfg) -> cfg.set("user_defined_functions_enabled", "true")).start())) {
            ...
            RestartFramework.at("after_table_and_function_create").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_node1_writes").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_node2_writes").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void groupByWithDeletesAndSrpOnRows() throws Throwable {
        try (Cluster cluster = init(builder().withNodes(2).withConfig((cfg) -> cfg.set("user_defined_functions_enabled", "true")).start())) {
            ...
            cluster.get(1).executeInternal(withKeyspace("DELETE FROM %s.tbl WHERE pk=0 AND ck='0'"));
            RestartFramework.at("after_node1_row_writes").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.get(2).executeInternal(withKeyspace("INSERT INTO %s.tbl (pk, ck) VALUES (0, '0') USING TIMESTAMP 0"));
            ...
            RestartFramework.at("after_node2_row_writes").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            for (String limitClause : new String[] { "", "LIMIT 1", "LIMIT 10", "PER PARTITION LIMIT 1", "PER PARTITION LIMIT 10" }) {
                String query = withKeyspace("SELECT concat(ck) FROM %s.tbl GROUP BY pk " + limitClause);
                for (int i = 1; i <= 4; i++) {
                    Iterator<Object[]> rows = cluster.coordinator(2).executeWithPaging(query, ConsistencyLevel.ALL, i);
    ...
    public void testGroupByWithAggregatesAndPaging() throws Throwable {
        try (Cluster cluster = init(builder().withNodes(2).withConfig((cfg) -> cfg.set("user_defined_functions_enabled", "true")).start())) {
            ...
            for (int i = 1; i <= 4; i++) {
                assertRows(cluster.coordinator(1).executeWithPaging(withKeyspace("select concat(v1), concat(v2), concat(v3) from %s.tbl where pk = 1 group by pk"), ConsistencyLevel.ALL, i), row("_ 1 2 3", "_ 1 2 3", "_ 1 2 3"));
                assertRows(cluster.coordinator(1).executeWithPaging(withKeyspace("select concat(v1), concat(v2), concat(v3) from %s.tbl where pk = 1 group by pk limit 1"), ConsistencyLevel.ALL, i), row("_ 1 2 3", "_ 1 2 3", "_ 1 2 3"));
                assertRows(cluster.coordinator(1).executeWithPaging(withKeyspace("select * from %s.tbl where pk = 1 group by pk"), ConsistencyLevel.ALL, i), row(1, 1, "1", "1", "1"));
            }
        }
    }
    ...
    public void testGroupByTimeRangesWithTimestampType() throws Throwable {
        try (Cluster cluster = init(builder().withNodes(3).start())) {
            ...
            RestartFramework.at("after_timestamp_inserts").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            for (int pageSize : new int[] { 2, 3, 4, 5, 7, 10 }) {
                for (String startingTime : new String[] { "", ", '2016-09-27 UTC'" }) {
                    ...
                    assertRows(pagingRows, row(1, toTimestamp("2016-09-27 16:10:00 UTC"), 1, 3, 3L), row(1, toTimestamp("2016-09-27 16:15:00 UTC"), 4, 4, 1L), row(1, toTimestamp("2016-09-27 16:20:00 UTC"), 5, 6, 2L), row(1, toTimestamp("2016-09-27 16:25:00 UTC"), 7, 8, 2L), row(2, toTimestamp("2016-09-27 16:25:00 UTC"), 10, 10, 1L), row(2, toTimestamp("2016-09-27 16:30:00 UTC"), 11, 11, 1L));
                    ...
                    assertRows(pagingRows, row(1, toTimestamp("2016-09-27 16:10:00 UTC"), 1, 3, 3L), row(1, toTimestamp("2016-09-27 16:15:00 UTC"), 4, 4, 1L));
                    ...
                    pagingRows = cluster.coordinator(1).executeWithPaging(withKeyspace(stmt), QUORUM, pageSize);
                    assertRows(pagingRows, row(1, toTimestamp("2016-09-27 16:10:00 UTC"), 1, 3, 3L), row(2, toTimestamp("2016-09-27 16:25:00 UTC"), 10, 10, 1L));
                    ...
                    pagingRows = cluster.coordinator(1).executeWithPaging(withKeyspace(stmt), QUORUM, pageSize);
                    assertRows(pagingRows, row(1, toTimestamp("2016-09-27 16:25:00 UTC"), 7, 8, 2L), row(1, toTimestamp("2016-09-27 16:20:00 UTC"), 5, 6, 2L), row(1, toTimestamp("2016-09-27 16:15:00 UTC"), 4, 4, 1L), row(1, toTimestamp("2016-09-27 16:10:00 UTC"), 1, 3, 3L));
                    ...
                    pagingRows = cluster.coordinator(1).executeWithPaging(withKeyspace(stmt), QUORUM, pageSize);
                    assertRows(pagingRows, row(1, toTimestamp("2016-09-27 16:25:00 UTC"), 7, 8, 2L), row(1, toTimestamp("2016-09-27 16:20:00 UTC"), 5, 6, 2L));
                }
            }
        }
    }
    ...
    public void testGroupByTimeRangesWithDateType() throws Throwable {
        try (Cluster cluster = init(builder().withNodes(3).start())) {
            ...
            RestartFramework.at("after_date_inserts").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            for (int pageSize : new int[] { 2, 3, 4, 5, 7, 10 }) {
                for (String startingTime : new String[] { "", ", '2016-06-01'" }) {
                    ...
                    Iterator<Object[]> pagingRows = cluster.coordinator(1).executeWithPaging(withKeyspace(stmt), QUORUM, pageSize);
                    assertRows(pagingRows, row(1, toLocalDate("2016-09-01"), 1, 4, 4L), row(1, toLocalDate("2016-10-01"), 5, 7, 3L), row(1, toLocalDate("2016-11-01"), 8, 8, 1L), row(2, toLocalDate("2016-11-01"), 10, 11, 2L));
                    ...
                    pagingRows = cluster.coordinator(1).executeWithPaging(withKeyspace(stmt), QUORUM, pageSize);
                    assertRows(pagingRows, row(1, toLocalDate("2016-09-01"), 1, 4, 4L), row(1, toLocalDate("2016-10-01"), 5, 7, 3L));
                    ...
                    pagingRows = cluster.coordinator(1).executeWithPaging(withKeyspace(stmt), QUORUM, pageSize);
                    assertRows(pagingRows, row(1, toLocalDate("2016-09-01"), 1, 4, 4L), row(2, toLocalDate("2016-11-01"), 10, 11, 2L));
                    ...
                    pagingRows = cluster.coordinator(1).executeWithPaging(withKeyspace(stmt), QUORUM, pageSize);
                    assertRows(pagingRows, row(1, toLocalDate("2016-11-01"), 8, 8, 1L), row(1, toLocalDate("2016-10-01"), 5, 7, 3L), row(1, toLocalDate("2016-09-01"), 1, 4, 4L));
                    ...
                    pagingRows = cluster.coordinator(1).executeWithPaging(withKeyspace(stmt), QUORUM, pageSize);
                    assertRows(pagingRows, row(1, toLocalDate("2016-11-01"), 8, 8, 1L), row(1, toLocalDate("2016-10-01"), 5, 7, 3L));
                }
            }
        }
    }
    ...
    public void testGroupByTimeRangesWithTimeType() throws Throwable {
        try (Cluster cluster = init(builder().withNodes(3).start())) {
            ...
            RestartFramework.at("after_time_inserts").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            for (int pageSize : new int[] { 2, 3, 4, 5, 7, 10 }) {
                String stmt = "SELECT pk, date, floor(time, 5m), min(v), max(v), count(v) FROM %s.testWithTime GROUP BY pk, date, floor(time, 5m)";
                Iterator<Object[]> pagingRows = cluster.coordinator(1).executeWithPaging(withKeyspace(stmt), QUORUM, pageSize);
                assertRows(pagingRows, row(1, toLocalDate("2016-09-27"), toTime("16:10:00"), 1, 3, 3L), row(1, toLocalDate("2016-09-27"), toTime("16:15:00"), 4, 4, 1L), row(1, toLocalDate("2016-09-27"), toTime("16:20:00"), 5, 6, 2L), row(1, toLocalDate("2016-09-27"), toTime("16:25:00"), 7, 8, 2L), row(1, toLocalDate("2016-09-28"), toTime("16:25:00"), 9, 10, 2L));
                stmt = "SELECT pk, date, floor(time, 5m), min(v), max(v), count(v) FROM %s.testWithTime GROUP BY pk, date, floor(time, 5m) LIMIT 2";
                pagingRows = cluster.coordinator(1).executeWithPaging(withKeyspace(stmt), QUORUM, pageSize);
                assertRows(pagingRows, row(1, toLocalDate("2016-09-27"), toTime("16:10:00"), 1, 3, 3L), row(1, toLocalDate("2016-09-27"), toTime("16:15:00"), 4, 4, 1L));
                ...
                pagingRows = cluster.coordinator(1).executeWithPaging(withKeyspace(stmt), QUORUM, pageSize);
                assertRows(pagingRows, row(1, toLocalDate("2016-09-28"), toTime("16:25:00"), 9, 10, 2L), row(1, toLocalDate("2016-09-27"), toTime("16:25:00"), 7, 8, 2L), row(1, toLocalDate("2016-09-27"), toTime("16:20:00"), 5, 6, 2L), row(1, toLocalDate("2016-09-27"), toTime("16:15:00"), 4, 4, 1L), row(1, toLocalDate("2016-09-27"), toTime("16:10:00"), 1, 3, 3L));
                ...
                pagingRows = cluster.coordinator(1).executeWithPaging(withKeyspace(stmt), QUORUM, pageSize);
                assertRows(pagingRows, row(1, toLocalDate("2016-09-28"), toTime("16:25:00"), 9, 10, 2L), row(1, toLocalDate("2016-09-27"), toTime("16:25:00"), 7, 8, 2L));
            }
        }
    }

    private static void initFunctions(Cluster cluster) {
        cluster.schemaChange(withKeyspace("CREATE FUNCTION %s.concat_strings_fn(a text, b text) " + "RETURNS NULL ON NULL INPUT " + "RETURNS text " + "LANGUAGE java " + "AS 'return a + \" \" + b;'"));
        cluster.schemaChange(withKeyspace("CREATE AGGREGATE %s.concat(text)" + " SFUNC concat_strings_fn" + " STYPE text" + " INITCOND '_'"));
    }
