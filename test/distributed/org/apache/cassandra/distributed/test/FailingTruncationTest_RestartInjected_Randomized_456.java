...
public class FailingTruncationTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void testFailingTruncation() throws IOException {
        try (Cluster cluster = init(Cluster.build(2).withInstanceInitializer(BBFailHelper::install).start())) {
            ...
            RestartFramework.at("after_truncate_failure").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public static class BBFailHelper {
        ...
        static void install(ClassLoader cl, int nodeNumber) {
            if (nodeNumber == 2) {
                new ByteBuddy().redefine(ColumnFamilyStore.class).method(named("truncateBlocking")).intercept(MethodDelegation.to(BBFailHelper.class)).make().load(cl, ClassLoadingStrategy.Default.INJECTION);
            }
        }
