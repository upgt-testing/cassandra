...
public class CASTest_RestartInjected_Randomized_123 extends CASCommonTestCases {
    ...
    public static void beforeClass() throws Throwable {
        ...
        Consumer<IInstanceConfig> conf = config -> config.set("paxos_variant", "v2").set("write_request_timeout", REQUEST_TIMEOUT).set("cas_contention_timeout", CONTENTION_TIMEOUT).set("request_timeout", REQUEST_TIMEOUT);
    ...
    public void testIncompleteWriteSupersededByConflictingRejectedCondition() throws Throwable {
        ...
        RestartFramework.at("after_cas_timeout").on(THREE_NODES).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_cas_update").on(THREE_NODES).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_table_create").on(THREE_NODES).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertRows(THREE_NODES.coordinator(2).execute("UPDATE " + KEYSPACE + "." + tableName + " SET v = 2 WHERE pk = 1 and ck = 1 IF v = 1", QUORUM), row(false));
        drop1.off();
    ...
    private static int[] paxosAndReadVerbs() {
        return new int[] { PAXOS_PREPARE_REQ.id, PAXOS2_PREPARE_REQ.id, PAXOS2_PREPARE_REFRESH_REQ.id, PAXOS2_COMMIT_AND_PREPARE_REQ.id, PAXOS_PROPOSE_REQ.id, PAXOS2_PROPOSE_REQ.id, PAXOS_COMMIT_REQ.id, READ_REQ.id };
    }
    ...
    public void readConsistencyAfterWriteTimeoutTest() throws IOException {
        BiConsumer<String, ICoordinator> operation = (table, coordinator) -> assertRows(coordinator.execute("SELECT * FROM " + table + " WHERE k=0", SERIAL));
    ...
    public void fastReadsAndFailedWrites() throws IOException {
        ...
        IMessageFilters.Filter dropProposeFilter = THREE_NODES.filters().verbs(PAXOS_PROPOSE_REQ.id, PAXOS2_PROPOSE_REQ.id, PAXOS_COMMIT_REQ.id, PAXOS2_COMMIT_AND_PREPARE_REQ.id).to(1, 2).drop();
        try {
            // shouldn't timeout
            THREE_NODES.coordinator(1).execute("SELECT * FROM " + table + " WHERE k = 1", SERIAL);
            THREE_NODES.coordinator(1).execute("SELECT * FROM " + table + " WHERE k = 1", SERIAL);
            THREE_NODES.coordinator(1).execute("SELECT * FROM " + table + " WHERE k = 1", SERIAL);
            RestartFramework.at("after_serial_reads").on(THREE_NODES).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void nonApplyingCasConsistencyAfterWriteTimeout() throws IOException {
        ...
        BiConsumer<String, ICoordinator> operation = (table, coordinator) -> assertCasNotApplied(coordinator.execute("UPDATE " + table + " SET v = 1 WHERE k = 0 IF v = 0", ANY));
        consistencyAfterWriteTimeoutTest(operation, operation, false, THREE_NODES);
        consistencyAfterWriteTimeoutTest(operation, operation, true, THREE_NODES);
    }
    ...
    public void mixedReadAndNonApplyingCasConsistencyAfterWriteTimeout() throws IOException {
        BiConsumer<String, ICoordinator> operation1 = (table, coordinator) -> assertRows(coordinator.execute("SELECT * FROM " + table + " WHERE k=0", SERIAL));
        BiConsumer<String, ICoordinator> operation2 = (table, coordinator) -> assertCasNotApplied(coordinator.execute("UPDATE " + table + " SET v = 1 WHERE k = 0 IF v = 0", QUORUM));
    ...
    public void mixedNonApplyingCasAndReadConsistencyAfterWriteTimeout() throws IOException {
        // Note: we use CL.ANY so that the operation don't timeout in the case where we "lost" the operation1 commits.
        // The commit CL shouldn't have impact on this test anyway, so this doesn't diminishes the test.
        BiConsumer<String, ICoordinator> operation1 = (table, coordinator) -> assertCasNotApplied(coordinator.execute("UPDATE " + table + " SET v = 1 WHERE k = 0 IF v = 0", ANY));
        BiConsumer<String, ICoordinator> operation2 = (table, coordinator) -> assertRows(coordinator.execute("SELECT * FROM " + table + " WHERE k=0", SERIAL));
        consistencyAfterWriteTimeoutTest(operation1, operation2, false, THREE_NODES);
        consistencyAfterWriteTimeoutTest(operation1, operation2, true, THREE_NODES);
    }
    ...
    private void assertCasNotApplied(Object[][] resultSet) {
        assertFalse("Expected a CAS resultSet (with at least application result) but got an empty one.", resultSet.length == 0);
        assertFalse("Invalid empty first row in CAS resultSet.", resultSet[0].length == 0);
        Object wasApplied = resultSet[0][0];
        assertTrue("Expected 1st column of CAS resultSet to be a boolean, but got a " + wasApplied.getClass(), wasApplied instanceof Boolean);
    ...
    public void testSuccessfulWriteBeforeRangeMovement() throws Throwable {
        ...
        assertRows(executeWithRetry(FOUR_NODES.coordinator(1), "INSERT INTO " + KEYSPACE + "." + tableName + " (pk, ck, v1) VALUES (?, 1, 1) IF NOT EXISTS", ONE, pk), row(true));
        ...
        RestartFramework.at("after_table_create").on(FOUR_NODES).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_cas_insert").on(FOUR_NODES).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_ring_change").on(FOUR_NODES).restart("node").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        // {4} reads from !{2} => {3, 4}
        drop(FOUR_NODES, 4, to(2), to(2), to());
        assertRows(executeWithRetry(FOUR_NODES.coordinator(4), "INSERT INTO " + KEYSPACE + "." + tableName + " (pk, ck, v2) VALUES (?, 1, 2) IF NOT EXISTS", ONE, pk), row(false, pk, 1, 1, null));
    }
    ...
    public void testConflictingWritesWithStaleRingInformation() throws Throwable {
        String tableName = tableName("tbl");
        RestartFramework.at("after_table_create").on(FOUR_NODES).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_second_cas_insert").on(FOUR_NODES).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        FOUR_NODES.schemaChange("CREATE TABLE " + KEYSPACE + "." + tableName + " (pk int, ck int, v1 int, v2 int, PRIMARY KEY (pk, ck))");
        // make it so {1} is unaware (yet) that {4} is an owner of the token
        ...
        assertRows(executeWithRetry(FOUR_NODES.coordinator(4), "INSERT INTO " + KEYSPACE + "." + tableName + " (pk, ck, v1) VALUES (?, 1, 1) IF NOT EXISTS", ONE, pk), row(true));
        RestartFramework.at("after_first_cas_insert").on(FOUR_NODES).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        FOUR_NODES.get(1).acceptsOnInstance(CASTestBase::assertNotVisibleInRing).accept(FOUR_NODES.get(4));
        // {1} promises, accepts and commmits on !{3} => {1, 2}
        drop(FOUR_NODES, 1, to(3), to(3), to(3));
        assertRows(executeWithRetry(FOUR_NODES.coordinator(1), "INSERT INTO " + KEYSPACE + "." + tableName + " (pk, ck, v2) VALUES (?, 1, 2) IF NOT EXISTS", ONE, pk), row(false, pk, 1, 1, null));
        FOUR_NODES.get(1).acceptsOnInstance(CASTestBase::assertVisibleInRing).accept(FOUR_NODES.get(4));
    }
    ...
    public void testSucccessfulWriteDuringRangeMovementFollowedByRead() throws Throwable {
        ...
        int pk = pk(FOUR_NODES, 1, 2);
        FOUR_NODES.get(1).acceptsOnInstance(CASTestBase::assertNotVisibleInRing).accept(FOUR_NODES.get(4));
        RestartFramework.at("after_cas_insert").on(FOUR_NODES).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // {1} promises and accepts on !{3} => {1, 2}; commmits on !{2, 3} => {1}
        drop(FOUR_NODES, 1, to(3), to(3), to(2, 3));
        assertRows(executeWithRetry(FOUR_NODES.coordinator(1), "INSERT INTO " + KEYSPACE + "." + tableName + " (pk, ck, v) VALUES (?, 1, 1) IF NOT EXISTS", ONE, pk), row(true));
        RestartFramework.at("after_bootstrap_setup").on(FOUR_NODES).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        FOUR_NODES.get(1).acceptsOnInstance(CASTestBase::assertVisibleInRing).accept(FOUR_NODES.get(4));
        // finish topology change
        for (int i = 1; i <= 4; ++i) FOUR_NODES.get(i).acceptsOnInstance(CASTestBase::addToRingNormal).accept(FOUR_NODES.get(4));
        RestartFramework.at("after_topology_change").on(FOUR_NODES).restart("node").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        // {3} reads from !{2} => {3, 4}
        drop(FOUR_NODES, 3, to(2), to(), to());
        assertRows(FOUR_NODES.coordinator(3).execute("SELECT * FROM " + KEYSPACE + "." + tableName + " WHERE pk = ?", SERIAL, pk), row(pk, 1, 1));
    }
    ...
    public void testSuccessfulWriteDuringRangeMovementFollowedByConflicting() throws Throwable {
        ...
        drop(FOUR_NODES, 1, to(3), to(3), to(2, 3));
        RestartFramework.at("after_cas_insert").on(FOUR_NODES).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertRows(executeWithRetry(FOUR_NODES.coordinator(1), "INSERT INTO " + KEYSPACE + ".tbl (pk, ck, v1) VALUES (?, 1, 1) IF NOT EXISTS", ONE, pk), row(true));
        FOUR_NODES.get(1).acceptsOnInstance(CASTestBase::assertVisibleInRing).accept(FOUR_NODES.get(4));
        // finish topology change
        for (int i = 1; i <= 4; ++i) FOUR_NODES.get(i).acceptsOnInstance(CASTestBase::addToRingNormal).accept(FOUR_NODES.get(4));
        // {3} reads from !{2} => {3, 4}
        drop(FOUR_NODES, 3, to(2), to(), to());
        assertRows(FOUR_NODES.coordinator(3).execute("INSERT INTO " + KEYSPACE + ".tbl (pk, ck, v2) VALUES (?, 1, 2) IF NOT EXISTS", ONE, pk), row(false, pk, 1, 1, null));
    ...
    public void testIncompleteWriteFollowedBySuccessfulWriteWithStaleRingDuringRangeMovementFollowedByRead() throws Throwable {
        String tableName = tableName("tbl");
        FOUR_NODES.schemaChange("CREATE TABLE " + KEYSPACE + "." + tableName + " (pk int, ck int, v1 int, v2 int, PRIMARY KEY (pk, ck))");
        RestartFramework.at("after_incomplete_cas").on(FOUR_NODES).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // make it so {4} is bootstrapping, and this has propagated to only a quorum of other nodes
        for (int i = 1; i <= 4; ++i) {
            FOUR_NODES.get(i).acceptsOnInstance(CASTestBase::removeFromRing).accept(FOUR_NODES.get(4));
            FOUR_NODES.get(i).acceptsOnInstance(CASTestBase::assertNotVisibleInRing).accept(FOUR_NODES.get(4));
        ...
        RestartFramework.at("after_topology_change").on(FOUR_NODES).restart("node").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_second_cas_insert").on(FOUR_NODES).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        FOUR_NODES.get(1).acceptsOnInstance(CASTestBase::assertVisibleInRing).accept(FOUR_NODES.get(4));
        RestartFramework.at("after_table_create").on(FOUR_NODES).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_bootstrap_setup").on(FOUR_NODES).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        // finish topology change
        ...
        assertRows(FOUR_NODES.coordinator(3).execute("SELECT * FROM " + KEYSPACE + "." + tableName + " WHERE pk = ?", SERIAL, pk), expectRow);
    }
    ...
    public void testIncompleteWriteFollowedBySuccessfulWriteWithStaleRingDuringRangeMovementFollowedByWrite() throws Throwable {
        ...
        assertRows(FOUR_NODES.coordinator(3).execute("INSERT INTO " + KEYSPACE + "." + tableName + " (pk, ck, v2) VALUES (?, 1, 2) IF NOT EXISTS", ONE, pk), expectRow);
    }
    ...
    static void consistencyAfterWriteTimeoutTest(BiConsumer<String, ICoordinator> postTimeoutOperation1, BiConsumer<String, ICoordinator> postTimeoutOperation2, boolean loseCommitOfOperation1, Cluster cluster) {
        ...
        // We do a CAS insertion, but have with the PROPOSE message dropped on node 1 and 2. The CAS will not get
        // through and should timeout. Importantly, node 3 does receive and answer the PROPOSE.
        IMessageFilters.Filter dropProposeFilter = cluster.filters().verbs(PAXOS_PROPOSE_REQ.id, PAXOS2_PROPOSE_REQ.id).to(1, 2).drop();
        // Prepare A to {1, 2, 3}
        // Propose A to {3}
        // Timeout
        try {
            // NOTE: the consistency below is the "commit" one, so it doesn't matter at all here.
            cluster.coordinator(1).execute("INSERT INTO " + table + "(k, v) VALUES (0, 0) IF NOT EXISTS", ONE);
    ...
    public void testWriteTimeoutExceptionUsingPaxosInLwtPerformaceTest() throws IOException {
        ...
        RestartFramework.at("after_table_create").on(THREE_NODES).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_keyspace_create").on(THREE_NODES).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_cas_update").on(THREE_NODES).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        String tableName = tableName("t");
        RestartFramework.at("after_cas_insert").on(THREE_NODES).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        String table = KEYSPACE + "." + tableName;
        THREE_NODES.schemaChange("CREATE TABLE " + table + " (k int PRIMARY KEY, v int)");
