 ...
 */
public class ReplicaFilteringProtectionTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public static void setup() throws IOException {
        cluster = init(Cluster.build().withNodes(REPLICAS).withConfig(config -> config.set("hinted_handoff_enabled", false)).start());
    ...
    private void testMissedUpdates(String tableName, int warnThreshold, int failThreshold, boolean shouldWarn) {
        ...
        for (int i = 0; i < PARTITIONS; i++) for (int j = 0; j < ROWS_PER_PARTITION; j++) cluster.coordinator(1).execute("INSERT INTO " + fullTableName + "(k, c, v) VALUES (?, ?, 'old')", ALL, i, j);
        RestartFramework.at("after_second_update").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_first_update").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_initial_query").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_threshold_config").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertRows(initialRows, row(1, 0, "old"), row(1, 1, "old"), row(1, 2, "old"), row(0, 0, "old"), row(0, 1, "old"), row(0, 2, "old"), row(2, 0, "old"), row(2, 1, "old"), row(2, 2, "old"));
        RestartFramework.at("after_initial_inserts").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_read_repair").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_divergent_query").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Object[][] newRows = newResult.toObjectArrays();
        assertRows(newRows, row(1, 0, "new"), row(1, 1, "new"), row(1, 2, "new"), row(0, 0, "new"), row(0, 1, "new"), row(0, 2, "new"), row(2, 0, "new"), row(2, 1, "new"), row(2, 2, "new"));
        ...
        RestartFramework.at("after_final_query").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        Object[][] futureRows = futureResult.toObjectArrays();
        assertRows(futureRows, row(1, 0, "future"), row(1, 1, "future"), row(1, 2, "future"), row(0, 0, "future"), row(0, 1, "future"), row(0, 2, "future"), row(2, 0, "future"), row(2, 1, "future"), row(2, 2, "future"));
    ...
    private void updateAllRowsOn(int node, String table, String value) {
        for (int i = 0; i < PARTITIONS; i++) for (int j = 0; j < ROWS_PER_PARTITION; j++) cluster.get(node).executeInternal("UPDATE " + table + " SET v = ? WHERE k = ? and c = ?", value, i, j);
    }
    ...
    private long protectionQueryCount(IInvokableInstance instance, String tableName) {
        return instance.callOnInstance(() -> Keyspace.open(KEYSPACE).getColumnFamilyStore(tableName).metric.replicaFilteringProtectionRequests.getCount());
    }

    private long maxRowsCachedPerQuery(IInvokableInstance instance, String tableName) {
        return instance.callOnInstance(() -> Keyspace.open(KEYSPACE).getColumnFamilyStore(tableName).metric.rfpRowsCachedPerQuery.getSnapshot().getMax());
    }

    private long minRowsCachedPerQuery(IInvokableInstance instance, String tableName) {
        return instance.callOnInstance(() -> Keyspace.open(KEYSPACE).getColumnFamilyStore(tableName).metric.rfpRowsCachedPerQuery.getSnapshot().getMin());
    }

    private long rowsCachedPerQueryCount(IInvokableInstance instance, String tableName) {
        return instance.callOnInstance(() -> Keyspace.open(KEYSPACE).getColumnFamilyStore(tableName).metric.rfpRowsCachedPerQuery.getCount());
    }
}
