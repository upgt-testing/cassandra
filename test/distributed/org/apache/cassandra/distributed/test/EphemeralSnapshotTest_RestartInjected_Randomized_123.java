...
public class EphemeralSnapshotTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public void testStartupRemovesEphemeralSnapshotOnEphemeralFlagInManifest() throws Exception {
        try (Cluster c = init(builder().withNodes(1).withConfig(config -> config.with(GOSSIP, NETWORK, NATIVE_PROTOCOL)).start())) {
            Pair<String, String[]> initialisationData = initialise(c);
            RestartFramework.at("after_initialization").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            rewriteManifestToEphemeral(initialisationData.left, initialisationData.right);
            RestartFramework.at("after_manifest_rewrite").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            verify(c.get(1));
            RestartFramework.at("after_verification").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public void testStartupRemovesEphemeralSnapshotOnMarkerFile() throws Exception {
        try (Cluster c = init(builder().withNodes(1).withConfig(config -> config.with(GOSSIP, NETWORK, NATIVE_PROTOCOL)).start())) {
            ...
            Path ephemeralMarkerFile = Paths.get(dataDirs[0]).resolve(KEYSPACE).resolve(format("%s-%s", tableName, tableId)).resolve("snapshots").resolve(snapshotName).resolve("ephemeral.snapshot");
            Files.createFile(ephemeralMarkerFile);
            RestartFramework.at("after_marker_file_create").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            verify(c.get(1));
            RestartFramework.at("after_verification").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public void testEphemeralSnapshotIsNotClearableFromNodetool() throws Exception {
        try (Cluster c = init(builder().withNodes(1).withConfig(config -> config.with(GOSSIP, NETWORK, NATIVE_PROTOCOL)).start())) {
            ...
            RestartFramework.at("after_listsnapshots").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            assertTrue(instance.nodetoolResult("listsnapshots", "-e").getStdout().contains(snapshotName));
            RestartFramework.at("after_clearsnapshot_attempt").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertFalse(instance.logs().grep("Skipping deletion of ephemeral snapshot 'snapshotname' in keyspace distributed_test_keyspace. " + "Ephemeral snapshots are not removable by a user.").getResult().isEmpty());
            RestartFramework.at("after_verification").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public void testClearingAllSnapshotsFromNodetoolWillKeepEphemeralSnaphotsIntact() throws Exception {
        try (Cluster c = init(builder().withNodes(1).withConfig(config -> config.with(GOSSIP, NETWORK, NATIVE_PROTOCOL)).start())) {
            ...
            RestartFramework.at("after_clearsnapshot_all").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertTrue(instance.nodetoolResult("listsnapshots", "-e").getStdout().contains(snapshotName));
    ...
    private void waitForSnapshot(IInvokableInstance instance, String snapshotName) {
        await().timeout(20, SECONDS).pollInterval(1, SECONDS).until(() -> instance.nodetoolResult("listsnapshots", "-e").getStdout().contains(snapshotName));
    }
    ...
    private Path findManifest(String[] dataDirs, String tableId) {
        for (String dataDir : dataDirs) {
            Path manifest = Paths.get(dataDir).resolve(KEYSPACE).resolve(format("%s-%s", tableName, tableId)).resolve("snapshots").resolve(snapshotName).resolve("manifest.json");
