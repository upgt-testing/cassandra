...
public class SSTableLoaderEncryptionOptionsTest_RestartInjected_Randomized_123 extends AbstractEncryptionOptionsImpl {
    ...
    public static void setupCluster() throws IOException {
        CLUSTER = Cluster.build().withNodes(1).withConfig(c -> {
            // need gossip to get hostid for java driver
            c.with(Feature.NATIVE_PROTOCOL, Feature.NETWORK, Feature.GOSSIP);
            c.set("server_encryption_options", ImmutableMap.builder().putAll(validKeystore).put("internode_encryption", "all").put("optional", false).build());
            c.set("client_encryption_options", ImmutableMap.builder().putAll(validKeystore).put("enabled", true).put("optional", false).put("accepted_protocols", Collections.singletonList("TLSv1.2")).build());
        }).start();
    ...
    public void bulkLoaderSuccessfullyStreamsOverSslWithDeprecatedSslStoragePort() throws Throwable {
        File sstables_to_upload = prepareSstablesForUpload();
        RestartFramework.at("after_sstable_preparation").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ToolRunner.ToolResult tool = ToolRunner.invokeClass(BulkLoader.class, "--nodes", NODES, "--port", Integer.toString(NATIVE_PORT), "--storage-port", Integer.toString(STORAGE_PORT), "--ssl-storage-port", Integer.toString(SSL_STORAGE_PORT), "--keystore", validKeyStorePath, "--keystore-password", validKeyStorePassword, "--truststore", validTrustStorePath, "--truststore-password", validTrustStorePassword, "--conf-path", "test/conf/sstableloader_with_encryption.yaml", sstables_to_upload.absolutePath());
        tool.assertOnCleanExit();
        assertTrue(tool.getStdout().contains("Summary statistics"));
        assertTrue(tool.getStdout().contains("ssl storage port is deprecated and not used"));
        RestartFramework.at("after_bulk_load_deprecated_port").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertRows(CLUSTER.get(1).executeInternal("SELECT count(*) FROM ssl_upload_tables.test"), row(42L));
    }

    @Test
    public void bulkLoaderCannotAgreeOnClientTLSProtocol() {
        RestartFramework.at("before_tls_protocol_mismatch").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ToolRunner.ToolResult tool = ToolRunner.invokeClass(BulkLoader.class, "--ssl-protocol", "TLSv1", "--nodes", NODES, "--port", Integer.toString(NATIVE_PORT), "--storage-port", Integer.toString(STORAGE_PORT), "--keystore", validKeyStorePath, "--keystore-password", validKeyStorePassword, "--truststore", validTrustStorePath, "--truststore-password", validTrustStorePassword, "test/data/legacy-sstables/na/legacy_tables/legacy_na_clust");
    ...
    private static void generateSstables() throws IOException {
        ...
        CLUSTER.get(1).runOnInstance(rethrow(() -> StorageService.instance.forceKeyspaceFlush("ssl_upload_tables", ColumnFamilyStore.FlushReason.UNIT_TESTS)));
    }
