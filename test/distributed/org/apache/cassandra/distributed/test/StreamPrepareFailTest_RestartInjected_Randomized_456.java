...
public class StreamPrepareFailTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void streamPrepareFailTest() throws Throwable {
        try (Cluster cluster = init(Cluster.build(2).withInstanceInitializer(StreamFailHelper::install).withConfig(config -> config.with(NETWORK, GOSSIP)).start())) {
            ...
            try {
                RestartFramework.at("before_rebuild_failure").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public static class StreamFailHelper {
        ...
        static void install(ClassLoader cl, int nodeNumber) {
            new ByteBuddy().redefine(StreamSession.class).method(named("prepareAsync")).intercept(MethodDelegation.to(StreamFailHelper.class)).make().load(cl, ClassLoadingStrategy.Default.INJECTION);
        }
