...
public class AutoSnapshotTtlTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void testAutoSnapshotTTlOnTruncate() throws IOException {
        try (Cluster cluster = init(build().withNodes(1).withConfig(c -> c.with(Feature.GOSSIP).set("auto_snapshot_ttl", String.format("%ds", FIVE_SECONDS))).start())) {
            ...
            RestartFramework.at("after_truncate").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            await().timeout(10, SECONDS).pollInterval(1, SECONDS).until(() -> !instance.nodetoolResult("listsnapshots").getStdout().contains(SNAPSHOT_DROP_PREFIX));
        }
    }
    ...
    public void testAutoSnapshotTTlOnDropAfterRestart() throws IOException {
        ...
        int ONE_MINUTE = 60;
        try (Cluster cluster = init(build().withNodes(1).withConfig(c -> c.with(Feature.GOSSIP).set("auto_snapshot_ttl", String.format("%ds", ONE_MINUTE))).start())) {
            IInvokableInstance instance = cluster.get(1);
            cluster.schemaChange(withKeyspace("CREATE TABLE %s.tbl (key int, value text, PRIMARY KEY (key))"));
            RestartFramework.at("after_table_create").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            populate(cluster);
            ...
            instance.startup();
            RestartFramework.at("after_manual_restart").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Check snapshot is listed after restart
            instance.nodetoolResult("listsnapshots").asserts().success().stdoutContains(SNAPSHOT_DROP_PREFIX);
            RestartFramework.at("after_snapshot_check").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Check snapshot is removed after at most auto_snapshot_ttl + 1s
            await().timeout(ONE_MINUTE + 1, SECONDS).pollInterval(1, SECONDS).until(() -> !instance.nodetoolResult("listsnapshots").getStdout().contains(SNAPSHOT_DROP_PREFIX));
        }
    }
    ...
    public void testAutoSnapshotTtlDisabled() throws IOException, InterruptedException {
        try (Cluster cluster = init(build().withNodes(1).withConfig(c -> c.with(Feature.GOSSIP)).start())) {
            IInvokableInstance instance = cluster.get(1);
            cluster.schemaChange(withKeyspace("CREATE TABLE %s.tbl (key int, value text, PRIMARY KEY (key))"));
            RestartFramework.at("after_table_create").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            populate(cluster);
