...
public class MultipleDataDirectoryTest_RestartInjected extends TestBaseImpl
{
    ...
    {
        RestartFramework.at("after_data_population")
            .on(CLUSTER)
            .restart("node")
            .withIndex(0)
    ...
    public void testDetectSSTableMisplaced()
    {
        ...
        RestartFramework.at("after_setup_misplaced_sstables")
            .on(CLUSTER)
            .restart("node")
            .withIndex(0)
            .withMode(RestartMode.GRACEFUL)
    ...
    public void testNodeToolRelocateSSTablesFindNoFilesToMove()
    {
        ...
        RestartFramework.at("before_relocatesstables")
            .on(CLUSTER)
            .restart("node")
            .withIndex(0)
            .withMode(RestartMode.GRACEFUL)
        ...
        NODE.nodetoolResult("relocatesstables", KEYSPACE, "cf")
            .asserts()
            .success();
        RestartFramework.at("after_relocatesstables")
            .on(CLUSTER)
            .restart("node")
            .withIndex(0)
            .withMode(RestartMode.GRACEFUL)
    ...
    {
        setupMisplacedSSTables();
        RestartFramework.at("after_setup_misplaced_sstables_for_relocation")
            .on(CLUSTER)
            .restart("node")
            .withIndex(0)
            .withMode(RestartMode.GRACEFUL)
            .execute();
        long logStartLoc = NODE.logs().mark();
        RestartFramework.at("before_relocatesstables_with_misplaced")
            .on(CLUSTER)
            .restart("node")
            .withIndex(0)
            .withMode(RestartMode.GRACEFUL)
        ...
        NODE.nodetoolResult("relocatesstables", KEYSPACE, "cf")
            .asserts()
            .success();
        RestartFramework.at("after_relocatesstables_with_misplaced")
            .on(CLUSTER)
            .restart("node")
            .withIndex(0)
            .withMode(RestartMode.GRACEFUL)
