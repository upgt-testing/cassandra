...
public class JVMDTestTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public void instanceLogs() throws IOException, TimeoutException {
        try (Cluster cluster = init(Cluster.build(2).withConfig(c -> c.with(Feature.values())).start())) {
            ...
            RestartFramework.at("after_exception_trigger").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            List<String> errors = logs.watchFor(mark, "^ERROR").getResult();
            Assertions.assertThat(errors).// can't check for "fail without fail" since thats on the next line, and watchFor doesn't
            // stitch lines together like grepForError does
            allMatch(s -> s.contains("ERROR")).allMatch(s -> s.contains("isolatedExecutor")).allMatch(s -> s.contains("Exception in thread")).as("Unable to find 'ERROR', 'isolatedExecutor', and 'Exception in thread'").isNotEmpty();
        }
    }
    ...
    public void nonSharedConfigClassTest() throws IOException {
        Map<String, Object> commitLogCompression = ImmutableMap.of("class_name", "org.apache.cassandra.io.compress.LZ4Compressor", "parameters", Collections.<String, Object>emptyMap());
        Map<String, Object> encryptionOptions = ImmutableMap.of("cipher_suites", Collections.singletonList("FakeCipher"), "optional", false, "enabled", false);
    ...
    public void modifySchemaWithStoppedNode() throws Throwable {
        try (Cluster cluster = init(Cluster.build().withNodes(2).withConfig(c -> c.with(Feature.GOSSIP).with(Feature.NETWORK)).start())) {
            ...
            RestartFramework.at("after_first_schema_change").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_second_schema_change").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            assertFalse(cluster.get(1).isShutdown());
            assertFalse(cluster.get(2).isShutdown());
            RestartFramework.at("after_both_nodes_restart").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.schemaChangeIgnoringStoppedInstances("CREATE TABLE " + KEYSPACE + ".tbl3 (id int primary key, i int)");
            RestartFramework.at("after_third_schema_change").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            assertRows(cluster.get(1).executeInternal("SELECT table_name FROM system_schema.tables WHERE keyspace_name = ?", KEYSPACE), row("tbl1"), row("tbl2"), row("tbl3"));
            assertRows(cluster.get(2).executeInternal("SELECT table_name FROM system_schema.tables WHERE keyspace_name = ?", KEYSPACE), row("tbl1"), row("tbl2"), row("tbl3"));
            ...
            assertRows(cluster.get(2).executeInternal("SELECT table_name FROM system_schema.tables WHERE keyspace_name = ?", KEYSPACE), row("tbl1"), row("tbl2"), row("tbl3"), row("tbl4"));
        }
    }
}
