...
public class CASAddTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public void testAddition() throws Throwable {
        try (Cluster cluster = init(Cluster.create(3))) {
            ...
            RestartFramework.at("after_failed_cas_update").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertRows(cluster.coordinator(1).execute("SELECT * FROM " + KEYSPACE + ".tbl WHERE pk = 1", ConsistencyLevel.SERIAL), row(1, 1));
            cluster.coordinator(1).execute("UPDATE " + KEYSPACE + ".tbl SET v = v + 1 WHERE pk = 1 IF v = 1", ConsistencyLevel.QUORUM);
            RestartFramework.at("after_successful_cas_update").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testAdditionNotExists() throws Throwable {
        try (Cluster cluster = init(Cluster.create(3))) {
            ...
            RestartFramework.at("after_null_insert").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_null_update").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_batch_errors").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_real_insert").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_cas_update").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testConcat() throws Throwable {
        try (Cluster cluster = init(Cluster.create(3))) {
            ...
            RestartFramework.at("after_failed_cas_concat").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertRows(cluster.coordinator(1).execute("SELECT * FROM " + KEYSPACE + ".tbl WHERE pk = 1", ConsistencyLevel.SERIAL), row(1, "foo"));
            cluster.coordinator(1).execute("UPDATE " + KEYSPACE + ".tbl SET v = v + 'bar' WHERE pk = 1 IF v = 'foo'", ConsistencyLevel.QUORUM);
            RestartFramework.at("after_successful_cas_concat").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
