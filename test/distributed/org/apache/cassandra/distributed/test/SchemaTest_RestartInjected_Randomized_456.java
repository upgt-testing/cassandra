...
public class SchemaTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void readRepair() throws Throwable {
        try (Cluster cluster = init(Cluster.build(2).start())) {
            ...
            RestartFramework.at("after_alter_table_node1").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_flush_node2").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            cluster.get(2).schemaChangeInternal("ALTER TABLE " + KEYSPACE + ".tbl ADD " + name + " list<int>");
            RestartFramework.at("after_alter_table_node2").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            cluster.get(2).shutdown().get();
            cluster.get(2).startup();
            RestartFramework.at("after_node2_restart").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            cluster.get(2).forceCompact(KEYSPACE, "tbl");
            RestartFramework.at("after_compaction").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public void readRepairWithCompaction() throws Throwable {
        try (Cluster cluster = init(Cluster.build(2).start())) {
            cluster.schemaChange("CREATE TABLE " + KEYSPACE + ".tbl (pk int, ck int, v1 int, v2 int,  primary key (pk, ck))");
            RestartFramework.at("after_table_create_compaction").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            String name = "v10";
            cluster.get(1).schemaChangeInternal("ALTER TABLE " + KEYSPACE + ".tbl ADD " + name + " list<int>");
            RestartFramework.at("after_alter_node1_compaction").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.get(1).executeInternal("INSERT INTO " + KEYSPACE + ".tbl (pk, ck, v1, v2) values (?,1,1,1)", 1);
            RestartFramework.at("after_insert_node1").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            selectSilent(cluster, name);
            cluster.get(2).flush(KEYSPACE);
            RestartFramework.at("after_first_flush_node2").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            cluster.get(2).schemaChangeInternal("ALTER TABLE " + KEYSPACE + ".tbl ADD " + name + " list<int>");
            RestartFramework.at("after_alter_node2_compaction").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ...
            cluster.get(2).flush(KEYSPACE);
            RestartFramework.at("after_second_flush_node2").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            cluster.get(2).forceCompact(KEYSPACE, "tbl");
            RestartFramework.at("after_first_compaction").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            cluster.get(2).shutdown().get();
            cluster.get(2).startup();
            RestartFramework.at("after_node2_restart_compaction").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            cluster.get(2).forceCompact(KEYSPACE, "tbl");
            RestartFramework.at("after_second_compaction").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public void schemaReset() throws Throwable {
        ...
        try (Cluster cluster = init(Cluster.build(2).withConfig(cfg -> cfg.with(Feature.GOSSIP, Feature.NETWORK)).start())) {
            ...
            RestartFramework.at("after_table_one_create").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            await(30).until(() -> cluster.get(1).callOnInstance(() -> {
                return Gossiper.instance.getLiveMembers().stream().allMatch(e -> e.equals(getBroadcastAddressAndPort()));
            }));
            RestartFramework.at("after_node2_shutdown").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_table_one_drop").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_table_two_create").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_schema_clear_initiated").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // when the 2nd node is started, schema should be back in sync
            cluster.get(2).startup();
            await(30).until(() -> clear1.isDone() && clear1.get());
            RestartFramework.at("after_node2_startup").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_schema_sync").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // now let's break schema locally and let it be reset
            cluster.get(1).runOnInstance(() -> Schema.instance.getLocalKeyspaces().get(SchemaConstants.SCHEMA_KEYSPACE_NAME).get().tables.forEach(t -> ColumnFamilyStore.getIfExists(t.keyspace, t.name).truncateBlockingWithoutSnapshot()));
            RestartFramework.at("after_schema_break").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            assertTrue(checkTablesPropagated(cluster.get(1), true, false));
            assertTrue(checkTablesPropagated(cluster.get(2), true, false));
            RestartFramework.at("after_schema_reset").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    private static boolean checkTablesPropagated(IInvokableInstance instance, boolean one, boolean two) {
        return instance.callOnInstance(() -> {
            return (Schema.instance.getTableMetadata(KEYSPACE, TABLE_ONE) != null ^ !one) && (Schema.instance.getTableMetadata(KEYSPACE, TABLE_TWO) != null ^ !two);
        });
    }
}
