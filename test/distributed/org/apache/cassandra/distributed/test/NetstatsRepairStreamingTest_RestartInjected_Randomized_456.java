...
public class NetstatsRepairStreamingTest_RestartInjected_Randomized_456 extends AbstractNetstatsStreaming {
    ...
    private void executeTest(boolean compressionEnabled) throws Exception {
        ...
        try (final Cluster cluster = Cluster.build().withNodeIdTopology(NetworkTopology.singleDcNetworkTopology(2, "dc0", "rack0")).withConfig(config -> config.with(NETWORK, GOSSIP, NATIVE_PROTOCOL).set("stream_throughput_outbound", "122KiB/s").set("compaction_throughput", "1MiB/s").set("stream_entire_sstables", false)).start()) {
            ...
            RestartFramework.at("after_disable_autocompaction").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            populateData(compressionEnabled);
            RestartFramework.at("after_populate_data").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_replication_factor_change").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
