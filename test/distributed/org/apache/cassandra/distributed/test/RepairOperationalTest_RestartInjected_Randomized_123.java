...
public class RepairOperationalTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public void compactionBehindTest() throws IOException {
        try (Cluster cluster = init(Cluster.build(2).withConfig(config -> config.with(GOSSIP).with(NETWORK)).withInstanceInitializer(ByteBuddyHelper::install).start())) {
            ...
            RestartFramework.at("after_threshold_set").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_repair_rejection").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.get(2).runOnInstance(() -> ByteBuddyHelper.pendingCompactions = 499);
            RestartFramework.at("after_reduce_compactions").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            cluster.forEach(i -> i.nodetoolResult("repair", "--full").asserts().success());
    ...
    public static class ByteBuddyHelper {
        ...
        static void install(ClassLoader cl, int nodeNumber) {
            if (nodeNumber == 2) {
                new ByteBuddy().redefine(CompactionManager.class).method(named("getPendingTasks")).intercept(MethodDelegation.to(ByteBuddyHelper.class)).make().load(cl, ClassLoadingStrategy.Default.INJECTION);
            }
        }
    ...
    public void repairUnreplicatedKStest() throws IOException {
        try (Cluster cluster = init(Cluster.build(4).withDCs(2).withConfig(config -> config.with(GOSSIP).with(NETWORK)).start())) {
            ...
            RestartFramework.at("after_repair_failure").on(cluster).restart("node").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
