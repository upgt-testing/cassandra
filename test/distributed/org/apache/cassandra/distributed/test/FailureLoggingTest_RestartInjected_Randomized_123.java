...
public class FailureLoggingTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public void testRequestBootstrapFail() throws Throwable {
        RestartFramework.at("after_bootstrap_state_set").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster.get(1).callOnInstance(() -> BBRequestFailures.bootstrapping = true);
        RestartFramework.at("after_bootstrap_fail_logged").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testRangeRequestFail() throws Throwable {
        ...
        RestartFramework.at("after_range_fail_logged").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }
    ...
    public void testReadRequestFail() throws Throwable {
        ...
        RestartFramework.at("after_read_fail_logged").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }
    ...
    public static class BBRequestFailures {
        ...
        static void install(ClassLoader cl, int nodeNumber) {
            ...
            bb.redefine(StorageService.class).method(named("isBootstrapMode")).intercept(MethodDelegation.to(BBRequestFailures.class)).make().load(cl, ClassLoadingStrategy.Default.INJECTION);
            bb.redefine(RangeCommandIterator.class).method(named("sendNextRequests")).intercept(MethodDelegation.to(BBRequestFailures.class)).make().load(cl, ClassLoadingStrategy.Default.INJECTION);
            bb.redefine(StorageProxy.class).method(named("fetchRows")).intercept(MethodDelegation.to(BBRequestFailures.class)).make().load(cl, ClassLoadingStrategy.Default.INJECTION);
        }
        ...
        @SuppressWarnings("unused")
        public static PartitionIterator fetchRows(List<SinglePartitionReadCommand> commands, org.apache.cassandra.db.ConsistencyLevel consistencyLevel, Dispatcher.RequestTime requestTime) {
            throw UnavailableException.create(org.apache.cassandra.db.ConsistencyLevel.ALL, 1, 0);
        }
    }
}
