 ...
 */
public class ClusterStorageUsageTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public void testNoReplication() throws Throwable {
        ...
        try (Cluster cluster = init(builder().withNodes(2).start(), 1)) {
            ...
            RestartFramework.at("after_verify_metrics").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public void testMultiDatacenterReplication() throws Throwable {
        ...
        try (Cluster cluster = builder().withDC("DC1", 2).withDC("DC2", 2).start()) {
            ...
            populateUserKeyspace(cluster);
            RestartFramework.at("after_populate").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            verifyLoadMetricsWithReplication(cluster.get(1));
            verifyLoadMetricsWithReplication(cluster.get(2));
            verifyLoadMetricsWithReplication(cluster.get(3));
            verifyLoadMetricsWithReplication(cluster.get(4));
            RestartFramework.at("after_verify_metrics").on(cluster).restart("node").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    private long computeUnreplicatedMetric(IInvokableInstance node, SerializableFunction<ColumnFamilyStore, Long> metric) {
        return node.callOnInstance(() -> {
            long sum = 0;
            for (Keyspace keyspace : Keyspace.all()) for (ColumnFamilyStore table : keyspace.getColumnFamilyStores()) sum += metric.apply(table) / keyspace.getReplicationStrategy().getReplicationFactor().fullReplicas;
            return sum;
        });
    }
}
