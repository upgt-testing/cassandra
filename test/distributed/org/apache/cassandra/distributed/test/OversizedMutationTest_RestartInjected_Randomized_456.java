...
public class OversizedMutationTest_RestartInjected_Randomized_456 extends TestBaseImpl {

    @Test
    public void testSingleOversizedMutation() throws Throwable {
        try (Cluster cluster = init(builder().withNodes(1).withConfig(c -> c.set("max_mutation_size", "48KiB")).start())) {
            ...
            Assertions.assertThatThrownBy(() -> cluster.coordinator(1).execute(withKeyspace(query), ALL)).hasMessageContaining("Rejected an oversized mutation (").hasMessageContaining("/49152) for keyspace: distributed_test_keyspace. Top keys are: t.1");
        }
    }
    ...
    public void testOversizedBatch() throws Throwable {
        try (Cluster cluster = init(builder().withNodes(1).withConfig(c -> c.set("max_mutation_size", "48KiB")).start())) {
            ...
            Assertions.assertThatThrownBy(() -> cluster.coordinator(1).execute(withKeyspace(query), ALL)).hasMessageContaining("Rejected an oversized mutation (").hasMessageContaining("/49152) for keyspace: ks1. Top keys are: t.1");
        }
    }
}
