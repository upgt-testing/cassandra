...
public class ClearSnapshotTest_RestartInjected_Randomized_456 extends TestBaseImpl {

    private static final Logger logger = LoggerFactory.getLogger(ClearSnapshotTest_RestartInjected.class);
    ...
    public void clearSnapshotSlowTest() throws IOException, InterruptedException, ExecutionException {
        try (Cluster cluster = init(Cluster.build(3).withConfig(config -> config.with(GOSSIP).with(NETWORK)).withInstanceInitializer(BB::install).start())) {
            ...
            RestartFramework.at("after_repair_start").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_active_repairs").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
    ...
    public static class BB {
        ...
        public static void install(ClassLoader classLoader, Integer num) {
            new ByteBuddy().rebase(Directories.class).method(named("snapshotExists")).intercept(MethodDelegation.to(BB.class)).make().load(classLoader, ClassLoadingStrategy.Default.INJECTION);
        }
