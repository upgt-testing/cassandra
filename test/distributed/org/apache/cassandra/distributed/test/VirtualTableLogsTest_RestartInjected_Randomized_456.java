...
public class VirtualTableLogsTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void testVTableOutput() throws Throwable {
        try (WithProperties properties = new WithProperties().set(LOGBACK_CONFIGURATION_FILE, "test/conf/logback-dtest_with_vtable_appender.xml");
            Cluster cluster = Cluster.build(1).withConfig(c -> c.with(Feature.values())).start()) {
            ...
            RestartFramework.at("after_query_logs").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testMultipleAppendersFailToStartNode() throws Throwable {
        ...
        try (WithProperties properties = new WithProperties().set(LOGBACK_CONFIGURATION_FILE, "test/conf/logback-dtest_with_vtable_appender_invalid.xml");
            Cluster cluster = Cluster.build(1).withConfig(c -> c.with(Feature.values()).set(Constants.KEY_DTEST_API_STARTUP_FAILURE_AS_SHUTDOWN, false)).createWithoutStarting()) {
            cluster.startup();
            fail("Node should not start as there is supposed to be invalid logback configuration file.");
        } catch (IllegalStateException ex) {
            assertEquals(format("There are multiple appenders of class %s " + "of names CQLLOG,CQLLOG2. There is only one appender of such class allowed.", VirtualTableAppender.class.getName()), ex.getMessage());
        }
    }
