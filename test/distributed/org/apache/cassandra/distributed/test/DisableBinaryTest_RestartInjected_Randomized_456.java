...
public class DisableBinaryTest_RestartInjected_Randomized_456 extends TestBaseImpl {

    private static int REQUESTS = 100;

    private static com.datastax.driver.core.Cluster driver(Cluster sut) {
        return com.datastax.driver.core.Cluster.builder().addContactPoint(sut.get(1).broadcastAddress().getHostString()).withSocketOptions(new SocketOptions().setReadTimeoutMillis(Integer.MAX_VALUE)).withPoolingOptions(new PoolingOptions().setHeartbeatIntervalSeconds(Integer.MAX_VALUE)).build();
    }

    @Test
    public void testFinishInProgressQueries() throws Throwable {
        ExecutorService executor = Executors.newFixedThreadPool(REQUESTS);
        try (Cluster control = init(Cluster.build().withNodes(1).withInstanceInitializer(BlockingSelect::install).withConfig(config -> config.with(GOSSIP, NETWORK, NATIVE_PROTOCOL)).start());
            ...
            RestartFramework.at("after_blocking_enabled").on(control).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            List<Future<?>> futures = new ArrayList<>();
            for (int i = 0; i < REQUESTS; i++) futures.add(CompletableFuture.supplyAsync(() -> session.execute("select * from tbl").one(), executor));
            control.get(1).runOnInstance(() -> {
                try {
            ...
            RestartFramework.at("after_disable_binary").on(control).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testDisallowsNewRequests() throws Throwable {
        ...
        try (Cluster control = init(Cluster.build().withNodes(1).withInstanceInitializer(BlockingSelect::install).withConfig(config -> config.with(GOSSIP, NETWORK, NATIVE_PROTOCOL)).start());
            ...
            RestartFramework.at("after_transport_stop").on(control).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public static class BlockingSelect {
        ...
        static void install(ClassLoader cl, int nodeNumber) {
            new ByteBuddy().rebase(SelectStatement.class).method(named("execute").and(takesArguments(QueryState.class, QueryOptions.class, Dispatcher.RequestTime.class))).intercept(MethodDelegation.to(BlockingSelect.class)).make().load(cl, ClassLoadingStrategy.Default.INJECTION);
        }
