...
public class SchemaDisagreementTest_RestartInjected extends TestBaseImpl
{
    /**
    ...
    public void writeWithInconsequentialSchemaDisagreement() throws Throwable
    {
        try (Cluster cluster = init(builder().withNodes(3).withConfig(config -> config.with(NETWORK)).start()))
        {
            ...
            RestartFramework.at("after_schema_disagreement")
                .on(cluster)
                .restart("node")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
            ...
            RestartFramework.at("after_write_with_disagreement")
                .on(cluster)
                .restart("node")
                .withIndex(2)
                .withMode(RestartMode.GRACEFUL)
