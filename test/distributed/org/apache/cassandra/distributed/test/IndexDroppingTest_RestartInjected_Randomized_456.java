 ...
 */
public class IndexDroppingTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void testIndexDropping() throws Throwable {
        ...
        RestartFramework.at("before_restart").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_restart").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_drop_index").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        CLUSTER.schemaChange("CREATE INDEX tb ON " + KEYSPACE + ".tb1 (c3)");
        RestartFramework.at("after_index_built").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
