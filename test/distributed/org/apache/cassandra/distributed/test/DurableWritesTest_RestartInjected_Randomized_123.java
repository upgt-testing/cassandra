...
public class DurableWritesTest_RestartInjected_Randomized_123 extends TestBaseImpl {

    @Test
    public void durableWritesDisabledTest() throws Throwable {
        try (Cluster cluster = init(Cluster.build(1).withConfig(c -> c.set("commitlog_segment_size_in_mb", 1)).start())) {
            cluster.schemaChange(withKeyspace("DROP KEYSPACE %s"));
            RestartFramework.at("after_keyspace_drop").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            cluster.get(1).runOnInstance(() -> {
                ...
                boolean containsTbl = CommitLog.instance.segmentManager.getActiveSegments().stream().anyMatch(s -> s.getDirtyTableIds().contains(wanted));
                Assert.assertFalse("Commitlog should not contain data from 'tbl'", containsTbl);
            });
            RestartFramework.at("after_commitlog_check").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
}
