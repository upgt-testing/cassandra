...
public class UpgradeSSTablesTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void upgradeSSTablesInterruptsOngoingCompaction() throws Throwable {
        try (ICluster<IInvokableInstance> cluster = init(builder().withNodes(1).withInstanceInitializer(CompactionLatchByteman::install).start())) {
            ...
            for (int cnt = 0; cnt < 5; cnt++) {
                for (int i = 0; i < 100; i++) {
                    cluster.coordinator(1).execute("INSERT INTO " + KEYSPACE + ".tbl (pk, ck, v) VALUES (?,?,?)", ConsistencyLevel.QUORUM, (cnt * 1000) + i, i, blob);
                }
            ...
            RestartFramework.at("after_upgradesstables").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            future.get();
    ...
    public void compactionDoesNotCancelUpgradeSSTables() throws Throwable {
        try (ICluster<IInvokableInstance> cluster = init(builder().withNodes(1).start())) {
            cluster.schemaChange("CREATE TABLE " + KEYSPACE + ".tbl (pk int, ck int, v text, PRIMARY KEY (pk, ck));");
            RestartFramework.at("after_table_create_test2").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.get(1).acceptsOnInstance((String ks) -> {
                ColumnFamilyStore cfs = Keyspace.open(ks).getColumnFamilyStore("tbl");
                cfs.disableAutoCompaction();
                CompactionManager.instance.setCoreCompactorThreads(1);
            ...
            RestartFramework.at("after_data_flush_test2").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            LogAction logAction = cluster.get(1).logs();
            logAction.mark();
            Assert.assertEquals(0, cluster.get(1).nodetool("upgradesstables", "-a", KEYSPACE, "tbl"));
            RestartFramework.at("after_upgradesstables_test2").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Assert.assertFalse(logAction.watchFor("Compacting").getResult().isEmpty());
            cluster.get(1).acceptsOnInstance((String ks) -> {
                ColumnFamilyStore cfs = Keyspace.open(ks).getColumnFamilyStore("tbl");
                FBUtilities.allOf(CompactionManager.instance.submitMaximal(cfs, FBUtilities.nowInSeconds(), false, OperationType.COMPACTION)).awaitUninterruptibly(1, TimeUnit.MINUTES);
            }).accept(KEYSPACE);
    ...
    public void cleanupDoesNotInterruptUpgradeSSTables() throws Throwable {
        try (ICluster<IInvokableInstance> cluster = init(builder().withNodes(1).withInstanceInitializer(UpgradeSStablesLatchByteman::install).start())) {
            cluster.schemaChange("CREATE TABLE " + KEYSPACE + ".tbl (pk int, ck int, v text, PRIMARY KEY (pk, ck));");
            RestartFramework.at("after_table_create_test3").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.get(1).acceptsOnInstance((String ks) -> {
                ColumnFamilyStore cfs = Keyspace.open(ks).getColumnFamilyStore("tbl");
                cfs.disableAutoCompaction();
            }).accept(KEYSPACE);
            ...
            for (int i = 0; i < 10000; i++) {
                cluster.coordinator(1).execute("INSERT INTO " + KEYSPACE + ".tbl (pk, ck, v) VALUES (?,?,?)", ConsistencyLevel.QUORUM, i, i, blob);
            }
            cluster.get(1).nodetool("flush", KEYSPACE, "tbl");
            RestartFramework.at("after_flush_test3").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            LogAction logAction = cluster.get(1).logs();
            logAction.mark();
    ...
    public void truncateWhileUpgrading() throws Throwable {
        try (ICluster<IInvokableInstance> cluster = init(builder().withNodes(1).withInstanceInitializer(UpgradeSStablesLatchByteman::install).start())) {
            ...
            RestartFramework.at("after_table_create_test4").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.get(1).acceptsOnInstance((String ks) -> {
                ColumnFamilyStore cfs = Keyspace.open(ks).getColumnFamilyStore("tbl");
                cfs.disableAutoCompaction();
                CompactionManager.instance.setCoreCompactorThreads(1);
            ...
            String blob = "blob";
            for (int i = 0; i < 10; i++) blob += blob;
            for (int i = 0; i < 500; i++) {
                cluster.coordinator(1).execute(withKeyspace("INSERT INTO %s.tbl (pk, ck, v) VALUES (?,?,?)"), ConsistencyLevel.QUORUM, i, i, blob);
                if (i > 0 && i % 100 == 0)
                    cluster.get(1).nodetool("flush", KEYSPACE, "tbl");
            }
            RestartFramework.at("after_data_writes_test4").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            LogAction logAction = cluster.get(1).logs();
            logAction.mark();
        ...
        try (ICluster<IInvokableInstance> cluster = builder().withNodes(1).withDataDirCount(1).start()) {
            for (String compressionBefore : new String[] { "{'class' : 'LZ4Compressor', 'chunk_length_in_kb' : 32}", "{'enabled': 'false'}" }) {
                for (String command : new String[] { "upgradesstables", "recompress_sstables" }) {
                    ...
                    RestartFramework.at("after_table_create_test5").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                    cluster.get(1).acceptsOnInstance((String ks) -> {
                    ...
                    cluster.get(1).nodetool("flush", KEYSPACE, "tbl");
                    RestartFramework.at("after_first_flush_test5").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                    Assert.assertEquals(0, cluster.get(1).nodetool("upgradesstables", "-a", KEYSPACE, "tbl"));
                    ...
                    cluster.get(1).nodetool("flush", KEYSPACE, "tbl");
                    RestartFramework.at("after_second_flush_test5").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                    LogAction logAction = cluster.get(1).logs();
                    logAction.mark();
    ...
    public static class UpgradeSStablesLatchByteman {
        ...
        public static void install(ClassLoader classLoader, Integer num) {
            new ByteBuddy().rebase(ActiveCompactions.class).method(named("beginCompaction")).intercept(MethodDelegation.to(UpgradeSStablesLatchByteman.class)).make().load(classLoader, ClassLoadingStrategy.Default.INJECTION);
        }
    ...
    public static class CompactionLatchByteman {
        ...
        public static void install(ClassLoader classLoader, Integer num) {
            new ByteBuddy().rebase(ActiveCompactions.class).method(named("beginCompaction")).intercept(MethodDelegation.to(CompactionLatchByteman.class)).make().load(classLoader, ClassLoadingStrategy.Default.INJECTION);
        }
