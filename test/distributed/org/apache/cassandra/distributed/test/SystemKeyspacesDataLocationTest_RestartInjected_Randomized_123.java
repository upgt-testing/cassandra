...
public class SystemKeyspacesDataLocationTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public void preparedStatementsShouldNotBeMovedOnStartupTest() throws IOException, ExecutionException, InterruptedException {
        try (Cluster cluster = Cluster.build(1).withDataDirCount(3).start()) {
            ...
            RestartFramework.at("after_sstables_create").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_initial_location_verification").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.get(1).shutdown().get();
            // we expect that for prepared statements, sstables will not be moved to the first data directory on startup
            cluster.get(1).startup();
            RestartFramework.at("after_manual_startup").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private static Set<String> getPreparedStatementsDataLocations(IInvokableInstance i) {
        return i.callOnInstance(() -> ColumnFamilyStore.getIfExists(SchemaConstants.SYSTEM_KEYSPACE_NAME, SystemKeyspace.PREPARED_STATEMENTS).getLiveSSTables().stream().// sstables are located at <data-location>/<keyspace-name>/<table-name-with-id> so we need to go up two directories to get the data location
        map(sstr -> sstr.descriptor.directory.parent().parent().toString()).collect(Collectors.toSet()));
    }
