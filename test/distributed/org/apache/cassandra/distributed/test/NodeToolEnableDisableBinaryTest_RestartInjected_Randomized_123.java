...
public class NodeToolEnableDisableBinaryTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public void setupEnv() throws IOException {
        if (cluster == null) {
            cluster = init(builder().withNodes(1).withConfig(config -> config.with(NETWORK, GOSSIP, NATIVE_PROTOCOL)).start());
    ...
    public void testMaybeChangeDocs() {
        ...
        RestartFramework.at("after_disablebinary_help").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_enablebinary_help").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testEnableDisableBinary() throws Throwable {
        ...
        RestartFramework.at("after_disable_verification").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_enable_binary").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertTrue(tool.getCleanedStderr().isEmpty());
        assertEquals(0, tool.getExitCode());
        RestartFramework.at("after_disable_binary").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_initial_connection_check").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        assertEquals(0, tool.getExitCode());
        assertTrue(canConnect());
        RestartFramework.at("after_enable_verification").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }
