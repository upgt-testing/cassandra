 ...
 */
public class StreamDisconnectedWhileReceivingTest_RestartInjected_Randomized_123 extends TestBaseImpl {

    static final Logger logger = LoggerFactory.getLogger(StreamDisconnectedWhileReceivingTest_RestartInjected.class);
    ...
    private void disconnectControlChannel(boolean zeroCopyStreaming) throws IOException, InterruptedException {
        try (Cluster cluster = Cluster.build(2).withInstanceInitializer(BBHelper::install).withConfig(c -> c.with(Feature.values()).set("stream_entire_sstables", zeroCopyStreaming).set("autocompaction_on_startup_enabled", false)).start()) {
            ...
            RestartFramework.at("during_repair").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
    ...
    public static class BBHelper {
        ...
        public static void install(ClassLoader classLoader, Integer num) {
            ...
            num != 2)
                return;
            new ByteBuddy().rebase(StreamSession.class).method(named("receive").and(takesArguments(1))).intercept(MethodDelegation.to(BBHelper.class)).make().load(classLoader, ClassLoadingStrategy.Default.INJECTION);
        }
    }
}
