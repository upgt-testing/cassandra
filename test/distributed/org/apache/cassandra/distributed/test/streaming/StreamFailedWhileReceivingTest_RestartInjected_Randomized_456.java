 ...
 */
public class StreamFailedWhileReceivingTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    private void streamClose(boolean zeroCopyStreaming) throws IOException {
        try (Cluster cluster = Cluster.build(2).withInstanceInitializer(BBHelper::install).withConfig(c -> c.with(Feature.values()).set("stream_entire_sstables", zeroCopyStreaming).set("autocompaction_on_startup_enabled", false)).start()) {
            init(cluster);
            ...
            for (int i = 1; i <= 100; i++) node1.executeInternal(withKeyspace("INSERT INTO %s.tbl (pk) VALUES (?)"), i);
            ...
            RestartFramework.at("after_scrub_check").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public static class BBHelper {
        ...
        // per-instance
        static CountDownLatch firstStreamAbort = CountDownLatch.newCountDownLatch(1);
        ...
        public static Descriptor newSSTableDescriptor(File directory, Version version, @SuperCall Callable<Descriptor> zuper) throws Exception {
            if (isCaller(CassandraEntireSSTableStreamReader.class.getName(), "read")) // handles compressed and non-compressed
            {
                if (firstSession != null) {
                    firstSession.abort();
                    // delay here until CassandraStreamReceiver abort is called on NonPeriodic tasks
        ...
        public static void install(ClassLoader classLoader, Integer num) {
            if (// only target the second instance
            num != 2)
                return;
            new ByteBuddy().rebase(CassandraStreamManager.class).method(named("prepareIncomingStream").and(takesArguments(2))).intercept(MethodDelegation.to(BBHelper.class)).make().load(classLoader, ClassLoadingStrategy.Default.INJECTION);
            ...
            new ByteBuddy().rebase(ColumnFamilyStore.class).method(named("newSSTableDescriptor").and(takesArguments(3))).intercept(MethodDelegation.to(BBHelper.class)).make().load(classLoader, ClassLoadingStrategy.Default.INJECTION);
            new ByteBuddy().rebase(CassandraStreamReceiver.class).method(named("abort").and(takesArguments(0))).intercept(MethodDelegation.to(BBHelper.class)).make().load(classLoader, ClassLoadingStrategy.Default.INJECTION);
        }
    }
}
