...
public class StreamFailureLogsFailureDueToSessionTimeoutTest_RestartInjected_Randomized_123 extends AbstractStreamFailureLogs {

    @Test
    public void failureDueToSessionTimeout() throws IOException {
        try (Cluster cluster = Cluster.build(2).withInstanceInitializer(BBStreamTimeoutHelper::install).withConfig(c -> c.with(Feature.values()).// when die, this will try to halt JVM, which is easier to validate in the test
        // other levels require checking state of the subsystems
        set("stream_transfer_task_timeout", "1ms")).start()) {
            init(cluster);
            ...
            try {
                Awaitility.await("Did not see stream running or timed out").atMost(3, TimeUnit.MINUTES).until(() -> State.STREAM_IS_RUNNING.await(false) || searchForLog(cluster.get(1), false, "Session timed out"));
                RestartFramework.at("after_first_await").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            } finally {
                State.UNBLOCK_STREAM.signal();
            }
            RestartFramework.at("after_unblock_signal").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            Awaitility.await("Unable to find 'Session timed out'").atMost(1, TimeUnit.MINUTES).until(() -> searchForLog(cluster.get(1), false, "Session timed out"));
    ...
    public static class BBStreamTimeoutHelper {
        ...
        public static void install(ClassLoader classLoader, Integer num) {
            if (num != FAILING_NODE)
                return;
            new ByteBuddy().rebase(SequentialWriter.class).method(named("writeDirectlyToChannel").and(takesArguments(1))).intercept(MethodDelegation.to(BBStreamTimeoutHelper.class)).make().load(classLoader, ClassLoadingStrategy.Default.INJECTION);
            new ByteBuddy().rebase(RangeAwareSSTableWriter.class).method(named("append").and(takesArguments(1))).intercept(MethodDelegation.to(BBStreamTimeoutHelper.class)).make().load(classLoader, ClassLoadingStrategy.Default.INJECTION);
        }
    }
}
