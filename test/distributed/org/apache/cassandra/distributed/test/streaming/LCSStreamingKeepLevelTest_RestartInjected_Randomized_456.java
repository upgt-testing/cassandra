...
public class LCSStreamingKeepLevelTest_RestartInjected_Randomized_456 extends TestBaseImpl {

    @Test
    public void testDecom() throws IOException {
        try (Cluster cluster = builder().withNodes(4).withConfig(config -> config.with(NETWORK, GOSSIP, NATIVE_PROTOCOL)).withoutVNodes().withDataDirCount(1).start()) {
            populate(cluster);
            RestartFramework.at("after_populate_decom").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.get(4).nodetoolResult("decommission").asserts().success();
            RestartFramework.at("after_decommission").on(cluster).restart("node").withIndex(3).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testMove() throws IOException {
        try (Cluster cluster = builder().withNodes(4).withConfig(config -> config.with(NETWORK, GOSSIP, NATIVE_PROTOCOL)).withoutVNodes().withDataDirCount(1).start()) {
            populate(cluster);
            RestartFramework.at("after_populate_move").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_move").on(cluster).restart("node").withIndex(3).withMode(RestartMode.GRACEFUL).execute();
            assertEmptyL0(cluster);
        }
    }
    ...
    private static void assertEmptyL0(Cluster cluster) {
        for (IInvokableInstance i : cluster) {
            i.runOnInstance(() -> {
                for (SSTableReader sstable : Keyspace.open(KEYSPACE).getColumnFamilyStore("decom_test").getLiveSSTables()) assertTrue(sstable.getSSTableLevel() > 0);
            });
        }
    }
}
