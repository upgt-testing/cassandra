...
public class StreamFailureLogsFailureInTheMiddleWithEOFTest_RestartInjected extends AbstractStreamFailureLogs
{
    @Test
    public void failureInTheMiddleWithEOF() throws IOException
    {
        streamTestWithRestart(false, "Session peer /127.0.0.1:7012 Failed because there was an java.nio.channels.ClosedChannelException with state=STREAMING", FAILING_NODE);
    }
    ...
    protected void streamTestWithRestart(boolean zeroCopyStreaming, String reason, Integer failedNode) throws IOException
    {
                                      ...
                                      .start())
        {
            ...
            RestartFramework.at("after_failed_repair")
                .on(cluster)
                .restart("node")
                .withIndex(1)
                .withMode(RestartMode.GRACEFUL)
            ...
            RestartFramework.at("after_log_search")
                .on(cluster)
                .restart("node")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
