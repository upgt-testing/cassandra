...
public class StreamCloseInMiddleTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    private void streamClose(boolean zeroCopyStreaming) throws IOException {
        try (Cluster cluster = Cluster.build(2).withTokenSupplier(TokenSupplier.evenlyDistributedTokens(3)).withInstanceInitializer(BBHelper::install).withConfig(c -> c.with(Feature.values()).set("stream_entire_sstables", zeroCopyStreaming).// when die, this will try to halt JVM, which is easier to validate in the test
        // other levels require checking state of the subsystems
        set("disk_failure_policy", "die")).start()) {
            init(cluster);
            ...
            RestartFramework.at("after_trigger_streaming").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // make sure disk failure policy is not triggered
            assertNoNodeShutdown(cluster);
            RestartFramework.at("after_first_shutdown_check").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ...
            node3.startup();
            RestartFramework.at("after_bootstrap_startup").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            for (String line : // bootstrap failed
            Arrays.// bootstrap failed
            asList(// bootstrap failed
            ...
            RestartFramework.at("after_bootstrap_log_verification").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            assertNoNodeShutdown(cluster);
        }
    }
    ...
    public static class BBHelper {
        ...
        public static boolean append(UnfilteredRowIterator partition, @SuperCall Callable<Boolean> zuper) throws Exception {
            if (// handles compressed and non-compressed
            isCaller(CassandraIncomingFile.class.getName(), "read"))
                throw new java.nio.channels.ClosedChannelException();
            // different context; pass through
            return zuper.call();
        }
        ...
        public static void install(ClassLoader classLoader, Integer num) {
            new ByteBuddy().rebase(SequentialWriter.class).method(named("writeDirectlyToChannel").and(takesArguments(1))).intercept(MethodDelegation.to(BBHelper.class)).make().load(classLoader, ClassLoadingStrategy.Default.INJECTION);
            new ByteBuddy().rebase(RangeAwareSSTableWriter.class).method(named("append").and(takesArguments(1))).intercept(MethodDelegation.to(BBHelper.class)).make().load(classLoader, ClassLoadingStrategy.Default.INJECTION);
        }
    }
}
