...
public class ByteBuddyExamplesTest_RestartInjected_Randomized_123 extends TestBaseImpl {

    @Test
    public void writeFailureTest() throws Throwable {
        try (Cluster cluster = init(Cluster.build(1).withInstanceInitializer(BBFailHelper::install).start())) {
            ...
            RestartFramework.at("after_write_attempt").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public static class BBFailHelper {
        ...
        static void install(ClassLoader cl, int nodeNumber) {
            new ByteBuddy().redefine(ModificationStatement.class).method(named("execute")).intercept(MethodDelegation.to(BBFailHelper.class)).make().load(cl, ClassLoadingStrategy.Default.INJECTION);
        }
    ...
    public void countTest() throws IOException {
        try (Cluster cluster = init(Cluster.build(2).withInstanceInitializer(BBCountHelper::install).start())) {
            ...
            RestartFramework.at("after_select_node1").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.coordinator(2).execute("select * from " + KEYSPACE + ".tbl;", ConsistencyLevel.ALL);
            RestartFramework.at("after_select_node2").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
    ...
    public static class BBCountHelper {
        ...
        static void install(ClassLoader cl, int nodeNumber) {
            if (nodeNumber != 1)
                return;
            new ByteBuddy().rebase(SelectStatement.class).method(named("execute").and(takesArguments(3))).intercept(MethodDelegation.to(BBCountHelper.class)).make().load(cl, ClassLoadingStrategy.Default.INJECTION);
        }
