...
public class OptimiseStreamsRepairTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void testBasic() throws Exception {
        try (Cluster cluster = init(Cluster.build(3).withInstanceInitializer(BBHelper::install).withConfig(config -> config.set("hinted_handoff_enabled", false).with(GOSSIP).with(NETWORK)).start())) {
            ...
            RestartFramework.at("after_initial_data_insert").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_divergent_writes").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.get(2).startup();
            RestartFramework.at("after_node2_startup").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            Thread.sleep(1000);
            RestartFramework.at("after_sleep").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.forEach(c -> c.flush(KEYSPACE));
            RestartFramework.at("after_flush_all").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            res.asserts().success();
            RestartFramework.at("after_optimised_repair").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_validation_repair").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public static class BBHelper {
        ...
        public static void install(ClassLoader cl, int id) {
            new ByteBuddy().rebase(RepairJob.class).method(named("createOptimisedSyncingSyncTasks").and(takesArguments(1))).intercept(MethodDelegation.to(BBHelper.class)).make().load(cl, ClassLoadingStrategy.Default.INJECTION);
        }

        public static List<SyncTask> createOptimisedSyncingSyncTasks(List<TreeResponse> trees, @SuperCall Callable<List<SyncTask>> zuperCall) {
    ...
    public void randomTest() throws IOException, TimeoutException {
        try (Cluster cluster = init(Cluster.build(3).withConfig(config -> config.set("hinted_handoff_enabled", false).with(GOSSIP).with(NETWORK)).start())) {
            ...
            RestartFramework.at("after_random_inserts").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            long[] marks = PreviewRepairTest.logMark(cluster);
            NodeToolResult res = cluster.get(1).nodetoolResult("repair", KEYSPACE, "-os");
            res.asserts().success();
            RestartFramework.at("after_optimised_repair").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
