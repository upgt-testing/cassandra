...
public class QueriesTableTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public static void createCluster() throws IOException {
        SHARED_CLUSTER = init(Cluster.build(1).withInstanceInitializer(QueryDelayHelper::install).withConfig(c -> c.with(Feature.NATIVE_PROTOCOL, Feature.GOSSIP)).start());
    ...
    public void shouldExposeReadsAndWrites() throws Throwable {
        ...
        RestartFramework.at("after_table_create").on(SHARED_CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_sync_queries").on(SHARED_CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_async_queries").on(SHARED_CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        while (result.toObjectArrays().length < 4) result = SHARED_CLUSTER.get(1).executeInternalWithResult("SELECT * FROM system_views.queries");
    ...
    public void shouldExposeCAS() throws Throwable {
        ...
        RestartFramework.at("cas_after_async_update").on(SHARED_CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        SimpleQueryResult result = SHARED_CLUSTER.get(1).executeInternalWithResult("SELECT * FROM system_views.queries");
        RestartFramework.at("cas_after_async_select").on(SHARED_CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        while (result.toObjectArrays().length < 2) result = SHARED_CLUSTER.get(1).executeInternalWithResult("SELECT * FROM system_views.queries");
        while (result.hasNext()) {
            Row row = result.next();
            String threadId = row.get("thread_id").toString();
            String task = row.get("task").toString();
        ...
        RestartFramework.at("cas_after_table_create").on(SHARED_CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertTrue(readVisible);
    ...
    public static class QueryDelayHelper {
        ...
        static void install(ClassLoader cl, int nodeNumber) {
            new ByteBuddy().rebase(Mutation.class).method(named("apply").and(takesArguments(3))).intercept(MethodDelegation.to(QueryDelayHelper.class)).make().load(cl, ClassLoadingStrategy.Default.INJECTION);
            new ByteBuddy().rebase(ReadCommand.class).method(named("executeLocally").and(takesArguments(1))).intercept(MethodDelegation.to(QueryDelayHelper.class)).make().load(cl, ClassLoadingStrategy.Default.INJECTION);
        }
        ...
        @SuppressWarnings("unused")
        public static UnfilteredPartitionIterator executeLocally(ReadExecutionController executionController, @SuperCall Callable<UnfilteredPartitionIterator> zuper) {
            try {
