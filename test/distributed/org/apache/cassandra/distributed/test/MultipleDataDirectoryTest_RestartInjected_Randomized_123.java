...
public class MultipleDataDirectoryTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public void populateData() {
        ...
        for (int k = 0; k < files; k++) {
            for (int i = k * rowsPerFile; i < k * rowsPerFile + rowsPerFile; ++i) NODE.executeInternal(withKeyspace("INSERT INTO %s.cf (k, c1, c2) VALUES (?, 'value1', 'value2');"), Integer.toString(i));
            NODE.nodetool("flush");
        }
    }
    ...
    public void testSSTablesAreInCorrectLocation() {
        RestartFramework.at("after_data_population").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        NODE.runOnInstance(() -> {
            ColumnFamilyStore cfs = Keyspace.open(KEYSPACE).getColumnFamilyStore("cf");
            Assert.assertFalse("All SSTables should be in the correct location", cfs.hasMisplacedSSTables());
        });
    }
    ...
    public void testDetectSSTableMisplaced() {
        ...
        RestartFramework.at("after_setup_misplaced_sstables").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        NODE.runOnInstance(() -> {
            ColumnFamilyStore cfs = Keyspace.open(KEYSPACE).getColumnFamilyStore("cf");
            Assert.assertTrue("Some SSTable should be misplaced", cfs.hasMisplacedSSTables());
        });
    }
    ...
    public void testNodeToolRelocateSSTablesFindNoFilesToMove() {
        ...
        RestartFramework.at("after_relocatesstables").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        NODE.nodetoolResult("relocatesstables", KEYSPACE, "cf").asserts().success();
        String expectedLog = String.format("No sstables to RELOCATE for %s.%s", KEYSPACE, "cf");
        RestartFramework.at("before_relocatesstables").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Assert.assertEquals("relocatesstables should find no sstables to move", 1, NODE.logs().grep(logStartLoc, expectedLog).getResult().size());
    }

    @Test
    public void testNodeToolRelocateSSTables() {
        RestartFramework.at("after_setup_misplaced_sstables_for_relocation").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        setupMisplacedSSTables();
        long logStartLoc = NODE.logs().mark();
        RestartFramework.at("after_relocatesstables_with_misplaced").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("before_relocatesstables_with_misplaced").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        NODE.nodetoolResult("relocatesstables", KEYSPACE, "cf").asserts().success();
        String expectedLog = String.format("Finished Relocate sstables to correct disk for %s.%s successfully", KEYSPACE, "cf");
        Assert.assertEquals("relocatesstables should find sstables to move", 1, NODE.logs().grep(logStartLoc, expectedLog).getResult().size());
        NODE.runOnInstance(() -> {
            ColumnFamilyStore cfs = Keyspace.open(KEYSPACE).getColumnFamilyStore("cf");
            Assert.assertFalse("All SSTables should be in the correct location", cfs.hasMisplacedSSTables());
        });
