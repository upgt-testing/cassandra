...
public class PaxosRepair2Test_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public void paxosRepairPreventsStaleReproposal() throws Throwable {
        ...
        try (Cluster cluster = init(Cluster.create(3, cfg -> cfg.set("paxos_variant", "v2").set("paxos_purge_grace_period", "0s").set("truncate_request_timeout_in_ms", 1000L)))) {
            ...
            RestartFramework.at("after_paxos_repair").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // stop and start node 2 to test loading paxos repair history from disk
            cluster.get(2).shutdown().get();
            cluster.get(2).startup();
            RestartFramework.at("after_node_restart").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_stale_proposal_saved").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // shutdown node 3 so we're guaranteed to see the stale proposal
            cluster.get(3).shutdown().get();
            RestartFramework.at("after_node3_shutdown").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_cas_query").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void paxosRepairHistoryIsntUpdatedInForcedRepair() throws Throwable {
        ...
        try (Cluster cluster = init(Cluster.create(3, cfg -> cfg.with(Feature.GOSSIP, Feature.NETWORK).set("paxos_variant", "v2").set("truncate_request_timeout_in_ms", 1000L)))) {
            cluster.schemaChange("CREATE TABLE " + KEYSPACE + '.' + TABLE + " (k int primary key, v int)");
            RestartFramework.at("after_table_create").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ClusterUtils.stopUnchecked(cluster.get(3));
            RestartFramework.at("after_node3_stopped").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            Awaitility.waitAtMost(1, TimeUnit.MINUTES).until(() -> !cluster.get(1).callOnInstance(() -> FailureDetector.instance.isAlive(node3)));
            repair(cluster, KEYSPACE, TABLE, true);
            RestartFramework.at("after_forced_repair").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        try (ReadExecutionController controller = query.executionController();
            ...
            while (partitions.hasNext()) {
                ...
                while (partition.hasNext()) {
                    rows.put(Int32Type.instance.compose(partition.partitionKey().getKey()), new PaxosRow(partition.partitionKey(), partition.next()));
                }
            }
        }
        return rows;
    ...
    public void paxosAutoRepair() throws Throwable {
        try (WithProperties properties = new WithProperties().set(AUTO_REPAIR_FREQUENCY_SECONDS, 1).set(DISABLE_PAXOS_AUTO_REPAIRS, true);
            Cluster cluster = init(Cluster.create(3, cfg -> cfg.set("paxos_variant", "v2").set("paxos_repair_enabled", true).set("truncate_request_timeout_in_ms", 1000L)))) {
            cluster.forEach(i -> {
                Assert.assertFalse(CassandraRelevantProperties.CLOCK_GLOBAL.isPresent());
            ...
            RestartFramework.at("after_paxos_commit_drop").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            try {
            ...
            RestartFramework.at("after_cas_timeout").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_system_flush").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // re-enable repairs
            ...
            RestartFramework.at("after_enable_auto_repairs").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Thread.sleep(2000);
            for (int i = 0; i < 20; i++) {
                if (!cluster.get(1).callsOnInstance(() -> PaxosState.uncommittedTracker().hasInflightAutoRepairs()).call() && !cluster.get(2).callsOnInstance(() -> PaxosState.uncommittedTracker().hasInflightAutoRepairs()).call())
                    break;
    ...
    public void paxosPurgeGraceSeconds() throws Exception {
        int graceSeconds = 5;
        try (Cluster cluster = init(Cluster.create(3, cfg -> cfg.set("paxos_variant", "v2").set("paxos_purge_grace_period", graceSeconds + "s").set("paxos_state_purging", Config.PaxosStatePurging.repaired.toString()).set("truncate_request_timeout_in_ms", 1000L)))) {
            cluster.schemaChange("CREATE TABLE " + KEYSPACE + '.' + TABLE + " (pk int, ck int, v int, PRIMARY KEY (pk, ck))");
            RestartFramework.at("after_table_create").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.coordinator(1).execute("INSERT INTO " + KEYSPACE + '.' + TABLE + " (pk, ck, v) VALUES (1, 1, 1) IF NOT EXISTS", ConsistencyLevel.QUORUM);
            RestartFramework.at("after_cas_insert").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            repair(cluster, KEYSPACE, TABLE);
    ...
    public static class OffsettableClock implements Clock {
        ...
        public long nanoTime() {
            // checkstyle: permit system clock
    ...
    public void legacyPurgeRepairLoop() throws Exception {
        try {
            ...
            try (Cluster cluster = init(Cluster.create(3, cfg -> cfg.set("paxos_variant", "v2").set("paxos_state_purging", "legacy").set("paxos_purge_grace_period", "0s").set("truncate_request_timeout_in_ms", 1000L)))) {
                ...
                RestartFramework.at("after_first_cas_timeout").on(cluster).restart("node").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
                ...
                RestartFramework.at("after_second_cas").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                ...
                cluster.get(1).runOnInstance(() -> {
                    ...
                    try (PaxosState state = PaxosState.get(dk, table)) {
                        state.updateStateUnsafe(s -> {
                            ...
                            Commit.CommittedWithTTL committed = new Commit.CommittedWithTTL(s.committed.ballot, s.committed.update, ballotDeletion(s.committed));
                ...
                RestartFramework.at("after_auto_repair").on(cluster).restart("node").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
                assertUncommitted(cluster.get(1), KEYSPACE, TABLE, 0);
                assertUncommitted(cluster.get(2), KEYSPACE, TABLE, 0);
