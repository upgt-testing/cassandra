...
public class TableMetricTest_RestartInjected extends TestBaseImpl
{
    static
    {
    ...
    public void systemTables() throws IOException
    {
        try (Cluster cluster = Cluster.build(1).start())
        {
            ...
            RestartFramework.at("after_load_system_tables")
                .on(cluster)
                .restart("node")
                .withIndex(0)
    ...
    public void userTables() throws IOException
    {
        try (Cluster cluster = init(Cluster.build(3).start()))
        {
            ...
            RestartFramework.at("after_alter_comment")
                .on(cluster)
                .restart("node")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
            ...
            RestartFramework.at("after_alter_rename")
                .on(cluster)
                .restart("node")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
            ...
            RestartFramework.at("after_alter_drop_column")
                .on(cluster)
                .restart("node")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
