...
public class TableMetricTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void systemTables() throws IOException {
        try (Cluster cluster = Cluster.build(1).start()) {
            ...
            RestartFramework.at("after_load_system_tables").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void userTables() throws IOException {
        try (Cluster cluster = init(Cluster.build(3).start())) {
            ...
            cluster.forEach(i -> assertTableMetricsExist(i, KEYSPACE, "tbl"));
            RestartFramework.at("after_alter_comment").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.schemaChange(withKeyspace("ALTER TABLE %s.tbl ADD (value bigint)"));
            cluster.forEach(i -> assertTableMetricsExist(i, KEYSPACE, "tbl"));
            RestartFramework.at("after_alter_add_column").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.schemaChange(withKeyspace("ALTER TABLE %s.tbl RENAME pk TO pk2"));
            cluster.forEach(i -> assertTableMetricsExist(i, KEYSPACE, "tbl"));
            RestartFramework.at("after_alter_rename").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.schemaChange(withKeyspace("ALTER TABLE %s.tbl DROP value"));
            cluster.forEach(i -> assertTableMetricsExist(i, KEYSPACE, "tbl"));
            RestartFramework.at("after_alter_drop_column").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private static void loadSystemTables(Cluster cluster) {
        SYSTEM_TABLES = cluster.get(1).callOnInstance(() -> {
            ...
            Arrays.asList(SystemKeyspace.metadata(), AuthKeyspace.metadata(), SystemDistributedKeyspace.metadata(), SchemaKeyspace.metadata(), TraceKeyspace.metadata()).forEach(meta -> {
    ...
    private static void assertTableMetricsExist(IInvokableInstance inst, String keyspace, String table) {
        ...
        for (String metric : TABLE_METRIC_NAMES) assertTableMetricExists(inst, keyspace, table, metric);
    }
    ...
    private static void assertTableMetricsDoesNotExist(IInvokableInstance inst, String keyspace, String table) {
        ...
        for (String metric : TABLE_METRIC_NAMES) assertTableMetricDoesNotExists(inst, keyspace, table, metric);
    }

    private static void assertKeyspaceMetricDoesNotExists(IInvokableInstance inst, String keyspace) {
        for (String metric : TABLE_METRIC_NAMES) assertKeyspaceMetricDoesNotExists(inst, keyspace, metric);
    }
    ...
    private static void assertTableMBeanExists(IInvokableInstance inst, String keyspace, String table) {
        inst.runOnInstance(() -> {
            ...
            Assert.assertTrue("Unable to find table mbean for " + keyspace + "." + table, mbeans.isRegistered(ColumnFamilyStore.getTableMBeanName(keyspace, table, false)));
            Assert.assertTrue("Unable to find column family mbean for " + keyspace + "." + table, mbeans.isRegistered(ColumnFamilyStore.getColumnFamilieMBeanName(keyspace, table, false)));
        });
    }
    ...
    private static void assertTableMBeanDoesNotExists(IInvokableInstance inst, String keyspace, String table) {
        inst.runOnInstance(() -> {
            // cast only to make sure it linked properly
            MapMBeanWrapper mbeans = getMapMBeanWrapper();
            Assert.assertFalse("Found table mbean for " + keyspace + "." + table, mbeans.isRegistered(ColumnFamilyStore.getTableMBeanName(keyspace, table, false)));
            Assert.assertFalse("Found column family mbean for " + keyspace + "." + table, mbeans.isRegistered(ColumnFamilyStore.getColumnFamilieMBeanName(keyspace, table, false)));
        });
    }
    ...
    private static void assertKeyspaceMetricMayExists(MapMBeanWrapper mbeans, String keyspace, String name) {
        ...
        String errorMessage = keyspaceExists ? "Unable to find keyspace metric " + keyspaceMBean + " for " + keyspace : "Found keyspace metric " + keyspaceMBean + " for " + keyspace;
