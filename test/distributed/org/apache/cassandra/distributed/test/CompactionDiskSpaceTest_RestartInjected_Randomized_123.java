...
public class CompactionDiskSpaceTest_RestartInjected_Randomized_123 extends TestBaseImpl {

    @Test
    public void testNoSpaceLeft() throws IOException {
        try (Cluster cluster = init(Cluster.build(1).withConfig(config -> config.set("min_free_space_per_drive_in_mb", "0")).withDataDirCount(3).withInstanceInitializer(BB::install).start())) {
            ...
            RestartFramework.at("after_compaction_tests").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public static class BB {
        ...
        public static void install(ClassLoader cl, Integer node) {
            new ByteBuddy().rebase(ActiveCompactions.class).method(named("estimatedRemainingWriteBytes")).intercept(MethodDelegation.to(BB.class)).make().load(cl, ClassLoadingStrategy.Default.INJECTION);
            new ByteBuddy().rebase(FileStoreUtils.class).method(named("tryGetSpace")).intercept(MethodDelegation.to(BB.class)).make().load(cl, ClassLoadingStrategy.Default.INJECTION);
        }
