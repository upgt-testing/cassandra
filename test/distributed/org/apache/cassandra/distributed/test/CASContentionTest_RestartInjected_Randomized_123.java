...
public class CASContentionTest_RestartInjected_Randomized_123 extends CASTestBase {
    ...
    public static void beforeClass() throws Throwable {
        ...
        Consumer<IInstanceConfig> conf = config -> config.set("paxos_variant", "v2").set("write_request_timeout_in_ms", 20000L).set("cas_contention_timeout_in_ms", 20000L).set("request_timeout_in_ms", 20000L);
    ...
    public void testDynamicContentionTracing() throws Throwable {
        try {
            ...
            RestartFramework.at("after_filter_setup").on(THREE_NODES).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            THREE_NODES.get(1).runOnInstance(() -> ContentionStrategy.setStrategy("trace=1"));
            RestartFramework.at("after_contention_strategy_setup").on(THREE_NODES).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            haveStarted.await();
            RestartFramework.at("after_first_insert_started").on(THREE_NODES).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            THREE_NODES.coordinator(2).execute("INSERT INTO " + KEYSPACE + '.' + tableName + " (pk, v) VALUES (1, 1) IF NOT EXISTS", QUORUM);
            RestartFramework.at("after_second_insert").on(THREE_NODES).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            haveInvalidated.countDown();
            THREE_NODES.filters().reset();
            RestartFramework.at("after_filter_reset").on(THREE_NODES).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            insert.get();
            RestartFramework.at("after_first_insert_complete").on(THREE_NODES).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Uninterruptibles.sleepUninterruptibly(1L, TimeUnit.SECONDS);
            THREE_NODES.forEach(i -> i.runOnInstance(() -> FBUtilities.waitOnFuture(Stage.TRACING.submit(() -> {
            }))));
            RestartFramework.at("after_tracing_complete").on(THREE_NODES).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Object[][] result = THREE_NODES.coordinator(1).execute("SELECT parameters FROM system_traces.sessions", QUORUM);
            RestartFramework.at("after_trace_query").on(THREE_NODES).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
