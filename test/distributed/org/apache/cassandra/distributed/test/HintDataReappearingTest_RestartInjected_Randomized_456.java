...
public class HintDataReappearingTest_RestartInjected_Randomized_456 extends AbstractHintWindowTest {

    private static final Logger logger = LoggerFactory.getLogger(HintDataReappearingTest_RestartInjected.class);

    private static final ScheduledExecutorService scheduler = Executors.newScheduledThreadPool(1, new ThreadFactoryBuilder().setNameFormat("hint reappearance test").setDaemon(true).build());
    ...
    public void doHintReappearData(final boolean preventReappearance, final boolean dropTwoWrites) throws Exception {
        try (Cluster cluster = init(Cluster.build(3).withDataDirCount(1).withConfig(config -> config.with(NETWORK, GOSSIP, NATIVE_PROTOCOL).set("hinted_handoff_enabled", true).set("max_hints_delivery_threads", "1").set("use_creation_time_for_hint_ttl", preventReappearance ? "true" : "false").set("write_request_timeout", "30000ms").set("hints_flush_period", "1s").set("max_hints_file_size", "10MiB")).start(), 3)) {
            ...
            final String createTableStatement = format("CREATE TABLE %s.cf (k text PRIMARY KEY, c1 text) " + "WITH gc_grace_seconds = " + gc_grace_seconds + " AND compaction = {'class': 'SizeTieredCompactionStrategy', 'min_threshold': 2, 'max_threshold': 32 } ", KEYSPACE);
            ...
            RestartFramework.at("after_pause_hints").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            logger.info("Inserting data");
            List<UUID> keys = IntStream.range(0, 1).mapToObj(x -> UUID.randomUUID()).collect(Collectors.toList());
            scheduler.submit(() -> {
                if (dropTwoWrites) {
                    Assertions.assertThatThrownBy(() -> insertData(cluster, keys)).is(isThrowable(WriteTimeoutException.class));
                } else {
            ...
            RestartFramework.at("after_insert_and_filter_reset").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            node1.flush(KEYSPACE);
            node2.flush(KEYSPACE);
            node3.flush(KEYSPACE);
            RestartFramework.at("after_first_flush").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ...
            node2.flush(KEYSPACE);
            node3.flush(KEYSPACE);
            RestartFramework.at("after_second_flush").on(cluster).restart("node").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_first_verification").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_compaction").on(cluster).restart("node").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_hint_transfer").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("before_final_verification").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
    ...
    private List<SimpleQueryResult> selectData(Cluster cluster, List<UUID> keys) {
        ...
        for (UUID partitionKey : keys) {
            SimpleQueryResult result = cluster.coordinator(1).executeWithResult(withKeyspace("SELECT * FROM %s.cf where k=?;"), ALL, partitionKey.toString());
            results.add(result);
        }
        return results;
    }
    ...
    private void insertData(Cluster cluster, List<UUID> inserts) {
        for (int i = 0; i < inserts.size(); i++) {
            ...
            cluster.coordinator(1).execute(withKeyspace("INSERT INTO %s.cf (k, c1) VALUES (?, ?);"), LOCAL_QUORUM, partitionKey.toString(), UUID.randomUUID().toString());
        }
    }

    void deleteData(Cluster cluster, List<UUID> insertedKeys) {
        for (UUID partitionKey : insertedKeys) {
            cluster.coordinator(1).execute(withKeyspace("DELETE c1 FROM %s.cf where k=?;"), LOCAL_QUORUM, partitionKey.toString());
        }
    }
}
