...
public class PartialWritesWithRepairTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public void test() throws IOException {
        try (Cluster cluster = Cluster.build(2).withConfig(c -> c.with(Feature.values())).start()) {
            ...
            RestartFramework.at("after_complete_range_writes").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_partial_range_writes").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
