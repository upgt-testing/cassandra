...
public class ConcurrencyFactorTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void init() throws IOException {
        cluster = init(Cluster.build(NODES).withTokenSupplier(generateTokenSupplier()).withTokenCount(1).withConfig(config -> config.with(GOSSIP).with(NETWORK)).start());
    }
    ...
    public void testInitialConcurrencySelection() {
        cluster.schemaChange(String.format("CREATE TABLE %s.%s (pk int, state ascii, gdp bigint, PRIMARY KEY (pk)) WITH compaction = " + " {'class' : 'SizeTieredCompactionStrategy', 'enabled' : false }", KEYSPACE, SAI_TABLE));
        ...
        RestartFramework.at("after_second_query").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_third_query").on(cluster).restart("node").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_data_insertion").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        runAndValidate("Submitting range requests on 3 ranges with a concurrency of 3", query, 3_000_000_000L, 7_000_000_000L);
        RestartFramework.at("after_first_query").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_fourth_query").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private static TokenSupplier generateTokenSupplier() {
        List<List<String>> tokens = Arrays.asList(Collections.singletonList("-9223372036854775808"), Collections.singletonList("-3074457345618258602"), Collections.singletonList("3074457345618258603"));
