...
public class IndexStreamingFailureTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public void testAvailabilityAfterFailedNonEntireFileStreaming() throws Exception {
        try (Cluster cluster = init(Cluster.build(2).withConfig(c -> c.with(NETWORK, GOSSIP)).withInstanceInitializer((classLoader, threadGroup, num, generation) -> {
    ...
    private void testAvailabilityAfterStreaming(Cluster cluster, String table, boolean streamEntireSSTables) {
        ...
        assertThat(rs.length).isEqualTo(0);
        RestartFramework.at("after_insert_and_flush").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        rs = second.executeInternal(String.format("SELECT pk FROM %s.%s WHERE v = ?", KEYSPACE, table), "v1");
        RestartFramework.at("after_successful_repair").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
    ...
    public static class ByteBuddyHelper {
        ...
        static void installFlushError(ClassLoader loader) {
            new ByteBuddy().redefine(SegmentBuilder.class).method(named("flush")).intercept(MethodDelegation.to(ByteBuddyHelper.class)).make().load(loader, ClassLoadingStrategy.Default.INJECTION);
        }

        static void installValidateChecksumError(ClassLoader loader) {
            new ByteBuddy().redefine(SAICodecUtils.class).method(named("validateChecksum")).intercept(MethodDelegation.to(ByteBuddyHelper.class)).make().load(loader, ClassLoadingStrategy.Default.INJECTION);
        }
