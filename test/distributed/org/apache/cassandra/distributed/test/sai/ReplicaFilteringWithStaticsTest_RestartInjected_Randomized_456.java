...
public class ReplicaFilteringWithStaticsTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void testRowFilterDeletePurging(boolean sai) {
        ...
        CLUSTER.schemaChange(withKeyspace("CREATE TABLE %s." + table + " (pk0 double, ck0 boolean, s0 ascii static, v0 ascii, " + "PRIMARY KEY (pk0, ck0)) WITH CLUSTERING ORDER BY (ck0 DESC) AND read_repair = 'NONE'"));
        ...
        if (sai) {
            ...
            RestartFramework.at("after_sai_index_creation").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
        ...
        RestartFramework.at("after_initial_update").on(CLUSTER).restart("node").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testStaticMatchWithPartitionDelete(boolean sai) {
        ...
        CLUSTER.schemaChange(withKeyspace("CREATE TABLE %s." + table + " (pk0 boolean, ck0 ascii, s1 tinyint static, v0 boolean, PRIMARY KEY (pk0, ck0)) " + "WITH CLUSTERING ORDER BY (ck0 ASC) AND read_repair = 'NONE'"));
        RestartFramework.at("after_table_create_static").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        if (sai) {
            CLUSTER.schemaChange(withKeyspace("CREATE INDEX ON %s." + table + "(s1) USING 'sai'"));
            SAIUtil.waitForIndexQueryable(CLUSTER, KEYSPACE);
            RestartFramework.at("after_sai_index_creation_static").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
        ...
        RestartFramework.at("after_partition_delete").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_multiple_inserts").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_static_update").on(CLUSTER).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testMissingStaticRowWithNonStaticExpression(boolean sai) {
        ...
        CLUSTER.schemaChange(withKeyspace("CREATE TABLE %s." + table + " (pk0 int, ck0 int, ck1 int, s0 int static, s1 int static, v0 int, PRIMARY KEY (pk0, ck0, ck1)) " + "WITH CLUSTERING ORDER BY (ck0 ASC, ck1 DESC) AND read_repair = 'NONE'"));
        RestartFramework.at("after_table_create_expression").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        if (sai) {
            CLUSTER.schemaChange(withKeyspace("CREATE INDEX ON %s." + table + "(ck1) USING 'sai'"));
            SAIUtil.waitForIndexQueryable(CLUSTER, KEYSPACE);
            RestartFramework.at("after_sai_index_creation_expression").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
        CLUSTER.get(1).executeInternal(withKeyspace("INSERT INTO %s." + table + " (pk0, ck0, ck1, s0, s1, v0) " + "VALUES (0, 1, 2, 3, 4, 5) USING TIMESTAMP 1"));
        RestartFramework.at("after_initial_insert").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        CLUSTER.get(2).executeInternal(withKeyspace("UPDATE %s." + table + "  USING TIMESTAMP 2 SET s0 = 6, s1 = 7, v0 = 8 " + "WHERE  pk0 = 0 AND ck0 = 9 AND ck1 = 10"));
        RestartFramework.at("after_static_column_update").on(CLUSTER).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
