 ...
 */
public class StrictFilteringTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void shouldDegradeToUnionOnSingleStatic() {
        CLUSTER.schemaChange(withKeyspace("CREATE TABLE %s.single_static (pk0 int, ck0 int, ck1 int, s0 int static, v0 int, PRIMARY KEY (pk0, ck0, ck1)) " + "WITH read_repair = 'NONE' AND CLUSTERING ORDER BY (ck0 ASC, ck1 DESC)"));
        ...
        RestartFramework.at("after_insert").on(CLUSTER).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void shouldRejectNonStrictIN() {
        ...
        Assertions.assertThatThrownBy(() -> CLUSTER.coordinator(1).execute(select, ConsistencyLevel.ALL)).hasMessageContaining(String.format(StorageAttachedIndexQueryPlan.UNSUPPORTED_NON_STRICT_OPERATOR, Operator.IN));
    ...
    public void testPartialUpdatesOnNonIndexedColumnsAfterRepair() {
        RestartFramework.at("after_split_insert").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testPartialUpdatesWithDeleteBetween() {
        ...
        RestartFramework.at("after_index_create").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_second_insert").on(CLUSTER).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        SAIUtil.waitForIndexQueryable(CLUSTER, KEYSPACE);
        RestartFramework.at("after_insert").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testShortReadWithRegularColumns() {
        ...
        RestartFramework.at("after_first_insert").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_second_insert").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        SAIUtil.waitForIndexQueryable(CLUSTER, KEYSPACE);
        // insert a split row
    ...
    public void testNecessaryShortRead() {
        ...
        RestartFramework.at("after_third_insert").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
