...
public class CleanupFailureTest_RestartInjected_Randomized_123 extends TestBaseImpl {

    @Test
    public void cleanupDuringDecommissionTest() throws Throwable {
        try (Cluster cluster = init(builder().withNodes(2).withTokenSupplier(evenlyDistributedTokens(2)).withNodeIdTopology(NetworkTopology.singleDcNetworkTopology(2, "dc0", "rack0")).withConfig(config -> config.with(NETWORK, GOSSIP)).start(), 1)) {
            ...
            RestartFramework.at("after_status_to_decommission").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Add data to cluster while node is decomissioning
            int numRows = 100;
            cluster.schemaChange("CREATE TABLE IF NOT EXISTS " + KEYSPACE + ".tbl (pk int, ck int, v int, PRIMARY KEY (pk, ck))");
            RestartFramework.at("after_schema_change").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_cleanup_failure").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void cleanupDuringBootstrapTest() throws Throwable {
        ...
        try (Cluster cluster = init(builder().withNodes(originalNodeCount).withTokenSupplier(evenlyDistributedTokens(expandedNodeCount)).withNodeIdTopology(NetworkTopology.singleDcNetworkTopology(expandedNodeCount, "dc0", "rack0")).withConfig(config -> config.with(NETWORK, GOSSIP)).start(), 2)) {
            RestartFramework.at("after_cluster_setup").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            withProperty(JOIN_RING, false, () -> bootstrappingNode.startup(cluster));
            ...
            result.asserts().stderrContains("Node is involved in cluster membership changes. Not safe to run cleanup.");
            RestartFramework.at("after_cleanup_test").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
