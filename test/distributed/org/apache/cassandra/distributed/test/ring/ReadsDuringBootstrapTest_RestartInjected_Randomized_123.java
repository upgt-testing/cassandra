...
public class ReadsDuringBootstrapTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public void readsDuringBootstrapTest() throws IOException, ExecutionException, InterruptedException, TimeoutException {
        ...
        try (Cluster cluster = builder().withNodes(originalNodeCount).withTokenSupplier(TokenSupplier.evenlyDistributedTokens(expandedNodeCount)).withNodeIdTopology(NetworkTopology.singleDcNetworkTopology(expandedNodeCount, "dc0", "rack0")).withConfig(config -> config.with(NETWORK, GOSSIP).set("read_request_timeout", String.format("%dms", Integer.MAX_VALUE)).set("native_transport_timeout", String.format("%dms", Integer.MAX_VALUE)).set("request_timeout", String.format("%dms", Integer.MAX_VALUE))).withInstanceInitializer(BB::install).start()) {
            ...
            RestartFramework.at("after_read_block_setup").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            Future<?> read = es.submit(() -> cluster.coordinator(1).execute(query, ConsistencyLevel.QUORUM, 3));
            RestartFramework.at("after_read_submission").on(cluster).restart("node").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_new_node_log_watch").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            cluster.get(1).runOnInstance(() -> BB.block.set(false));
            RestartFramework.at("after_unblock_reads").on(cluster).restart("node").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
            // populate cache
            for (int i = 0; i < 10; i++) cluster.coordinator(1).execute(query, ConsistencyLevel.QUORUM, i);
            RestartFramework.at("after_cache_population").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public static class BB {
        ...
        private static void install(ClassLoader cl, Integer instanceId) {
            if (instanceId != 1)
                return;
            new ByteBuddy().rebase(AbstractReplicationStrategy.class).method(named("getCachedReplicas")).intercept(MethodDelegation.to(BB.class)).make().load(cl, ClassLoadingStrategy.Default.INJECTION);
        }

        public static EndpointsForRange getCachedReplicas(long ringVersion, Token t, @FieldValue("keyspaceName") String keyspaceName, @SuperCall Callable<EndpointsForRange> zuper) throws Exception {
