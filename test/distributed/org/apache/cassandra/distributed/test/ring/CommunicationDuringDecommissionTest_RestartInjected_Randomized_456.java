...
public class CommunicationDuringDecommissionTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void internodeConnectionsDuringDecom() throws Throwable {
        try (Cluster cluster = builder().withNodes(4).withConfig(config -> config.with(NETWORK, GOSSIP, NATIVE_PROTOCOL)).start()) {
            ...
            RestartFramework.at("after_decommission").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
