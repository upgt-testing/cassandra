...
public class BootstrapTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public void bootstrapUnspecifiedFailsOnResumeTest() throws Throwable {
        ...
        try (Cluster cluster = builder().withNodes(originalNodeCount).withoutVNodes().withTokenSupplier(TokenSupplier.evenlyDistributedTokens(expandedNodeCount)).withNodeIdTopology(NetworkTopology.singleDcNetworkTopology(expandedNodeCount, "dc0", "rack0")).withConfig(config -> config.with(NETWORK, GOSSIP)).start()) {
            ...
            RestartFramework.at("after_bootstrap_config").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            withProperty(JOIN_RING, false, () -> newInstance.startup(cluster));
            RestartFramework.at("after_new_node_startup").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            String cql = String.format("INSERT INTO %s.%s (keyspace_name, full_ranges, transient_ranges) VALUES (?, ?, ?)", SchemaConstants.SYSTEM_KEYSPACE_NAME, SystemKeyspace.AVAILABLE_RANGES_V2);
            newInstance.executeInternal(cql, KEYSPACE, fullSet.stream().map(SystemKeyspace::rangeToBytes).collect(Collectors.toSet()), partialSet.stream().map(SystemKeyspace::rangeToBytes).collect(Collectors.toSet()));
            RestartFramework.at("after_insert_available_ranges").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        Assert.assertTrue("Expected to see a RuntimeException w/'Discovered existing bootstrap data' in the error message; did not.", sawException);
    }
    ...
    private void bootstrapTest() throws Throwable {
        ...
        try (Cluster cluster = builder().withNodes(originalNodeCount).withTokenSupplier(TokenSupplier.evenlyDistributedTokens(expandedNodeCount)).withNodeIdTopology(NetworkTopology.singleDcNetworkTopology(expandedNodeCount, "dc0", "rack0")).withConfig(config -> config.with(NETWORK, GOSSIP)).start()) {
            ...
            RestartFramework.at("after_status_to_bootstrap").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.run(asList(pullSchemaFrom(cluster.get(1)), bootstrap()), newInstance.config().num());
            RestartFramework.at("after_bootstrap_complete").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void bootstrapJMXStatus() throws Throwable {
        ...
        try (Cluster cluster = builder().withNodes(originalNodeCount).withTokenSupplier(TokenSupplier.evenlyDistributedTokens(expandedNodeCount)).withNodeIdTopology(NetworkTopology.singleDcNetworkTopology(expandedNodeCount, "dc0", "rack0")).withConfig(config -> config.with(NETWORK, GOSSIP)).withInstanceInitializer(BootstrapTest.BB::install).start()) {
            ...
            RestartFramework.at("after_verify_in_progress").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            joiningInstance.nodetoolResult("bootstrap", "resume").asserts().success();
            RestartFramework.at("after_resume_bootstrap").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            joiningInstance.runOnInstance(() -> {
    ...
    public static void populate(ICluster cluster, int from, int to, int coord, int rf, ConsistencyLevel cl) {
        ...
        for (int i = from; i < to; i++) {
            cluster.coordinator(coord).execute("INSERT INTO " + KEYSPACE + ".tbl (pk, ck, v) VALUES (?, ?, ?)", cl, i, i, i);
        }
    }

    public static Map<Integer, Long> count(ICluster cluster) {
        return IntStream.rangeClosed(1, cluster.size()).boxed().collect(Collectors.toMap(nodeId -> nodeId, nodeId -> (Long) cluster.get(nodeId).executeInternal("SELECT count(*) FROM " + KEYSPACE + ".tbl")[0][0]));
    }
    ...
    public static class BB {
        ...
        public static void install(ClassLoader classLoader, Integer num) {
            ...
            new ByteBuddy().rebase(StorageService.class).method(named("bootstrapFinished")).intercept(MethodDelegation.to(DecommissionTest.BB.class)).make().load(classLoader, ClassLoadingStrategy.Default.INJECTION);
        }
    ...
    public void testStorageServiceMBeanIsPublishedOnJMXDuringBootstrap() throws Throwable {
        ...
        try (Cluster cluster = builder().withNodes(2).withConfig(config -> config.with(GOSSIP).with(NETWORK).with(Feature.JMX).set("auto_bootstrap", true)).withInstanceInitializer(BBBootstrapInterceptor::install).createWithoutStarting();
