...
public class NodeNotInRingTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public void nodeNotInRingTest() throws Throwable {
        try (Cluster cluster = builder().withNodes(3).withConfig(config -> config.with(NETWORK, GOSSIP)).start()) {
            ...
            RestartFramework.at("after_gossip_filter").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            cluster.run(GossipHelper.removeFromRing(cluster.get(3)), 1, 2);
            RestartFramework.at("after_node_removal").on(cluster).restart("node").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_ring_verification").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            populate(cluster, 0, 50, 1, ConsistencyLevel.ALL);
            RestartFramework.at("after_first_populate").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
