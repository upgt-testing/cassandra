...
public class ReadFailureTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void testSpecExecRace() throws Throwable {
        try (Cluster cluster = init(Cluster.build().withNodes(2).withConfig(config -> config.set("tombstone_failure_threshold", TOMBSTONE_FAIL_THRESHOLD)).start())) {
            ...
            RestartFramework.at("after_tombstone_creation").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Create a bunch of latency samples for this failed operation.
            loopFailStatement(cluster, 5000);
            RestartFramework.at("after_first_loop").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_speculation_update").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void loopFailStatement(ICluster cluster, int iterations) {
        ...
        for (int i = 0; i < iterations; ++i) {
            ...
            } catch (// Throwable because the raised ReadFailure is loaded from a different classloader and doesn't match "ours"
            Throwable t) {
                String onFail = String.format("Did not receive expected ReadFailureException. Instead caught %s\n%s", t, ExceptionUtils.getStackTrace(t));
