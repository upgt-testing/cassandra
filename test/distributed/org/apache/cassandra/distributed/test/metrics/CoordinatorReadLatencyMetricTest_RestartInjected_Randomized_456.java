...
public class CoordinatorReadLatencyMetricTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void internalPagingWithAggregateTest() throws Throwable {
        try (Cluster cluster = init(builder().withNodes(1).start())) {
            ...
            for (int i = 0; i < 100; i++) cluster.coordinator(1).execute(withKeyspace("insert into %s.tbl (pk, ck ,v) values (0, ?, 1)"), ConsistencyLevel.ALL, i);
            ...
            RestartFramework.at("after_aggregation_all").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.get(1).runOnInstance(() -> Paxos.setPaxosVariant(Config.PaxosVariant.v1));
            RestartFramework.at("after_paxos_v1_config").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            testAggregationQuery(cluster, ConsistencyLevel.SERIAL);
            RestartFramework.at("after_serial_query_v1").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.get(1).runOnInstance(() -> Paxos.setPaxosVariant(Config.PaxosVariant.v2));
            RestartFramework.at("after_paxos_v2_config").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            testAggregationQuery(cluster, ConsistencyLevel.SERIAL);
        }
    }
    ...
    public void multiplePartitionKeyInClauseTest() throws Throwable {
        try (Cluster cluster = init(builder().withNodes(1).start())) {
            ...
            RestartFramework.at("after_table_create").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            for (int i = 0; i < 100; i++) cluster.coordinator(1).execute(withKeyspace("insert into %s.tbl (pk, v) values (?, 1)"), ConsistencyLevel.ALL, i);
            RestartFramework.at("after_inserts").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            for (int partitionKeys : new int[] { 1, 100 }) {
                ...
                String pkList = IntStream.range(0, partitionKeys).mapToObj(Integer::toString).collect(Collectors.joining(",", "(", ")"));
                ...
                RestartFramework.at("after_in_clause_query_" + partitionKeys + "_keys").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
        }
    }

    private void verifyLatencyMetricsWhenPaging(Cluster cluster, int pagesize, int expectedQueries, String query, ConsistencyLevel consistencyLevel) {
        ...
        assertTrue(String.format("Total latency delta %s should not exceed wall clock time elapsed %s", totalLatencyRecorded, elapsedTime), totalLatencyRecorded <= elapsedTime);
    }
}
