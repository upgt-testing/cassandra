 ...
 */
public class HintsServiceMetricsTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void testHintsServiceMetrics() throws Exception {
        ...
        try (Cluster cluster = builder().withNodes(3).withConfig(config -> config.with(NETWORK, GOSSIP, NATIVE_PROTOCOL)).withInstanceInitializer(FailHints::install).start()) {
            ...
            RestartFramework.at("after_first_half_writes").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_second_half_writes").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_hints_applied").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_coordinator_metrics_verified").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_delay_metrics_verified").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private static void waitUntilAsserted(ThrowingRunnable assertion) {
        await().atMost(5, MINUTES).pollDelay(0, SECONDS).pollInterval(1, SECONDS).dontCatchUncaughtExceptions().untilAsserted(assertion);
    }
    ...
    private static Long countEndpointDelays(IInvokableInstance node, IInvokableInstance target) {
        return getHistogramCount(node, String.format("org.apache.cassandra.metrics.HintsService.Hint_delays-%s.%d", target.broadcastAddress().getAddress(), target.broadcastAddress().getPort()));
    }

    private static long getHistogramCount(IInvokableInstance node, String name) {
        return node.metrics().getHistograms(s -> s.equals(name), Metrics.MetricValue.COUNT).values().stream().findFirst().map(Math::round).orElse(0L);
    }
    ...
    public static class FailHints {
        ...
        private static void install(ClassLoader cl, int nodeNumber) {
            ...
            if (nodeNumber == 1)
                return;
            new ByteBuddy().rebase(Hint.class).method(named("applyFuture").and(takesArguments(0))).intercept(MethodDelegation.to(FailHints.class)).make().load(cl, ClassLoadingStrategy.Default.INJECTION);
        }
