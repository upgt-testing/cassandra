...
public class CoordinatorReadLatencyMetricTest_RestartInjected extends TestBaseImpl
{
    @Test
    public void internalPagingWithAggregateTest() throws Throwable
    {
        try (Cluster cluster = init(builder().withNodes(1).start()))
        {
            ...
            RestartFramework.at("after_aggregation_all")
                .on(cluster)
                .restart("node")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
            ...
            RestartFramework.at("after_paxos_v1_config")
                .on(cluster)
                .restart("node")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
            ...
            RestartFramework.at("after_serial_query_v1")
                .on(cluster)
                .restart("node")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
            ...
            RestartFramework.at("after_paxos_v2_config")
                .on(cluster)
                .restart("node")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
    ...
    public void multiplePartitionKeyInClauseTest() throws Throwable
    {
        try (Cluster cluster = init(builder().withNodes(1).start()))
        {
            ...
            for (int partitionKeys : new int[] {1, 100})
            {
                ...
                RestartFramework.at("after_in_clause_query_" + partitionKeys + "_keys")
                    .on(cluster)
                    .restart("node")
                    .withIndex(0)
                    .withMode(RestartMode.GRACEFUL)
