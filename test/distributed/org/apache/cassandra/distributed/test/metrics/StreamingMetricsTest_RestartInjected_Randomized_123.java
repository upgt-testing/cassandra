...
public class StreamingMetricsTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    private static InetAddressAndPort getNodeAddress(Cluster cluster, int num) {
        ...
        return InetAddressAndPort.getByAddressOverrideDefaults(broadcastAddress.getAddress(), broadcastAddress.getPort());
    }
    ...
    public void testMetricsWithStreamingFromTwoNodes(boolean useRepair) throws Exception {
        try (Cluster cluster = init(Cluster.build(3).withDataDirCount(1).withConfig(config -> config.with(NETWORK).set("stream_entire_sstables", false).set("hinted_handoff_enabled", false)).start(), 2)) {
            ...
            RestartFramework.at("after_data_insertion").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            drop1to3.off();
            ...
            RestartFramework.at("before_repair_or_rebuild").on(cluster).restart("node").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
            // Trigger streaming from node 3
            if (useRepair)
                cluster.get(3).nodetool("repair", "--full");
            else
                cluster.get(3).nodetool("rebuild", "--keyspace", KEYSPACE);
            RestartFramework.at("after_repair_or_rebuild").on(cluster).restart("node").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testMetricsWithStreamingToTwoNodes(boolean useRepair) throws Exception {
        try (Cluster cluster = init(Cluster.build(3).withDataDirCount(1).withConfig(config -> config.with(NETWORK, GOSSIP).set("stream_entire_sstables", false).set("hinted_handoff_enabled", false)).start(), 2)) {
            ...
            cluster.schemaChange(String.format("CREATE TABLE %s.cf (k text, c1, c2 text, PRIMARY KEY (k)) WITH compaction = {'class': '%s', 'enabled': 'false'}", KEYSPACE, "LeveledCompactionStrategy"));
            RestartFramework.at("after_table_create_to_two").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            final int rowsPerFile = 500;
            final int files = 5;
            ...
            RestartFramework.at("after_autocompaction_disabled").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            IMessageFilters.Filter drop1to3 = cluster.filters().verbs(MUTATION_REQ.id).from(1).to(3).drop();
            ...
            RestartFramework.at("after_first_batch_inserts").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            drop1to3.off();
            ...
            RestartFramework.at("after_second_batch_inserts").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            drop1to2.off();
            checkThatNoStreamingOccuredBetweenTheThreeNodes(cluster);
            ...
            RestartFramework.at("before_repair_or_rebuild_to_two").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            if (useRepair) {
                cluster.get(3).nodetool("repair", "--full");
            ...
            RestartFramework.at("after_repair_or_rebuild_to_two").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            // Check streaming metrics on node 1
    ...
    private void checkTotalDataSent(Cluster cluster, int node, long outgoingBytes, long outgoingRepairBytes, long outgoingRepairSSTables) {
        cluster.get(node).runOnInstance(() -> {
            long actual = StreamingMetrics.totalOutgoingBytes.getCount();
            assertThat(actual).describedAs("The total amount of data sent by the node" + node + " is not the expected one. [expected: " + outgoingBytes + ", actual: " + actual + "]").isEqualTo(outgoingBytes);
            actual = StreamingMetrics.totalOutgoingRepairBytes.getCount();
            assertThat(actual).describedAs("The total amount of data sent by the node" + node + " for repair is not the expected one. [expected: " + outgoingRepairBytes + ", actual: " + actual + "]").isEqualTo(outgoingRepairBytes);
            actual = StreamingMetrics.totalOutgoingRepairSSTables.getCount();
            assertThat(actual).describedAs("The total amount of SSTables sent by the node" + node + " for repair is not the expected one. [expected: " + outgoingRepairSSTables + ", actual: " + actual + "]").isEqualTo(outgoingRepairSSTables);
        });
    }
    ...
    private void checkTotalDataReceived(Cluster cluster, int node, long incomingBytes) {
        cluster.get(node).runOnInstance(() -> {
            ...
            assertThat(actual).describedAs("The total amount of data received by the node" + node + " is not the expected one. [expected: " + incomingBytes + ", actual: " + actual + "]").isEqualTo(incomingBytes);
        });
    }
}
