...
public class ToggleCDCOnRepairEnabledTest_RestartInjected_Randomized_456 extends TestBaseImpl {

    @Test
    public void testCDCOnRepairIsEnabled() throws Exception {
        testCDCOnRepairEnabled(true, cluster -> {
            cluster.get(2).runOnInstance(() -> {
                boolean containCDCInLog = CommitLog.instance.segmentManager.getActiveSegments().stream().anyMatch(s -> s.getCDCState() == CommitLogSegment.CDCState.CONTAINS);
                assertTrue("Mutation should be added to commit log when cdc_on_repair_enabled is true", containCDCInLog);
            });
        });
    }
    ...
    public void testCDCOnRepairIsDisabled() throws Exception {
        testCDCOnRepairEnabled(false, cluster -> {
            cluster.get(2).runOnInstance(() -> {
                boolean containCDCInLog = CommitLog.instance.segmentManager.getActiveSegments().stream().allMatch(s -> s.getCDCState() != CommitLogSegment.CDCState.CONTAINS);
                assertTrue("No mutation should be added to commit log when cdc_on_repair_enabled is false", containCDCInLog);
            });
        });
    }
    ...
    private void testCDCOnRepairEnabled(boolean enabled, Consumer<Cluster> assertion) throws Exception {
        try (Cluster cluster = init(Cluster.build(2).withConfig(c -> c.set("cdc_enabled", true).set("cdc_on_repair_enabled", enabled).with(Feature.NETWORK).with(Feature.GOSSIP)).start())) {
