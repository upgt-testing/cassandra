...
public class PartitionDenylistTest_RestartInjected_Randomized_456 extends TestBaseImpl {

    private static final Logger logger = LoggerFactory.getLogger(PartitionDenylistTest_RestartInjected.class);
    ...
    public void checkStartupWithoutTriggeringUnavailable() throws IOException, InterruptedException, ExecutionException, TimeoutException {
        ...
        try (Cluster cluster = Cluster.build(nodeCount).withConfig(config -> config.with(NETWORK).with(GOSSIP).set("partition_denylist_enabled", true).set("denylist_initial_load_retry", "1s")).createWithoutStarting()) {
            cluster.forEach(i -> {
                i.startup();
                i.runOnInstance(PartitionDenylistTest_RestartInjected::waitUntilStarted);
            });
            RestartFramework.at("after_all_nodes_started").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Do a cluster-wide no unavailables were recorded while the denylist was loaded.
            cluster.forEach(i -> i.runOnInstance(PartitionDenylistTest_RestartInjected::checkNoUnavailables));
            RestartFramework.at("after_unavailables_check").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    static private void checkNoUnavailables() {
        ...
        while (currentTimeMillis() < deadlineInMillis && StorageProxy.instance.getPartitionDenylistLoadSuccesses() == 0) {
            Uninterruptibles.sleepUninterruptibly(500, TimeUnit.MILLISECONDS);
        }
        Assert.assertTrue("Partition denylist must have loaded before checking unavailables", StorageProxy.instance.getPartitionDenylistLoadSuccesses() > 0);
    }
    ...
    public void checkTimerRetriesLoad() throws IOException {
        ...
        try (Cluster cluster = Cluster.build(nodeCount).withConfig(config -> config.with(NETWORK).with(GOSSIP).set("partition_denylist_enabled", true).set("denylist_initial_load_retry", "1s")).createWithoutStarting()) {
            ...
            cluster.get(1).runOnInstance(PartitionDenylistTest_RestartInjected::checkTimerActive);
            RestartFramework.at("after_timer_check").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
}
