...
public class HintsMaxSizeTest_RestartInjected_Randomized_123 extends TestBaseImpl {

    @Test
    public void testMaxHintedHandoffSize() throws Exception {
        try (Cluster cluster = init(Cluster.build(2).withDataDirCount(1).withConfig(config -> config.with(NETWORK, GOSSIP).set("hinted_handoff_enabled", true).set("max_hints_delivery_threads", "1").set("hints_flush_period", "1s").set("max_hints_size_per_host", "2MiB").set("max_hints_file_size", "1MiB")).start(), 2)) {
            final IInvokableInstance node1 = cluster.get(1);
            final IInvokableInstance node2 = cluster.get(2);
            waitForExistingRoles(cluster);
            RestartFramework.at("after_wait_existing_roles").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_get_node2_uuid").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            for (int i = 0; i < 70000; i++) {
                cluster.coordinator(1).execute(withKeyspace("INSERT INTO %s.cf (k, c1) VALUES (?, ?);"), ONE, valueOf(i), UUID.randomUUID().toString());
                if (i % 10000 == 0)
            ...
            RestartFramework.at("after_verify_hints_count").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertHintsSizes(node1, node2UUID);
            RestartFramework.at("after_first_assert_hints_sizes").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // restart the first node and check hints sizes again
            // to be sure sizes were picked up correctly
            node1.shutdown().get();
            node1.startup();
            RestartFramework.at("after_node1_restart").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertHintsSizes(node1, node2UUID);
        }
    }
    ...
    private static void waitForExistingRoles(Cluster cluster) {
        cluster.forEach(instance -> {
            await().pollDelay(1, SECONDS).pollInterval(1, SECONDS).atMost(60, SECONDS).until(() -> instance.callOnInstance(CassandraRoleManager::hasExistingRoles));
        });
    }
}
