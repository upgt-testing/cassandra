...
public class NetworkTopologyTest_RestartInjected_Randomized_123 extends TestBaseImpl {

    @Test
    public void namedDcTest() throws Throwable {
        try (ICluster<IInvokableInstance> cluster = builder().withNodeIdTopology(Collections.singletonMap(1, NetworkTopology.dcAndRack("somewhere", "rack0"))).withRack("elsewhere", "firstrack", 1).withRack("elsewhere", "secondrack", 2).withDC("nearthere", 4).createWithoutStarting()) {
    ...
    public void automaticNamedDcTest() throws Throwable {
        try (ICluster cluster = builder().withRacks(2, 1, 3).createWithoutStarting()) {
    ...
    public void noCountsAfterNamingDCsTest() throws IOException {
        builder().withDC("nameddc", 1).withDCs(1).createWithoutStarting();
    }

    @Test(expected = IllegalStateException.class)
    public void mustProvideNodeCountBeforeWithDCsTest() throws IOException {
        builder().withDCs(1).createWithoutStarting();
    }
