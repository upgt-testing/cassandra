 ...
 */
public class GuardrailItemsPerCollectionOnSSTableWriteTest_RestartInjected_Randomized_123 extends GuardrailTester {
    ...
    public static void setupCluster() throws IOException {
        cluster = init(Cluster.build(NUM_NODES).withConfig(c -> c.set("items_per_collection_warn_threshold", WARN_THRESHOLD).set("items_per_collection_fail_threshold", FAIL_THRESHOLD)).start());
    ...
    public void testSetSize() throws Throwable {
        RestartFramework.at("after_warning_inserts").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSetSizeWithUpdates() {
        ...
        RestartFramework.at("after_warning_updates").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_table_create").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        execute("UPDATE %s SET v = v - {1, 2, 3} WHERE k = 4");
        assertWarnedOnFlush(warnMessage("3", 3));
        execute("UPDATE %s SET v = v + {1, 2, 3, 4, 5} WHERE k = 5");
        RestartFramework.at("after_initial_updates").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSetSizeAfterCompaction() throws Throwable {
        ...
        assertNotWarnedOnFlush();
        RestartFramework.at("after_warning_compaction").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testListSizeAfterCompaction() throws Throwable {
        ...
        execute("INSERT INTO %s (k, v) VALUES (2, [1, 2])");
        assertNotWarnedOnFlush();
        RestartFramework.at("after_failed_compaction").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        execute("UPDATE %s SET v = v + [3, 4, 5] WHERE k = 2");
        assertWarnedOnFlush(warnMessage("2", 3));
        assertFailedOnCompact(failMessage("2", 5));
        execute("INSERT INTO %s (k, v) VALUES (3, [1, 2, 3])");
    ...
    private String warnMessage(String key, int numItems) {
        return String.format("Detected collection v in row %s in table %s with %d items, " + "this exceeds the warning threshold of %d.", key, qualifiedTableName, numItems, WARN_THRESHOLD);
    }

    private String failMessage(String key, int numItems) {
        return String.format("Detected collection v in row %s in table %s with %d items, " + "this exceeds the failure threshold of %d.", key, qualifiedTableName, numItems, FAIL_THRESHOLD);
    }
}
