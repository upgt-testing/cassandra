 ...
 */
public class GuardrailPartitionSizeTest_RestartInjected_Randomized_123 extends GuardrailTester {

    // bytes (1 MiB)
    private static final int WARN_THRESHOLD = 1024 * 1024;
    ...
    public static void setupCluster() throws IOException {
        cluster = init(Cluster.build(NUM_NODES).withConfig(c -> c.set("partition_size_warn_threshold", WARN_THRESHOLD + "B").set("partition_size_fail_threshold", FAIL_THRESHOLD + "B").set("memtable_heap_space", // avoids flushes
        "512MiB")).start());
    ...
    private void testPartitionSize(int warn, int fail) {
        ...
        RestartFramework.at("after_insert_below_threshold").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertNotWarnedOnFlush();
        ...
        RestartFramework.at("after_warn_threshold_inserts").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_fail_threshold_inserts").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_delete_reduce_size").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertNotWarnedOnFlush();
        ...
        RestartFramework.at("after_multi_partition_inserts").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_new_partition_insert").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertWarnedOnFlush(expectedMessages(3));
        RestartFramework.at("after_final_flush").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertWarnedOnCompact(expectedMessages(1, 2, 3));
        RestartFramework.at("after_final_compact").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }
    ...
    private String[] expectedMessages(int... keys) {
        ...
        for (int i = 0; i < keys.length; i++) messages[i] = String.format("Guardrail partition_size violated: Partition %s:%d", qualifiedTableName, keys[i]);
        return messages;
    }
}
