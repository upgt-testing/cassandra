...
public class GuardrailNonPartitionRestrictedQueryTest_RestartInjected_Randomized_123 extends GuardrailTester {
    ...
    public static void setupCluster() throws IOException {
        cluster = Cluster.build(3).withConfig(c -> c.with(Feature.GOSSIP, Feature.NATIVE_PROTOCOL).set("read_thresholds_enabled", "true").set("authenticator", "PasswordAuthenticator")).withDataDirCount(1).start();
    ...
    public void testGuardrailForLegacy2i() {
        prepareSchema(false);
        RestartFramework.at("after_schema_prepare").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSAIWarnThreshold() {
        ...
        RestartFramework.at("after_warn_threshold_trigger").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        String expectedMessage = tooManyIndexesReadWarnMessage(cluster.size(), 3, String.format("SELECT * FROM %s.%s WHERE v1 = %s ALLOW FILTERING", KEYSPACE, tableName, valueToQuery));
        ...
        compact(1);
        setThresholds(2, 5);
        RestartFramework.at("after_first_sstable_creation").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // notice we expect warnings from 2 nodes
        expectedMessage = tooManyIndexesReadWarnMessage(cluster.size() - 1, 3, String.format("SELECT * FROM %s.%s WHERE v1 = %s ALLOW FILTERING", KEYSPACE, tableName, valueToQuery));
        RestartFramework.at("after_schema_prepare").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertThat(getOnlyElement(executeSelect(valueToQuery, false))).contains(expectedMessage);
        ...
        compact(3);
        assertThat(executeSelect(valueToQuery, false)).isNull();
        RestartFramework.at("after_threshold_change").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSAIFailThreshold() {
        ...
        String expectedMessage = String.format("referenced %s SSTable indexes for a query without restrictions on partition key " + "and aborted the query SELECT * FROM %s.%s WHERE v1 = %s ALLOW FILTERING", 6, KEYSPACE, tableName, valueToQuery);
        ...
        assertThat(executeSelect(valueToQuery, false)).isNull();
        RestartFramework.at("after_fail_threshold_trigger").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // set thresholds back on nodes 2 and 3
        setThresholds(2, 5, 2, 3);
        assertThat(getOnlyElement(executeSelect(valueToQuery, true))).contains(expectedMessage);
    ...
    private void testGuardrailInternal() {
        enableGuardrail();
        Awaitility.await().pollDelay(5, TimeUnit.SECONDS).atMost(1, TimeUnit.MINUTES).pollInterval(5, TimeUnit.SECONDS).until(() -> {
            try {
    ...
    private long createSSTables(int numberOfSSTables, int... nodesToFlush) {
        ...
        for (int i = 0; i < numberOfSSTables; i++) {
            ...
            cluster.coordinator(1).execute(format("INSERT INTO %s (k, c, v1, v2) VALUES (?, ?, ?, ?)"), ConsistencyLevel.ALL, k, k, value, value + 10);
    ...
    private void setThresholds(int warn, int fail, int... nodes) {
        ...
        instances.forEach(instance -> instance.acceptsOnInstance((IIsolatedExecutor.SerializableBiConsumer<Integer, Integer>) (w, f) -> Guardrails.instance.setSaiSSTableIndexesPerQueryThreshold(w, f)).accept(warn, fail));
    ...
    private void assertTresholds(int expectedWarn, int expectedFail, int... nodes) {
        ...
        instances.forEach(instance -> {
            assertEquals(expectedWarn, instance.callsOnInstance((IIsolatedExecutor.SerializableCallable<Integer>) () -> Guardrails.instance.getSaiSSTableIndexesPerQueryWarnThreshold()).call().intValue());
            assertEquals(expectedFail, instance.callsOnInstance((IIsolatedExecutor.SerializableCallable<Integer>) () -> Guardrails.instance.getSaiSSTableIndexesPerQueryFailThreshold()).call().intValue());
        });
    }
