 ...
 */
public class GuardrailDiskUsageTest_RestartInjected_Randomized_456 extends GuardrailTester {
    ...
    public static void setupCluster() throws IOException {
        ...
        cluster = init(Cluster.build(2).withInstanceInitializer(DiskStateInjection::install).withConfig(c -> c.with(Feature.GOSSIP, Feature.NATIVE_PROTOCOL).set("data_disk_usage_max_disk_size", "10GiB").set("data_disk_usage_percentage_warn_threshold", 98).set("data_disk_usage_percentage_fail_threshold", 99).set("authenticator", "PasswordAuthenticator")).start(), 1);
    ...
    public void testDiskUsage() throws Throwable {
        ...
        RestartFramework.at("after_not_available_writes").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        String insert = format("INSERT INTO %s(k, v) VALUES (?, 0)");
        RestartFramework.at("after_not_available_state").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_both_full_writes").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_stuffed_writes").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        for (int i = 0; i < NUM_ROWS; i++) {
            ResultSet rs = driverSession.execute(insert, i);
            Assertions.assertThat(rs.getExecutionInfo().getWarnings()).isEmpty();
        }
        RestartFramework.at("after_final_spacious_writes").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_back_to_spacious_state").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_full_writes").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        Assertions.assertThat(numWarnings).isGreaterThan(0).isLessThan(NUM_ROWS);
        RestartFramework.at("after_spacious_writes").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_both_full_state").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_stuffed_state").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_full_state").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public static class DiskStateInjection {
        ...
        private static void install(ClassLoader cl, int node) {
            new ByteBuddy().rebase(DiskUsageMonitor.class).method(named("getState")).intercept(MethodDelegation.to(DiskStateInjection.class)).make().load(cl, ClassLoadingStrategy.Default.INJECTION);
        }
