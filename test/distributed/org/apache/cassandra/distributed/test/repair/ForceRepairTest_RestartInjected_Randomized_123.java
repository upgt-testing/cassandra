...
public class ForceRepairTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    private void force(boolean includeDifference) throws IOException {
        ...
        try (Cluster cluster = Cluster.build(3).withConfig(c -> c.set("hinted_handoff_enabled", false).with(Feature.values())).start()) {
            ...
            RestartFramework.at("after_failure_detector_wait").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_repair_tests").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            if (includeDifference) {
    ...
    private static void assertNoRepairedAt(Cluster cluster) {
        ...
        for (int i = 0; i < repairedAt.size(); i++) {
            ...
            for (long a : array) Assertions.assertThat(a).describedAs("node%d had a repaired sstable", i + 1).isEqualTo(0);
        }
    }
