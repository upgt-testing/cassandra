 ...
 * requests.
 */
public class ConcurrentValidationRequestsTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public void testConcurrentValidations() throws Throwable {
        try (Cluster cluster = init(builder().withNodes(NODES).withInstanceInitializer(ConcurrentValidationRequestsTest_RestartInjected.BBHelper::install).withConfig(c -> c.set("concurrent_merkle_tree_requests", MAX_REQUESTS).with(NETWORK, GOSSIP)).start(), RF)) {
            for (int t = 1; t <= TABLES; t++) cluster.schemaChange(withKeyspace("CREATE TABLE %s.t" + t + " (k int PRIMARY KEY, v int)"));
            RestartFramework.at("after_tables_create").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            int v = 0;
    ...
    public static class BBHelper {
        ...
        public static void install(ClassLoader cl, int node) {
            if (node != 1)
                return;
            new ByteBuddy().rebase(ValidationTask.class).method(named("run")).intercept(MethodDelegation.to(ConcurrentValidationRequestsTest_RestartInjected.BBHelper.class)).method(named("treesReceived")).intercept(MethodDelegation.to(ConcurrentValidationRequestsTest_RestartInjected.BBHelper.class)).make().load(cl, ClassLoadingStrategy.Default.INJECTION);
        }
