 ...
 */
public class HintedHandoffAddRemoveNodesTest_RestartInjected_Randomized_456 extends TestBaseImpl {

    @SuppressWarnings("Convert2MethodRef")
    @Test
    public void shouldAvoidHintTransferOnDecommission() throws Exception {
        try (Cluster cluster = init(builder().withNodes(3).withConfig(config -> config.set("transfer_hints_on_decommission", false).with(GOSSIP)).withoutVNodes().start())) {
            ...
            RestartFramework.at("after_hint_write").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            cluster.get(1).nodetoolResult("decommission", "--force").asserts().success();
            RestartFramework.at("after_decommission").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void shouldStreamHintsDuringDecommission() throws Exception {
        try (Cluster cluster = builder().withNodes(4).withConfig(config -> config.with(NETWORK, GOSSIP, NATIVE_PROTOCOL)).withoutVNodes().start()) {
            ...
            RestartFramework.at("after_data_population").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_hints_accumulate").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            // Decomision node 1...
            ...
            RestartFramework.at("after_first_decommission").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_node4_startup").on(cluster).restart("node").withIndex(3).withMode(RestartMode.GRACEFUL).execute();
            await().atMost(30, SECONDS).pollDelay(3, SECONDS).until(() -> count(cluster, "decom_hint_test", 4) >= totalHints);
            RestartFramework.at("after_hint_delivery").on(cluster).restart("node").withIndex(3).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_final_decommissions").on(cluster).restart("node").withIndex(3).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void shouldBootstrapWithHintsOutstanding() throws Exception {
        try (Cluster cluster = builder().withNodes(3).withTokenSupplier(TokenSupplier.evenlyDistributedTokens(4, 1)).withNodeIdTopology(NetworkTopology.singleDcNetworkTopology(4, "dc0", "rack0")).withConfig(config -> config.with(NETWORK, GOSSIP, NATIVE_PROTOCOL)).start()) {
            cluster.schemaChange(withKeyspace("CREATE KEYSPACE %s WITH replication = {'class': 'SimpleStrategy', 'replication_factor': 2}"));
            ...
            RestartFramework.at("after_bootstrap").on(cluster).restart("node").withIndex(3).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_node3_startup").on(cluster).restart("node").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
            await().atMost(30, SECONDS).pollDelay(3, SECONDS).until(() -> count(cluster, "boot_hint_test", 3) >= totalHints);
            RestartFramework.at("after_hint_delivery").on(cluster).restart("node").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void populate(Cluster cluster, String table, int coordinator, int start, int count, ConsistencyLevel cl) {
        for (int i = start; i < start + count; i++) cluster.coordinator(coordinator).execute("INSERT INTO " + KEYSPACE + '.' + table + " (key, value) VALUES (?, ?)", cl, i, i);
    }

    @SuppressWarnings("SameParameterValue")
    private void verify(Cluster cluster, String table, int coordinator, int start, int count, ConsistencyLevel cl) {
        for (int i = start; i < start + count; i++) {
            Object[][] row = cluster.coordinator(coordinator).execute("SELECT key, value FROM " + KEYSPACE + '.' + table + " WHERE key = ?", cl, i);
