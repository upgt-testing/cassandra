 ...
 */
@Ignore
public class ResourceLeakTest_RestartInjected_Randomized_456 extends TestBaseImpl {

    // Parameters to adjust while hunting for leaks
    // Set this value high to crash on leaks, or low when tracking down an issue.
    final int numTestLoops = 1;

    // Dump heap & possibly files every loop
    final boolean dumpEveryLoop = false;

    // Call lsof whenever dumping resources
    final boolean dumpFileHandles = false;

    // Whether to explicitly force finalization/gc for smaller heap dumps
    final boolean forceCollection = false;

    // Number of millis to wait before final resource dump to give gc a chance
    final long finalWaitMillis = 0L;
    ...
    static String outputFilename(String base, String description, String extension) {
        Path p = FileSystems.getDefault().getPath("build", "test", String.join("-", when, base, description) + extension);
        return p.toString();
    }
    ...
    public void looperEverythingTest() throws Throwable {
        doTest(2, config -> config.with(Feature.values()), cluster -> {
            testJmx(cluster);
        });
        if (forceCollection) {
            System.runFinalization();
