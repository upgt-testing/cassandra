...
public class MetricsTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public void testMetrics() throws Throwable {
        try (ICluster<IInvokableInstance> cluster = init(builder().withNodes(1).start())) {
            ...
            RestartFramework.at("after_single_counter_check").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Assert.assertEquals(ImmutableMap.of("test_counter", 100L, "test_counter_2", 101L), cluster.get(1).metrics().getCounters(s -> s.startsWith("test_counter")));
        }
    }
}
