...
public class FqlReplayDDLExclusionTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void test() throws Throwable {
        try (final Cluster cluster = init(builder().withNodes(1).withConfig(updater -> updater.with(NETWORK, GOSSIP, NATIVE_PROTOCOL)).start())) {
            ...
            try (com.datastax.driver.core.Cluster c = com.datastax.driver.core.Cluster.builder().addContactPoint("127.0.0.1").build();
                ...
                RestartFramework.at("after_negative_replay").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                ...
                RestartFramework.at("after_positive_replay").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
