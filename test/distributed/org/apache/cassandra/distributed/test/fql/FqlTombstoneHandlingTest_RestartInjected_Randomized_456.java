...
public class FqlTombstoneHandlingTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    public void testNullCellBindingInBatch() {
        ...
        RestartFramework.at("after_new_insert").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_replay").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_batch_insert").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        ToolRunner.ToolResult runner = ToolRunner.invokeClass("org.apache.cassandra.fqltool.FullQueryLogTool", "dump", "--", temporaryFolder.getRoot().getAbsolutePath());
        assertTrue(runner.getStdout().contains(insertTemplate));
        assertEquals(0, runner.getExitCode());
        RestartFramework.at("after_disable_fql").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_fql_dump").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_enable_fql").on(CLUSTER).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        runner = ToolRunner.invokeClass("org.apache.cassandra.fqltool.FullQueryLogTool", "replay", "--keyspace", KEYSPACE, "--target", "127.0.0.1", "--", temporaryFolder.getRoot().getAbsolutePath());
        assertEquals(0, runner.getExitCode());
