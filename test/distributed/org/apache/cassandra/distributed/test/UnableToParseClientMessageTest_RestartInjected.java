...
public class UnableToParseClientMessageTest_RestartInjected extends TestBaseImpl
{
    ...
    private void test(String expectedError, Message.Request request, Predicate<List<String>> shouldCheckLogs) throws IOException
    {
        ...
        RestartFramework.at("after_node_retrieval")
            .on(CLUSTER)
            .restart("node")
            .withIndex(0)
        ...
        RestartFramework.at("after_metrics_baseline")
            .on(CLUSTER)
            .restart("node")
            .withIndex(0)
            .withMode(RestartMode.GRACEFUL)
        ...
        try (SimpleClient client = SimpleClient.builder("127.0.0.1", 9042).protocolVersion(version).useBeta().build())
        {
            ...
            RestartFramework.at("after_client_connect")
                .on(CLUSTER)
                .restart("node")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
            ...
            RestartFramework.at("after_bad_message_sent")
                .on(CLUSTER)
                .restart("node")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
            ...
            RestartFramework.at("after_log_check")
                .on(CLUSTER)
                .restart("node")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
