...
public class NativeMixedVersionTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public void v4ConnectionCleansUpThreadLocalState() throws IOException {
        ...
        try (WithProperties properties = new WithProperties().set(IO_NETTY_EVENTLOOP_THREADS, 1);
            Cluster cluster = Cluster.build(1).withConfig(c -> c.with(Feature.values()).set("read_thresholds_enabled", true).set("local_read_size_warn_threshold", "1KiB")).start()) {
            init(cluster);
            ...
            RestartFramework.at("after_v5_protocol_test").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_v3_protocol_test").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
