...
public class CasWriteTest_RestartInjected_Randomized_456 extends TestBaseImpl {
    ...
    private static final Logger logger = LoggerFactory.getLogger(CasWriteTest_RestartInjected.class);
    ...
    public static void setupCluster() throws Throwable {
        cluster = init(Cluster.build().withNodes(3).withConfig(config -> config.set("paxos_state_purging", "repaired").set("paxos_variant", "v2").set("paxos_cache_size", "0MiB")).start());
    ...
    public void testCasWriteSuccessWithNoContention() {
        ...
        RestartFramework.at("after_cas_update").on((Cluster) cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertRows(cluster.coordinator(1).execute("SELECT * FROM " + KEYSPACE + ".tbl WHERE pk = 1", ConsistencyLevel.QUORUM), row(1, 1, 1));
        RestartFramework.at("after_cas_insert").on((Cluster) cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        cluster.coordinator(1).execute("UPDATE " + KEYSPACE + ".tbl SET v = 2 WHERE pk = 1 AND ck = 1 IF v = 1", ConsistencyLevel.QUORUM);
        assertRows(cluster.coordinator(1).execute("SELECT * FROM " + KEYSPACE + ".tbl WHERE pk = 1", ConsistencyLevel.QUORUM), row(1, 1, 2));
    }

    @Test
    public void testCasWriteTimeoutAtPreparePhase_ReqLost() {
        RestartFramework.at("after_filter_setup").on((Cluster) cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        expectCasWriteTimeout();
        // drop the internode messages to acceptors
        cluster.filters().verbs(Verb.PAXOS_PREPARE_REQ.id).from(1).to(2, 3).drop().on();
        // drop the internode messages to acceptors
        cluster.filters().verbs(Verb.PAXOS2_PREPARE_REQ.id).from(1).to(2, 3).drop().on();
    ...
    public void testCasWriteTimeoutAtPreparePhase_RspLost() {
        expectCasWriteTimeout();
        // drop the internode messages to acceptors
        cluster.filters().verbs(Verb.PAXOS_PREPARE_RSP.id).from(2, 3).to(1).drop().on();
        RestartFramework.at("after_filter_setup").on((Cluster) cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        // drop the internode messages to acceptors
        cluster.filters().verbs(Verb.PAXOS2_PREPARE_RSP.id).from(2, 3).to(1).drop().on();
        cluster.coordinator(1).execute(mkUniqueCasInsertQuery(1), ConsistencyLevel.QUORUM);
    }
    ...
    public void casWriteContentionTimeoutTest() throws InterruptedException {
        RestartFramework.at("before_contention_test").on((Cluster) cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        testWithContention(101, Arrays.asList(1, 3), c -> {
            c.filters().reset();
        ...
        }, failure -> failure.get() != null && failure.get().getClass().getCanonicalName().equals(CasWriteTimeoutException.class.getCanonicalName()), "Expecting cause to be CasWriteTimeoutException");
    }

    private void testWithContention(int testUid, List<Integer> contendingNodes, Consumer<ICluster> setupForEachRound, Function<AtomicReference<Throwable>, Boolean> expectedException, String assertHintMessage) throws InterruptedException {
        ...
        while (!hasExpectedException.get()) {
            ...
            contendingNodes.forEach(nodeId -> {
                ...
                futures.add(es.submit(() -> {
                    try {
                        latch.countDown();
                        // help threads start at approximately same time
    ...
    public void testWriteUnknownResult() {
        ...
        RestartFramework.at("after_filter_reset").on((Cluster) cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try {
            ...
            Assert.assertEquals("Expecting cause to be " + exceptionClass.getSimpleName(), exceptionClass.getCanonicalName(), t.getClass().getCanonicalName());
            return;
        }
    ...
    public void testStaleCommitInSystemPaxos() throws InterruptedException {
        cluster.filters().reset();
        int extraKeys = 10;
        RestartFramework.at("after_cas_delete").on((Cluster) cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_cas_insert").on((Cluster) cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        cluster.get(1).nodetool("repair", "--paxos-only", KEYSPACE, "tbl");
        RestartFramework.at("after_paxos_repair").on((Cluster) cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("before_compact").on((Cluster) cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
        for (int k = 1; k <= 3; ++k) {
            ((IInvokableInstance) cluster.get(k)).runOnInstance(() -> {
                ColumnFamilyStore cfs = Keyspace.open("system").getColumnFamilyStore("paxos");
        ...
        RestartFramework.at("after_paxos_purge_config").on((Cluster) cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        while (FBUtilities.timestampMicros() - (GC_GRACE_SECONDS * 1000_000) < TimeUUID.rawTimestampToUnixMicros(deleteTimestamp)) Thread.sleep(1000);
        cluster.get(1).nodetool("compact", KEYSPACE, "tbl");
        RestartFramework.at("after_compact").on((Cluster) cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        for (int i = 1; i <= 3; ++i) {
