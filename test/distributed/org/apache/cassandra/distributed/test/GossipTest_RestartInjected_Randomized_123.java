...
public class GossipTest_RestartInjected_Randomized_123 extends TestBaseImpl {

    private static final Logger logger = LoggerFactory.getLogger(GossipTest_RestartInjected.class);
    ...
    public void nodeDownDuringMove() throws Throwable {
        ...
        try (Cluster cluster = Cluster.build(2 + liveCount).withConfig(config -> config.with(NETWORK).with(GOSSIP)).createWithoutStarting()) {
            ...
            RestartFramework.at("after_normal_state").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_moving_state").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_late_node_startup").on(cluster).restart("node").withIndex(2).withMode(RestartMode.GRACEFUL).execute();
    ...
    public static class BBGossiper {
        ...
        public static void install(ClassLoader cl, Integer i) {
            new ByteBuddy().rebase(Gossiper.class).method(named("sendGossip")).intercept(MethodDelegation.to(BBGossiper.class)).make().load(cl, ClassLoadingStrategy.Default.INJECTION);
        }
        ...
        try (Cluster cluster = builder().withNodes(originalNodeCount).withTokenSupplier(TokenSupplier.evenlyDistributedTokens(expandedNodeCount)).withNodeIdTopology(NetworkTopology.singleDcNetworkTopology(expandedNodeCount, "dc0", "rack0")).withConfig(config -> config.with(NETWORK, GOSSIP)).withInstanceInitializer(BBGossiper::install).start()) {
            ...
            for (int i = 1; i <= originalNodeCount; ++i) {
                cluster.get(i).acceptsOnInstance((InetSocketAddress address) -> {
                    EndpointState ep;
                    InetAddressAndPort endpoint = toCassandraInetAddressAndPort(address);
                    while (null == (ep = Gossiper.instance.getEndpointStateForEndpoint(endpoint)) || ep.getApplicationState(ApplicationState.HOST_ID) == null) LockSupport.parkNanos(TimeUnit.MILLISECONDS.toNanos(10L));
                }).accept(newNodeAddress);
            }
            RestartFramework.at("after_new_node_gossip_propagate").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_node_removed").on(cluster).restart("node").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_gossip_recovery").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            nodeRecovered.get();
        }
    }
    ...
    public static class BBBootstrapInterceptor {
        ...
        static void install(ClassLoader cl, int nodeNumber) {
            if (nodeNumber != 2)
                return;
            new ByteBuddy().rebase(StorageService.class).method(named("bootstrap").and(takesArguments(2))).intercept(MethodDelegation.to(BBBootstrapInterceptor.class)).make().load(cl, ClassLoadingStrategy.Default.INJECTION);
        }
    ...
    public void testPreventStoppingGossipDuringBootstrap() throws Exception {
        ExecutorService es = Executors.newFixedThreadPool(1);
        try (Cluster cluster = builder().withNodes(2).withConfig(config -> config.with(GOSSIP).with(NETWORK).set("auto_bootstrap", true)).withInstanceInitializer(BBBootstrapInterceptor::install).createWithoutStarting();
    ...
    public void testPreventEnablingGossipDuringMove() throws Exception {
        try (Cluster cluster = builder().withNodes(2).withConfig(config -> config.with(GOSSIP).with(NETWORK)).start()) {
            ...
            RestartFramework.at("after_gossip_operations").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public void gossipShutdownUpdatesTokenMetadata() throws Exception {
        ...
        try (Cluster cluster = Cluster.build(3).withConfig(c -> c.with(Feature.GOSSIP, Feature.NETWORK)).withInstanceInitializer(FailureHelper::installMoveFailure).withoutVNodes().start()) {
            init(cluster, 2);
            populate(cluster);
            ...
            RestartFramework.at("after_move_initiation").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void restartGossipOnGossippingOnlyMember() throws Throwable {
        ...
        int expandedNodeCount = originalNodeCount + 1;
        try (Cluster cluster = builder().withNodes(originalNodeCount).withTokenSupplier(evenlyDistributedTokens(expandedNodeCount, 1)).withNodeIdTopology(singleDcNetworkTopology(expandedNodeCount, "dc0", "rack0")).withConfig(config -> config.with(NETWORK, GOSSIP)).start()) {
            IInstanceConfig config = cluster.newInstanceConfig();
            ...
            assertTrue(gossippingOnlyMember.callOnInstance((IIsolatedExecutor.SerializableCallable<Boolean>) () -> StorageService.instance.isGossipRunning()));
            RestartFramework.at("after_gossip_running_check").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            gossippingOnlyMember.runOnInstance((IIsolatedExecutor.SerializableRunnable) () -> StorageService.instance.stopGossiping());
            assertFalse(gossippingOnlyMember.callOnInstance((IIsolatedExecutor.SerializableCallable<Boolean>) () -> StorageService.instance.isGossipRunning()));
            gossippingOnlyMember.runOnInstance((IIsolatedExecutor.SerializableRunnable) () -> StorageService.instance.startGossiping());
            RestartFramework.at("after_gossip_restart").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertTrue(gossippingOnlyMember.callOnInstance((IIsolatedExecutor.SerializableCallable<Boolean>) () -> StorageService.instance.isGossipRunning()));
        }
    }
    ...
    void assertPendingRangesForPeer(final boolean expectPending, final InetSocketAddress movingAddress, final Cluster cluster) {
        for (IInvokableInstance inst : new IInvokableInstance[] { cluster.get(1), cluster.get(3) }) {
            boolean hasPending = inst.appliesOnInstance((InetSocketAddress address) -> {
                ...
                boolean isMoving = StorageService.instance.getTokenMetadata().getMovingEndpoints().stream().map(pair -> pair.right).anyMatch(peer::equals);
                return isMoving && !StorageService.instance.getTokenMetadata().getPendingRanges(KEYSPACE, peer).isEmpty();
            }).apply(movingAddress);
            assertEquals(String.format("%s should %shave PENDING RANGES for %s", inst.broadcastAddress().getHostString(), expectPending ? "" : "not ", movingAddress), hasPending, expectPending);
        }
    }
    ...
    public static class FailureHelper {
        ...
        static void installMoveFailure(ClassLoader cl, int nodeNumber) {
            if (nodeNumber == 2) {
                new ByteBuddy().redefine(StreamPlan.class).method(named("execute")).intercept(MethodDelegation.to(FailureHelper.class)).make().load(cl, ClassLoadingStrategy.Default.INJECTION);
            }
        }
