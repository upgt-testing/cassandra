...
public class PagingTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public void testPagingWithRangeTombstones() throws IOException {
        try (Cluster cluster = init(Cluster.build(2).start())) {
            ...
            RestartFramework.at("after_range_delete").on(cluster).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            Iterator<Object[]> iter = cluster.coordinator(1).executeWithPaging("SELECT pk,ck,regular FROM " + KEYSPACE + ".tbl " + "WHERE pk=? AND ck>=? ORDER BY ck DESC;", ConsistencyLevel.QUORUM, 1, 1, 1);
            assertRows(iter, row(1, 3, 3), row(1, 2, 2), row(1, 1, 1));
        }
    }
}
