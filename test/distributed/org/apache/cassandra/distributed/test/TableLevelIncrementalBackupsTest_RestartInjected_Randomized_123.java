...
public class TableLevelIncrementalBackupsTest_RestartInjected_Randomized_123 extends TestBaseImpl {
    ...
    public void testIncrementalBackupEnabledCreateTable() throws Exception {
        try (Cluster c = getCluster(true)) {
            ...
            RestartFramework.at("after_table_create_with_backups_enabled").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_first_flush_with_backups").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            c.coordinator(1).execute(withKeyspace("INSERT INTO %s.test_table (a, b) VALUES ('a', 1)"), ALL);
            flush(c, KEYSPACE);
            assertBackupSSTablesCount(c, 2, true, KEYSPACE, "test_table");
            RestartFramework.at("after_second_flush_with_backups").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            c.schemaChange(withKeyspace("DROP TABLE %s.test_table"));
            RestartFramework.at("after_drop_table_with_backups").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_table2_create_with_backups_disabled").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_flush_without_backups").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testIncrementalBackupEnabledAlterTable() throws Exception {
        try (Cluster c = getCluster(true)) {
            ...
            disableCompaction(c, KEYSPACE, "test_table");
            RestartFramework.at("after_alter_create_with_backups_disabled").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            c.coordinator(1).execute(withKeyspace("INSERT INTO %s.test_table (a, b) VALUES ('a', 1)"), ALL);
            flush(c, KEYSPACE);
            assertBackupSSTablesCount(c, 0, false, KEYSPACE, "test_table");
            RestartFramework.at("after_flush_with_backups_disabled").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            c.schemaChange(withKeyspace("ALTER TABLE %s.test_table  WITH incremental_backups = true"));
            RestartFramework.at("after_alter_enable_backups").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            c.coordinator(1).execute(withKeyspace("INSERT INTO %s.test_table (a, b) VALUES ('a', 1)"), ALL);
            flush(c, KEYSPACE);
            assertBackupSSTablesCount(c, 1, true, KEYSPACE, "test_table");
            RestartFramework.at("after_flush_with_backups_enabled").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            c.schemaChange(withKeyspace("DROP TABLE %s.test_table"));
            c.schemaChange(withKeyspace("CREATE TABLE %s.test_table2 (a text primary key, b int) WITH incremental_backups = true"));
            disableCompaction(c, KEYSPACE, "test_table2");
            RestartFramework.at("after_alter_table2_create_with_backups_enabled").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            c.coordinator(1).execute(withKeyspace("INSERT INTO %s.test_table2 (a, b) VALUES ('a', 1)"), ALL);
            flush(c, KEYSPACE);
            assertBackupSSTablesCount(c, 1, true, KEYSPACE, "test_table2");
            RestartFramework.at("after_alter_table2_flush_with_backups").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            c.schemaChange(withKeyspace("ALTER TABLE %s.test_table2  WITH incremental_backups = false"));
            RestartFramework.at("after_alter_disable_backups").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            c.coordinator(1).execute(withKeyspace("INSERT INTO %s.test_table2 (a, b) VALUES ('a', 1)"), ALL);
            flush(c, KEYSPACE);
            assertBackupSSTablesCount(c, 1, false, KEYSPACE, "test_table2");
            RestartFramework.at("after_alter_final_flush").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            c.schemaChange(withKeyspace("DROP TABLE %s.test_table2"));
        }
    }
    ...
    public void testIncrementalBackupWhenCreateTableByDefault() throws Exception {
        try (Cluster c = getCluster(true)) {
            ...
            disableCompaction(c, KEYSPACE, "test_table");
            RestartFramework.at("after_default_table_create").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            c.coordinator(1).execute(withKeyspace("INSERT INTO %s.test_table (a, b) VALUES ('a', 1)"), ALL);
            flush(c, KEYSPACE);
            assertBackupSSTablesCount(c, 1, true, KEYSPACE, "test_table");
            RestartFramework.at("after_default_first_flush").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            c.schemaChange(withKeyspace("ALTER TABLE %s.test_table  WITH incremental_backups = false"));
            RestartFramework.at("after_default_alter_disable").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            c.coordinator(1).execute(withKeyspace("INSERT INTO %s.test_table (a, b) VALUES ('a', 1)"), ALL);
            flush(c, KEYSPACE);
            assertBackupSSTablesCount(c, 1, false, KEYSPACE, "test_table");
            RestartFramework.at("after_default_flush_disabled").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            c.schemaChange(withKeyspace("ALTER TABLE %s.test_table  WITH incremental_backups = true"));
            RestartFramework.at("after_default_alter_reenable").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            c.coordinator(1).execute(withKeyspace("INSERT INTO %s.test_table (a, b) VALUES ('a', 1)"), ALL);
            flush(c, KEYSPACE);
            assertBackupSSTablesCount(c, 2, true, KEYSPACE, "test_table");
            RestartFramework.at("after_default_final_flush").on(c).restart("node").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            c.schemaChange(withKeyspace("DROP TABLE %s.test_table"));
        }
    }

    private Cluster getCluster(boolean incrementalBackups) throws IOException {
        return init(build(2).withDataDirCount(1).withConfig(c -> c.with(Feature.GOSSIP).set("incremental_backups", incrementalBackups)).start());
    }

    private void flush(Cluster cluster, String keyspace) {
        for (int i = 1; i < cluster.size() + 1; i++) cluster.get(i).flush(keyspace);
    }
    ...
    private static void assertSSTablesCount(Set<Descriptor> descs, String tableName, int expectedTablesCount) {
        ...
        List<String> seqSSTables = descs.stream().filter(descriptorPredicate).map(descriptor -> descriptor.baseFile().toString()).sorted().collect(Collectors.toList());
